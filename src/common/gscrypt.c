...
static gsi_i32 gsCryptRSAOAEPEncryptBuffer(const gsCryptRSAKey* publicKey,
                                           const unsigned char* plainText,
                                           gsi_u32 len,
                                           unsigned char buffer[GS_CRYPT_RSA_BYTE_SIZE]);
static gsi_i32 gsCryptRSAOAEPDecryptBuffer(const gsCryptRSAKey* privateKey,
...
#else
static gsi_i32 gsCryptRSAPKCS1EncryptBuffer(const gsCryptRSAKey* publicKey,
                                            const unsigned char* plainText,
                                            gsi_u32 len,
                                            unsigned char buffer[GS_CRYPT_RSA_BYTE_SIZE]);
static gsi_i32 gsCryptRSAPKCS1DecryptBuffer(const gsCryptRSAKey* privateKey,
                                            const unsigned char ciperText[GS_CRYPT_RSA_BYTE_SIZE],
...
gsi_i32 gsCryptRSAEncryptBuffer(const gsCryptRSAKey* publicKey,
                                const unsigned char* plainText,
                                gsi_u32 len,
                                unsigned char buffer[GS_CRYPT_RSA_BYTE_SIZE])
{
...
#ifdef GS_CRYPT_RSA_ES_OAEP
gsi_i32 gsCryptRSAOAEPEncryptBuffer(const gsCryptRSAKey* publicKey,
                                    const unsigned char* plainText,
                                    gsi_u32 len,
                                    unsigned char buffer[GS_CRYPT_RSA_BYTE_SIZE])
{
...
#if (GS_CRYPT_HASHSIZE == GS_CRYPT_MD5_HASHSIZE)
    const gsi_u8 lhash[GS_CRYPT_HASHSIZE] = {0xd4,
                                             0x1d,
                                             0xd4,
                                             0x1d,
                                             0x8c,
                                             ...
                                             0x7e}; // hash of ""
#else
    const gsi_u8 lhash[GS_CRYPT_HASHSIZE] = {0xda, 0x39, 0xa3, 0xee, 0x5e, 0x6b, 0x4b, 0x0d, 0x32, 0x55,
                                             0xbf, 0xef, 0x95, 0x60, 0x18, 0x90, 0xaf, 0xd8, 0x07, 0x09}; // hash of ""
#endif
...
#else
gsi_i32 gsCryptRSAPKCS1EncryptBuffer(const gsCryptRSAKey* publicKey,
                                     const unsigned char* plainText,
                                     gsi_u32 len,
                                     unsigned char buffer[GS_CRYPT_RSA_BYTE_SIZE])
{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_i32 gsCryptRSADecryptBuffer(const gsCryptRSAKey* privateKey,
                                const unsigned char cipherText[GS_CRYPT_RSA_BYTE_SIZE],
                                unsigned char* plainText,
                                gsi_u32* lenout)
{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_i32 gsCryptRSASignData(const gsCryptRSAKey* privateKey,
                           const unsigned char* plainText,
                           gsi_u32 plainTextLen,
                           unsigned char* signedDataOut,
                           gsi_u32* lenOut)
{
...
gsi_i32 gsCryptRSASignHash(const gsCryptRSAKey* privateKey,
...
gsi_i32 gsCryptRSAVerifySignedHash(const gsCryptRSAKey* publicKey,
                                   const unsigned char* hash,
                                   gsi_u32 hashLen,
                                   ...
                                   gsi_u32 sigLen)
{
    ...
    if ((i + sizeof(md5Header) + GS_CRYPT_MD5_HASHSIZE) == (lintRSASignature.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES)) {
        ...
        if (0 != memcmp(&packet[i], hash, GS_CRYPT_MD5_HASHSIZE))
            return -5;
    } else if ((i + sizeof(sha1Header) + GS_CRYPT_SHA1_HASHSIZE)
               == (lintRSASignature.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES)) {
        // SHA1 Hash
        //   1. verify header
