...
#else
    const char iop_params[] = SNPS2_IP_ADDR "\x00" SNPS2_SUB_MSK "\x00" SNPS2_GATEWAY;
...
int ent_cnf_set_config(void)
{
    ...
    ret =
        sceSifCallRpc(&cd, ENT_CNF_SIFRPC_SET_CONFIG, 0, NULL, 0, (void*)rpc_buffer, ee_rpc_size(sizeof(u_int)), 0, 0);
    if (ret < 0) {
...
//	PreparePowerOff();
    ...
    result =
        load_set_conf_extra(&gGSIInsockClientData, gGSIInsockSocketBuffer, NET_DB, USR_CONF_NAME, sceLIBNETF_AUTO_UPIF);
    if (result < 0)
        return result;

    result = sceLibnetWaitGetAddress(
        &gGSIInsockClientData, gGSIInsockSocketBuffer, if_id, sceLIBNET_MAX_INTERFACE, &myaddr, sceLIBNETF_AUTO_UPIF);
    if (result < 0)
        return result;
