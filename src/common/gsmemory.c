...
MEM_CHUNK*
MEM_CHUNK_POOLAllocChunk(MEM_CHUNK_POOL* _this,
                         size_t Size,
                         int Alignment,
                         gsi_bool Backwards); //int Alignment = PTR_ALIGNMENT, gsi_bool Backwards = gsi_false);
...
void MEM_CHUNK_POOLFillMemoryTable(MEM_CHUNK_POOL* _this,
                                   char* Table,
                                   const int TableSize,
                                   gsi_u32 _HeapStart,
                                   gsi_u32 _HeapSize);
...
gsi_bool MEM_CHUNK_POOLItemIsInPoolMemory(MEM_CHUNK_POOL* _this, void* ptr)
{
    ...
    return (((gsi_uint)ptr >= (gsi_uint)MEM_CHUNKMemPtrGet(_this->HeaderStart))
            && ((gsi_uint)ptr <= (gsi_uint)MEM_CHUNKMemPtrGet(_this->HeaderEnd)));
}
...
//--------------------------------------------------------------------------
{
    ...
    while (header) {
        MP_ASSERT((header->next == NULL)
                  || ((gsi_uint)header < (gsi_uint)header->next)) // infinite loop or out of place
        MP_ASSERT((header->prev == NULL)
                  || ((gsi_uint)header->prev < (gsi_uint)header))               // infinite loop or out of place
            ...
            if ((header != _this->HeaderStart) && (header != _this->HeaderEnd)) {
                ...
                if (_LogUse) {
                    gsDebugFormat(GSIDebugCat_App,
                                  ...
                                  MEM_CHUNKMemUsedGet(header),
                                  MemMgrBufferGetName((gsMemMgrContext)type));
                }
            }
        }
...
void MEM_CHUNK_POOLMemStatsGet(MEM_CHUNK_POOL* _this, MEM_STATS* pS)
{
    ...
    while (header) {
        ...
        if (!MEM_CHUNKIsFree(header)) {
            ...
            // free chunk
            MP_ASSERT((header->NextFree == NULL)
                      || ((gsi_uint)header < (gsi_uint)header->NextFree)) // infinite loop or out of place
...
// find pool corresponding to mem ptr.
{
    ...
    for (i = 0; i < gsMemMgrContext_Count; i++) {
        if (MEM_CHUNK_POOLIsValid(&gChunkPool[i]) && MEM_CHUNK_POOLItemIsInPoolMemory(&gChunkPool[i], ptr)) {
...
gsMemMgrContext gsMemMgrCreate(gsMemMgrContext context, const char* PoolName, void* thePoolBuffer, size_t thePoolSize)
{
    ...
    if (context == gsMemMgrContext_Count) {
        // Warn!!!!
        gsDebugFormat(GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "Out of memory context handles!\n");
        GS_ASSERT(0);
...
gsi_u32 gsMemMgrMemLargestAvailGet(gsMemMgrContext context)
{
    ...
    GS_ASSERT_STR(MEM_CHUNK_POOLIsValid(&gChunkPool[context]),
