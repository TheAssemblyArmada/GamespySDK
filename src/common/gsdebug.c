...
char* gGSIDebugTypeStrings[GSIDebugType_Count] = {" NET", "FILE", " MEM", "STAT", "MISC"};
char* gGSIDebugLevelStrings[GSIDebugLevel_Count] = {"*ERR", "****", "----", "    ", "    ", "    ", "  ->"};
...
static void
gsiDebugCallback(GSIDebugCategory category, GSIDebugType type, GSIDebugLevel level, const char* format, va_list params)
{
#if defined(_PSP)
    // Output line prefix
...
void gsDebugVaList(GSIDebugCategory theCat,
                   ...
                   va_list theParamList)
{
    ...
    if (aCurLevel & theLevel) // check the flag
    {
        ...
        if (gGSIDebugInstance.mGSIDebugFile) {
            fprintf(gGSIDebugInstance.mGSIDebugFile,
                    "[%s][%s][%s] ",
        ...
        if (gGSIDebugInstance.mDebugCallback != NULL) {
            (*gGSIDebugInstance.mDebugCallback)(theCat, theType, theLevel, theTokenStr, theParamList);
        } else {
            gsiDebugCallback(theCat, theType, theLevel, theTokenStr, theParamList);
        }
    }
...
void gsDebugBinary(GSIDebugCategory theCat,
                   GSIDebugType theType,
                   GSIDebugLevel theLevel,
...
void gsSetDebugFile(FILE* theFile)
{
    if (theFile != gGSIDebugInstance.mGSIDebugFile) {
        ...
        if (gGSIDebugInstance.mGSIDebugFile != NULL) {
            gsDebugFormat(
                GSIDebugCat_Common, GSIDebugType_Misc, GSIDebugLevel_Comment, "Debug disabled in this file\r\n");
        }

        // If the new file is valid, notify it of the opening
        {
            gsDebugFormat(
                GSIDebugCat_Common, GSIDebugType_Misc, GSIDebugLevel_Comment, "Debug enabled in this file\r\n");
        }
