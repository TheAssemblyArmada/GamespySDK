...
public:
    ...
    int issueEncodeTask(int16_t* inBuffer,
                        int inBufferSize,
                        int encodedFrameSize,
                        char* outBuffer,
                        int outBufferSize,
                        SpursSpeexTaskOutput* taskOuput,
                        char* m_userAllocatedSpeexBuffer,
                        int userAllocatedSpeexBufferSize);

    int issueEncodeInitTask(int theQuality,
                            int theGviSpeexSamplesPerSecond,
                            SpursSpeexTaskOutput* taskOutput,
                            char* m_userAllocatedSpeexBuffer,
                            int userAllocatedSpeexBufferSize);

    int issueDecodeAddTask(char* decoderStateBuffer,
    ...
    int issueDecodeSetTask(char* decoderStateBuffer,
                           int decoderStateBufferSize,
                           char* inBuffer,
                           int inBufferSize,
                           int encodedFrameSize,
    ...
    int issueDecodeInitTask(char* decoderStateBuffer,
                            int decoderStateBufferSize,
                            int sampleRate,
                            struct SpursSpeexTaskOutput* taskOutput);
