...
static gsi_bool
gsiLargeIntKMult(const l_word* data1, const l_word* data2, l_word length, l_word* dest, l_word* lenout, l_word maxlen);
static gsi_bool gsiLargeIntMult(const l_word* data1,
                                l_word length1,
                                const l_word* data2,
                                l_word length2,
                                l_word* dest,
                                l_word* lenout,
                                l_word maxlen);
static gsi_bool gsiLargeIntDiv(const l_word* src1,
                               l_word length1,
...
static gsi_bool gsiLargeIntAdd(const l_word* data1,
                               l_word length1,
                               const l_word* data2,
                               l_word length2,
                               l_word* dest,
                               ...
                               l_word maxlen);
static gsi_bool
gsiLargeIntSub(const l_word* amount, l_word length1, const l_word* from, l_word length2, l_word* dest, l_word* lenout);

// Special division, removes divisor directly from src1, leaving remainder
static gsi_bool gsiLargeIntSubDivide(l_word* src1,
                                     l_word length,
                                     const l_word* divisor,
                                     l_word dlen,
                                     gsi_u32 highbit,
                                     l_word* quotient);
...
gsi_bool
gsiLargeIntMultM(gsLargeInt_t* src1, gsLargeInt_t* src2, const gsLargeInt_t* mod, gsi_u32 modPrime, gsLargeInt_t* dest);
...
{
    gsi_bool result = gsiLargeIntAdd(
        src1->mData, src1->mLength, src2->mData, src2->mLength, dest->mData, &dest->mLength, GS_LARGEINT_MAX_DIGITS);
...
{
    gsi_bool result =
        gsiLargeIntSub(src1->mData, src1->mLength, src2->mData, src2->mLength, dest->mData, &dest->mLength);
...
{
    gsi_bool result = gsiLargeIntMult(
        src1->mData, src1->mLength, src2->mData, src2->mLength, dest->mData, &dest->mLength, GS_LARGEINT_MAX_DIGITS);
    if (gsi_is_false(result))
        memset(dest, 0, sizeof(gsLargeInt_t)); // overflow
    return result;
        ...
        if (data2[i] != 0) {
            ...
            for (k = 0; k < length1; k++) {
                ...
                while (carry) {
                    ...
                    digit++;
                    if ((digit > maxlen) || (digit == maxlen && carry > 0)) {
                        GSLINT_EXITTIMER(GSLintTimerMult);
                        return gsi_false; // overflow
                    }
                }
...
gsi_bool
gsLargeIntDiv(const gsLargeInt_t* src1, const gsLargeInt_t* divisor, gsLargeInt_t* dest, gsLargeInt_t* remainder)
{
...
static gsi_bool
gsiLargeIntDiv(const l_word* src, l_word len, const gsLargeInt_t* div, gsLargeInt_t* dest, gsLargeInt_t* remainder)
{
    ...
    do {
        ...
        if (result == -1) {
            ...
            //	_asm {int 3}
            if (gsi_is_false(gsiLargeIntSubDivide(&scopy[readIndex],
                                                  (l_word)readLength,
                                                  divisorData,
                                                  divisorLen,
                                                  (gsi_u32)divisorHighBit,
                                                  &quotientTemp))) {
                // overflow
                GSLINT_EXITTIMER(GSLintTimerDiv);
                return gsi_false;
...
static gsi_bool
gsiLargeIntSubDivide(l_word* src, l_word length, const l_word* divisor, l_word dlen, gsi_u32 highbit, l_word* quotient)
{
    l_dword aboveBits = 0;
    ...
    if (gsi_is_false(gsiLargeIntMult(divisor,
                                     dlen,
                                     quotientCopy.mData,
                                     quotientCopy.mLength,
                                     temp.mData,
                                     &temp.mLength,
...
gsi_bool gsLargeIntKMult(const gsLargeInt_t* src1, const gsLargeInt_t* src2, gsLargeInt_t* dest)
{
    ...
    if (src1->mLength != len || src2->mLength != len) {
        ...
        result =
            gsiLargeIntKMult(src1Copy.mData, src2Copy.mData, len, temp.mData, &temp.mLength, GS_LARGEINT_MAX_DIGITS);
    } else {
...
gsiLargeIntKMult(const l_word* data1, const l_word* data2, l_word length, l_word* dest, l_word* lenout, l_word maxlen)
{
    ...
    if (length <= GS_LARGEINT_KARATSUBA_CUTOFF)
        ...
        if (temp1.mLength > halfLen || temp2.mLength > halfLen)
            gsiLargeIntMult(temp1.mData,
                            temp1.mLength,
                            temp2.mData,
                            temp2.mLength,
                            &temp3.mData[halfLen],
                            &temp3.mLength,
                            (l_word)(GS_LARGEINT_MAX_DIGITS - halfLen));
        else {
        ...
        gsiLargeIntSub(&dest[length],
                       length,
                       &temp3.mData[halfLen],
                       (l_word)(temp3.mLength - halfLen),
                       &temp3.mData[halfLen],
                       &temp3.mLength);
...
gsi_bool gsLargeIntSquareMod(const gsLargeInt_t* lint, const gsLargeInt_t* mod, gsLargeInt_t* dest)
{
    ...
    result =
        gsiLargeIntAdd(otherSums, otherLen, squareSums, squareLen, squareSums, &squareLen, GS_LARGEINT_MAX_DIGITS * 2);
...
gsi_bool gsLargeIntPowerMod(const gsLargeInt_t* b, const gsLargeInt_t* p, const gsLargeInt_t* m, gsLargeInt_t* dest)
{
    ...
    while ((k > 1) && (gsi_u32)((k - 1) * (k << ((k - 1) << 1)) / ((1 << k) - k - 1)) >= expHighBit - 1) {
        --k;
    }
    pow2k = 1 << k;
    kmask = pow2k - 1;
    ...
    for (i = 0; i < kdigits; i++) {
        ...
        // use digitval to determine how many squaring and multiplication operations we need to perform
        {
            ...
            static USHORT oddtab[] = {
                0,  1,   1,   3,   1,  5,   3,   7,   1,  9,   5,   11,  3,  13,  7,   15,  1,  17,  9,   19,
                5,  21,  11,  23,  3,  25,  13,  27,  7,  29,  15,  31,  1,  33,  17,  35,  9,  37,  19,  39,
                5,  41,  21,  43,  11, 45,  23,  47,  3,  49,  25,  51,  13, 53,  27,  55,  7,  57,  29,  59,
                15, 61,  31,  63,  1,  65,  33,  67,  17, 69,  35,  71,  9,  73,  37,  75,  19, 77,  39,  79,
                5,  81,  41,  83,  21, 85,  43,  87,  11, 89,  45,  91,  23, 93,  47,  95,  3,  97,  49,  99,
                25, 101, 51,  103, 13, 105, 53,  107, 27, 109, 55,  111, 7,  113, 57,  115, 29, 117, 59,  119,
                15, 121, 61,  123, 31, 125, 63,  127, 1,  129, 65,  131, 33, 133, 67,  135, 17, 137, 69,  139,
                35, 141, 71,  143, 9,  145, 73,  147, 37, 149, 75,  151, 19, 153, 77,  155, 39, 157, 79,  159,
                5,  161, 81,  163, 41, 165, 83,  167, 21, 169, 85,  171, 43, 173, 87,  175, 11, 177, 89,  179,
                45, 181, 91,  183, 23, 185, 93,  187, 47, 189, 95,  191, 3,  193, 97,  195, 49, 197, 99,  199,
                25, 201, 101, 203, 51, 205, 103, 207, 13, 209, 105, 211, 53, 213, 107, 215, 27, 217, 109, 219,
                55, 221, 111, 223, 7,  225, 113, 227, 57, 229, 115, 231, 29, 233, 117, 235, 59, 237, 119, 239,
...
gsi_bool gsiLargeIntMultM(gsLargeInt_t* x, gsLargeInt_t* y, const gsLargeInt_t* m, gsi_u32 modPrime, gsLargeInt_t* dest)
{
    ...
    if (gsi_is_false(
            gsiLargeIntMult(x->mData, x->mLength, y->mData, y->mLength, temp, &tempLen, GS_LARGEINT_MAX_DIGITS * 2)))
        return gsi_false;
    ...
    for (tptr = &temp[0]; tptr <= lasttnptr; tptr++) {
        ...
        for (nptr = &m->mData[0]; nptr <= lastnptr; nptr++, tiptr++) {
            carry = (l_dword)mi * (l_dword)*nptr + (l_dword)*tiptr
    ...
    if (gsiLargeIntCompare(&temp[logB_r], tempLen - logB_r, m->mData, m->mLength) != -1) {
        if (gsi_is_false(
                gsiLargeIntSub(m->mData, m->mLength, &temp[logB_r], tempLen - logB_r, dest->mData, &dest->mLength))) {
            memset(temp, 0, sizeof(temp));
...
gsi_bool gsiLargeIntMultM(gsLargeInt_t* x, gsLargeInt_t* y, const gsLargeInt_t* m, gsi_u32 modPrime, gsLargeInt_t* dest)
{
    ...
    for (i = 0; (gsi_u32)i < m->mLength; i++) {
        ...
        if (gsi_is_false(gsiLargeIntMult(
                temp.mData, temp.mLength, y->mData, y->mLength, xiy.mData, &xiy.mLength, GS_LARGEINT_MAX_DIGITS))) {
            // overflow
            dest->mLength = 0;
            dest->mData[0] = 0;
            GSLINT_EXITTIMER(GSLintTimerMultM);
        ...
        //    Add both to A
        if (gsi_is_false(gsiLargeIntAdd(
                xiy.mData, xiy.mLength, A.mData, A.mLength, A.mData, &A.mLength, GS_LARGEINT_MAX_DIGITS))) {
            // overflow
            dest->mLength = 0;
            dest->mData[0] = 0;
            GSLINT_EXITTIMER(GSLintTimerMultM);
        ...
        if (gsi_is_false(gsiLargeIntAdd(
                temp.mData, temp.mLength, A.mData, A.mLength, A.mData, &A.mLength, GS_LARGEINT_MAX_DIGITS))) {
            // overflow
            dest->mLength = 0;
            dest->mData[0] = 0;
            GSLINT_EXITTIMER(GSLintTimerMultM);
