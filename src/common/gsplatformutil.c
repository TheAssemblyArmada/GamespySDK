...
////////////////////////////////////////////////////////////////////////////////
#if (defined(_WIN32) || /*defined(_PS2) ||*/ defined(_UNIX) || defined(_REVOLUTION)) && !defined(GSI_NO_THREADS)       \
    && !defined(GSI_NO_ASYNC_DNS)
...
int gsiStartResolvingHostname(const char* hostname, GSIResolveHostnameHandle* handle)
{
    ...
    gsDebugFormat(
        GSIDebugCat_Common, GSIDebugType_State, GSIDebugLevel_Comment, "(Asynchrounous) DNS lookup starting\n");
...
int gsiStartResolvingHostname(const char* hostname, GSIResolveHostnameHandle* handle)
{
    ...
    gsDebugFormat(
        GSIDebugCat_HTTP, GSIDebugType_State, GSIDebugLevel_Comment, "(NON-Asynchrounous) DNS lookup starting\n");
                          ...
                          int sc)
{
    ...
    tmptim = ((((tmptim)*365L + ((long)(yr - 1) >> 2) - (long)_LEAP_YEAR_ADJUST + (long)tmpdays) * 24L + (long)hr) * 60L
              + (long)mn)
                 * 60L
             + (long)sc;
...
gsi_time current_time() //returns current time in milliseconds
{
    ...
    if (!msec) {
        ...
        msec = (unsigned int)(DEC(lasttimecalled.day) * 86400000) + (unsigned int)(DEC(lasttimecalled.hour) * 3600000)
               + (unsigned int)(DEC(lasttimecalled.minute) * 60000) + (unsigned int)(DEC(lasttimecalled.second) * 1000);
    }
...
int B64DecodeLen(const char* input, int encodingType)
{
    ...
    while (*input) {
        if (*input == encoding[2])
            return (int)((input - holdin) / 4 * 3 + (input - holdin - 1) % 4);
        input++;
    }

    return (int)((input - holdin) / 4 * 3);
}
...
char* gsiXxteaAlg(const char* sIn, int nIn, char key[XXTEA_KEY_SIZE], int bEnc, int* nOut)
{
    ...
    if (bEnc == 1) // Encrypt
    {
        ...
        while (i-- != 0) {
            ...
            for (p = -1; ++p < nIn;) {
                y = v[(p < n1) ? p + 1 : 0];
                z = (v[p] += (((z >> 5) ^ (y << 2)) + ((y >> 3) ^ (z << 4))) ^ ((sum ^ y) + (k[(p ^ e) & 3] ^ z)));
            }
        }
    } else if (bEnc == 0) // Decrypt
    {
        ...
        while (sum != 0) {
            ...
            for (p = nIn; p-- != 0;) {
                z = v[(p != 0) ? p - 1 : n1];
                y = (v[p] -= (((z >> 5) ^ (y << 2)) + ((y >> 3) ^ (z << 4))) ^ ((sum ^ y) + (k[(p ^ e) & 3] ^ z)));
            }
            sum -= 0x9E3779B9;
        }
    } else
...
static const char* GetMAC(void)
{
    ...
    int result = sceInetInterfaceControl(
        &gGSIInsockClientData, &gGSIInsockSocketBuffer, 1, sceInetCC_GetHWaddr, MAC, sizeof(MAC));
...
const char* GOAGetUniqueID_Internal(void)
{
    ...
    if (ret != 0 || strlen(keyval) != 19) //need to generate a new key
    {
        ...
        if (docreate) {
            ret = RegCreateKeyExA(
                HKEY_CURRENT_USER, REG_KEY, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &thekey, &disp);
        }
