...
#ifdef SN_SYSTEMS
#undef FD_SET
#define FD_SET(s, p) ((p)->array[((s)-1) >> SN_FD_SHR] |= (unsigned int)(1 << (((s)-1) & SN_FD_BITS)))
...
#define MAX_IPS sceLIBNET_MAX_INTERFACE
    ...
    aNumInterfaces =
        sceInetGetInterfaceList(&gGSIInsockClientData, &gGSIInsockSocketBuffer, aInterfaceIdArray, MAX_IPS);
    if (aNumInterfaces < 1)
        return NULL;
    ...
    for (aInterfaceNum = 0; aInterfaceNum < aNumInterfaces; aInterfaceNum++) {
        ...
        int result = sceInetInterfaceControl(&gGSIInsockClientData,
...
gsi_u32 gsiGetBroadcastIP(void)
{
	...
	*/
    ...
    ret = SOGetInterfaceOpt(NULL, SO_SOL_CONFIG, SO_CONFIG_IP_ADDR_NUMBER, (u8*)&addrnum, &length);
    ...
    ret = SOGetInterfaceOpt(NULL, SO_SOL_CONFIG, SO_CONFIG_IP_ADDR_TABLE, (u8*)addrtbl, &length);
    ...
    ip = (u32)(addrtbl->bcastAddr[3] | (addrtbl->bcastAddr[2] << 8) | (addrtbl->bcastAddr[1] << 16)
