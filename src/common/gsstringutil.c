...
int _UTF8ToUCS2ConversionLengthOnly(const UTF8String theUTF8String)
{
    ...
    while (*theReadPos != '\0') {
        ...
        else if (UTF8_IS_THREE_BYTE(theReadPos[0]) && UTF8_IS_FOLLOW_BYTE(theReadPos[1])
...
int UTF8ToUCS2StringLen(const UTF8String theUTF8String, UCS2String theUCS2String, int theMaxLength)
{
    ...
    while (*anInStream != '\0' && theMaxLength > aTotalBytesRead) {
        ...
        aNumBytesRead =
            _ReadUCS2CharFromUTF8String((UTF8String)anInStream, anOutStream, theMaxLength - aTotalBytesRead);
        if (aNumBytesRead == 0) {
