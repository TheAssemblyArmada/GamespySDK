...
int _NetworkAdHocSocketCreate(gsi_u16 port)
{
    ...
    printf("Create Adhoc Socket: MAC:%x,%x,%x,%x,%x,%x\n",
           (gsi_u32)tmp_local.addr[0],
           (gsi_u32)tmp_local.addr[1],
           (gsi_u32)tmp_local.addr[2],
           (gsi_u32)tmp_local.addr[3],
           (gsi_u32)tmp_local.addr[4],
           (gsi_u32)tmp_local.addr[5]);
#endif
...
int _NetworkAdHocCanReceiveOnSocket(int socket_id)
{
	...
	*/
    ...
    if (ret < 0) {
        ...
        } else if (ret == SCE_ERROR_NET_ADHOC_NOT_INITIALIZED) {
