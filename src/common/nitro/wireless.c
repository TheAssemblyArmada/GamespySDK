...
static MenuScreenConfiguration msInitializingWireless = {"Initializing Wireless",
                                                         {{mscCancel}},
...
static MenuScreenConfiguration msFailedToInitializeWireless = {"Failed to Initialize Wireless",
                                                               {{mscOK}},
...
static MenuScreenConfiguration msRenameConfiguration = {"Rename Configuration",
                                                        {{mscRename}, {mscCancel}},
...
static MenuScreenConfiguration msConfirmDelete =
    {"Confirm Delete", {{mscYes}, {mscNo}}, ConfirmDeleteInit, ConfirmDeleteChose, NULL, SCREEN_OPTION_EXTRAS_CENTERED};

static void ConfigurationDeletedChose(const char* choice);

static MenuScreenConfiguration msConfigurationDeleted = {"Configuration Deleted",
                                                         {{mscOK}},
                                                         NULL,
...
static MenuScreenConfiguration msSearchingForSavedNetwork = {"Searching for Saved Network",
                                                             {{mscCancel}},
...
static MenuScreenConfiguration msSearchFailed =
    {"Search Failed", {{mscOK}}, SearchFailedInit, SearchFailedChose, NULL, SCREEN_OPTION_EXTRAS_CENTERED};

static void NetworkInformationInit(void);
static void NetworkInformationChose(const char* choice);

static MenuScreenConfiguration msNetworkInformation = {"Network Information",
                                                       {{mscBack}},
...
static MenuScreenConfiguration msSelectSecurityType = {"Select Security Type",
                                                       {{mscUseKeyType, CHOICE_OPTION_NEEDS_LIST_SELECTION}, {mscBack}},
...
static MenuScreenConfiguration msEnterSecurityKey = {"Enter Security (WEP) Key",
                                                     {{mscEnter}, {mscBack}},
...
static MenuScreenConfiguration msBadSecurityKeyEntered = {"Bad Security Key Entered",
                                                          {{mscBack}},
...
static MenuScreenConfiguration msConnectingToNetwork = {"Connecting to Network",
                                                        {{mscCancel}},
...
static MenuScreenConfiguration msFailedToConnectToNetwork = {"Failed to Connect to Network",
                                                             {{mscBack}},
...
static MenuScreenConfiguration msConnectingToInternet = {"Connecting to Internet",
                                                         {{mscCancel}},
...
static MenuScreenConfiguration msFailedToConnectToInternet = {"Failed to Connect to Internet",
                                                              {{mscBack}},
...
static MenuScreenConfiguration msConnectedToInternet = {"Connected to Internet",
                                                        {{mscContinue}},
...
static MenuScreenConfiguration msSaveNetworkConfiguration = {"Save Network Configuration",
                                                             {{mscSave}, {mscDontSave}},
...
static MenuScreenConfiguration msConfigurationSaved = {"Configuration Saved",
                                                       {{mscContinue}},
                                                       NULL,
                                                       ConfigurationSavedChose};

static MenuScreenConfiguration msCancelled = {"Wireless Setup Cancelled"};

static MenuScreenConfiguration msFailed = {"Wireless Setup Failed"};
...
static IWConfig WirelessConfig = {3, BSSDescBuffer, IW_BSS_SIZE* BSSDESC_MAX, 0};
...
static void NetworkInformationInit(void)
{
    ...
    // BSSID / MAC
    snprintf(screen->extraText[0],
             MAX_EXTRA_TEXT_STRING_LEN,
    ...
    // channel
    snprintf(screen->extraText[1], MAX_EXTRA_TEXT_STRING_LEN, "  Channel: %d", APList[APIndex].channel);
}
...
static void ConnectedToInternetInit(void)
{
    ...
    IP_GetAddr(NULL, ip);
    snprintf(
        screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  IP:        %d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);

    IP_GetAlias(NULL, ip);
    snprintf(
        screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  Alias:     %d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);

    IP_GetBroadcastAddr(NULL, ip);
    snprintf(
        screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  Broadcast: %d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);

    IP_GetNetmask(NULL, ip);
    snprintf(
        screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  Netmask:   %d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);

    IP_GetGateway(NULL, ip);
    snprintf(
        screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  Gateway:   %d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);

    SOC_GetResolver(&dns1, &dns2);
    snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  DNS1:      %s", inet_ntoa(dns1));
    snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  DNS2:      %s", inet_ntoa(dns2));

    IP_GetMacAddr(NULL, mac);
    snprintf(screen->extraText[line++],
             MAX_EXTRA_TEXT_STRING_LEN,
             "  MAC:       %02X-%02X-%02X-%02X-%02X-%02X",
             mac[0],
             mac[1],
