...
**************/
static void AddRequestCallback(reqtype_t reqtype,
                               int localid,
                               int profileid,
                               persisttype_t pdtype,
                               int pdindex,
...
static void SetPersistDataHelper(int localid,
                                 ...
                                 void* instance,
                                 int kvset);
void GetPersistDataValuesA(int localid,
                           int profileid,
                           persisttype_t type,
                           int index,
                           char* keys,
                           ...
                           void* instance);
void GetPersistDataValuesModifiedA(int localid,
                                   int profileid,
                                   persisttype_t type,
                                   int index,
...
BucketFunc bucketfuncs[NUMOPS] = {BucketSet, BucketAdd, BucketSub, BucketMult, BucketDiv, BucketConcat, BucketAvg};
...
statsgame_t NewGame(int usebuckets)
{
    ...
    if (sock != INVALID_SOCKET) {
        char respformat[] = "\xC\x1C\xA\x1D\x2\x2\x19\x24\x2C\x34\x6\x17\x3E\x1C\x6\xE\x39\x46\x10\x1D\x3\xD\x16\xB\x3B"
                            "\x17\x16\x36\x40\x7";
    ...
    if (sock == INVALID_SOCKET) {
#ifdef ALLOW_DISK
        char respformat[] = "\xC\x1C\xA\x1D\x2\x2\x19\x24\x2C\x34\x16\x1D\x23\x1\x4\xF\x1C\x3F\x51\x25\x2C\xB\xD\x19"
                            "\x3C\x1E\xA\x4\x2\x6\x28\x64\x14";
...
int SendGameSnapShotA(statsgame_t game, const char* snapshot, int final)
{
    ...
    if (sock != INVALID_SOCKET) {
        ...
        char respformat[] =
            "\xC\x7\x1F\xE\x2\x2\x19\x24\x2C\x34\x16\x1D\x23\x1\x4\xF\x1C\x3F\x51\x25\x2C\xB\xA\x16\x3E\x1B\xB\x36\x40"
            "\x7\x28\x25\x1F\x6\x00\x24\x75\x16\x33\xD\x4\xE\x11\x25\x11\x1C\x4\x24\x75\x1";
    ...
    if (sock == INVALID_SOCKET) {
        ...
        char respformat[] =
            "\xC\x7\x1F\xE\x2\x2\x19\x24\x2C\x34\x16\x1D\x23\x1\x4\xF\x1C\x3F\x51\x25\x2C\xB\xA\x16\x3E\x1B\xB\x36\x40"
            "\x7\x28\x25\x1F\x6\x0\x24\x75\x16\x33\xD\x4\xE\x11\x25\x11\x1C\x4\x24\x75\x1\x33\xE\x9\x3F\x45";
...
/****************************************************************************/
void PreAuthenticatePlayerPartner(int localid,
                                  const char* authtoken,
                                  const char* challengeresponse,
                                  PersAuthCallbackFn callback,
                                  void* instance)
{
    char respformat[] = "\xC\x13\x1A\x1E\xD\x13\x28\x1D\x11\x1D\x11\x10\x24\x1D\x04\x0F\x0B\x3F\x51\x32\x2C\x1A\x00\x0B"
                        "\x20\x2E\x4A\x19\x39\x0F\x1D\x25\x2C\x4D\x01";
...
/****************************************************************************/
void PreAuthenticatePlayerPM(int localid,
                             int profileid,
                             const char* challengeresponse,
                             PersAuthCallbackFn callback,
                             void* instance)
{
    char respformat[] =
        "\xC\x13\x1A\x1E\xD\x13\x28\x1D\x0\x1\x1\x24\x75\x16\x33\x18\x0\x10\x4\x1D\x55\x1B\x39\x14\x39\x16\x33\x4F\x1";
...
/****************************************************************************/
void PreAuthenticatePlayerCDA(int localid,
                              const char* nick,
                              const char* keyhash,
                              const char* challengeresponse,
                              PersAuthCallbackFn callback,
                              void* instance)
{
    char respformat[] = "\xC\x13\x1A\x1E\xD\x13\x28\x1D\x1E\x1\x6\x13\xC\x57\x1C\x36\xE\x6\xD\x29\x11\x1B\xD\x24\x75\x1"
                        "\x33\x18\x0\x10\x4\x1D\x55\x1B\x39\x14\x39\x16\x33\x4F\x1";
...
#ifdef GSI_UNICODE
void PreAuthenticatePlayerCDW(int localid,
...
{
    char respformat[] = "\xC\x15\xA\x1E\x15\xA\x10\x1D\x2C\x6\xC\x1B\x3B\x2E\x4A\x19\x39\x8\x11\x38\x18\x9\x16\x10\xC"
                        "\x57\x1C\x36\x9\xA\x10\x1D\x55\xC";
...
#ifdef GSI_UNICODE
void GetProfileIDFromCDW(int localid,
                         const unsigned short* nick,
                         const char* keyhash,
...
/****************************************************************************/
void GetPersistData(int localid,
                    int profileid,
                    persisttype_t type,
                    int index,
                    PersDataCallbackFn callback,
...
void GetPersistDataModified(int localid,
                            int profileid,
                            persisttype_t type,
                            int index,
                            time_t modifiedsince,
...
/****************************************************************************/
void SetPersistData(int localid,
                    int profileid,
                    persisttype_t type,
                    int index,
                    const char* data,
...
void GetPersistDataValuesModifiedA(int localid,
                                   ...
                                   void* instance)
{
    char respformat[] = "\xC\x15\xA\x1E\x15\x7\x28\x1D\x0\x1\x1\x24\x75\x16\x33\x1A\x11\x1A\x4\x24\x2C\x4D\x1\x24\x34"
                        "\x1B\x1\xE\x0\x1B\x28\x64\x14\x34\xE\x1D\x29\x1\x33\x4F\x16\x3F\x18\x28\x14\x34\x40\x1C";
...
#ifdef GSI_UNICODE
void GetPersistDataValuesModifiedW(int localid,
                                   int profileid,
                                   persisttype_t type,
                                   int index,
                                   time_t modifiedsince,
...
#ifdef GSI_UNICODE
void GetPersistDataValuesW(int localid,
                           int profileid,
                           persisttype_t type,
                           int index,
                           unsigned short* keys,
...
/****************************************************************************/
void SetPersistDataValuesA(int localid,
                           int profileid,
                           persisttype_t type,
                           int index,
...
#ifdef GSI_UNICODE
void SetPersistDataValuesW(int localid,
                           int profileid,
                           persisttype_t type,
                           int index,
                           const unsigned short* keyvalues,
                           PersDataSaveCallbackFn callback,
                           void* instance)
{
...
static int SendChallengeResponse(const char* indata, int gameport)
{
    ...
    char respformat[] = "\xC\x13\x1A\x1E\xD\x3F\x28\x26\x11\x5\x0\x16\x31\x1F\xA\x36\x40\x10\x28\x33\x15\x1B\x15\x17"
                        "\x3E\x1\xA\x36\x40\x10\x28\x31\x1F\x1A\x11\x24\x75\x16\x33\x3\x1\x3F\x45";
...
static void CheckDiskFile()
{
    ...
    while (!feof(f) && !ferror(f)) {
        ...
        if (fread(&check, sizeof(check), 1, f) == 0 || fread(&len, sizeof(len), 1, f) == 0)
            break;
        len ^= DISKLENXOR;
...
static void SetPersistDataHelper(int localid,
                                 ...
                                 int kvset)
{
    char respformat[] = "\xC\x1\xA\x1E\x15\x7\x28\x1D\x0\x1\x1\x24\x75\x16\x33\x1A\x11\x1A\x4\x24\x2C\x4D\x1\x24\x34"
                        "\x1B\x1\xE\x0\x1B\x28\x64\x14\x34\xE\xE\xC\x57\xB\x36\x9\xA\x10\x1D\x55\xC\x39\x14\x35\x1C\x8"
                        "\x1E\xD\x3F\x51\x25\x2C\xC\x4\xC\x31\x2E";
    ...
    if (type == pd_private_ro || type == pd_public_ro) { //can't set read-only types, check that client side
        if (callback)
...
static char* FindFinal(char* buff, int len)
{
    ...
    while (pos - buff < len - 6) {
        if (pos[0] == '\\' && pos[1] == 'f' && pos[2] == 'i' && pos[3] == 'n' && pos[4] == 'a' && pos[5] == 'l'
            && pos[6] == '\\') {
            return pos;
        } else
            pos++;
...
static int ProcessInBuffer(char* buff, int len)
{
    ...
    while ((len > 0) && (pos != NULL)) {
        DOXCODE(buff, (int)(pos - buff), enc1);
...
static void CallReqCallback(int reqindex, int success, time_t modified, char* data, int length)
{
    ...
    if (req->callback)
...
#else
        {
            ...
            break;
        case rt_datacb:
            ((PersDataCallbackFn)req->callback)(req->localid,
                                                req->profileid,
                                                req->pdtype,
                                                req->pdindex,
                                                success,
            ...
            break;
        case rt_savecb:
            ((PersDataSaveCallbackFn)req->callback)(
                req->localid, req->profileid, req->pdtype, req->pdindex, success, modified, req->instance);
            break;
        case rt_profilecb:
