...
static void PersDataCallback(int localid,
                             int profileid,
                             persisttype_t type,
                             int index,
                             int success,
...
static void PlayerDataCallback(int localid,
                               int profileid,
                               persisttype_t type,
                               int index,
                               int success,
                               ...
                               void* instance)
{
    ...
    if (success)
        printf("playerdata get callback: localid: %d profileid: %d success: %d modified: %d len: %d "
               "data:\n\tmodel:%d\n\thitpoints:%d\n\ttotaltime:%d\n\tskin:%s\n",
               localid,
               profileid,
               success,
...
static void PersDataSaveCallback(int localid,
                                 int profileid,
                                 persisttype_t type,
                                 int index,
                                 int success,
...
int test_main(int argc, char** argv)
{
	...
	************/
    ...
    SetPersistData(
        1, player1pid, pd_public_rw, 0, (char*)&playerdata, sizeof(playerdata_t), PersDataSaveCallback, &callbackct);
    SetPersistDataValues(2,
                         PROFILEID,
                         pd_public_rw,
                         0,
                         _T("\\key1\\value1\\key2\\value2\\key3\\value3"),
                         PersDataSaveCallback,
                         &callbackct);
    while (callbackct < 2 && IsStatsConnected()) {
        PersistThink();
        msleep(10);
	...
	************/
    ...
    SetPersistDataValues(2,
                         ...
                         pd_public_rw,
                         0,
                         _T("\\key1\\valueA\\key3\\valueB\\key4\\value400\\key5\\value500"),
                         PersDataSaveCallback,
                         &callbackct);
    GetPersistData(2, PROFILEID, pd_public_rw, 0, PersDataCallback, &callbackct);
	...
	************/
    SetPersistDataValues(2,
                         ...
                         pd_public_rw,
                         0,
                         _T("\\key2\\\\key3\\valueC\\key4\\value401\\mykeyname\\mykeyvalue\\otherkey\\othervalue"),
                         PersDataSaveCallback,
                         &callbackct);
    GetPersistData(2, PROFILEID, pd_public_rw, 0, PersDataCallback, &callbackct);
