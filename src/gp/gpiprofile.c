...
GPIBool gpiInitProfiles(GPConnection* connection)
{
    ...
    iconnection->profileList.profileTable =
        TableNew(sizeof(GPIProfile), 32, gpiProfilesTableHash, gpiProfilesTableCompare, gpiProfilesTableFree);
...
static GPResult gpiOpenDiskProfiles(GPConnection* connection, GPIBool write, GPIBool* failed)
{
    FILE* fp = NULL;
    GPIConnection* iconnection = (GPIConnection*)*connection;
...
static void gpiCloseDiskProfiles(GPConnection* connection)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
...
static GPResult gpiReadDiskProfile(GPConnection* connection, GPIBool* failedOut)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    FILE* fp;
    GPIProfile profile;
    ...
    do {
        ...
        if (!failed) {
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_Comment, "%d: %s=%s\n", profile.profileId, key, value);
...
static GPResult gpiReadVersion(const GPConnection* connection, int* version)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    FILE* fp;
...
static void gpiWriteVersion(GPConnection* connection, int version)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    FILE* fp;
...
GPResult gpiLoadDiskProfiles(GPConnection* connection)
{
    ...
    if (failed) {
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_HotError, "Failed to open the disk cache file.\n");
        return GP_NO_ERROR;
    }
    ...
    if (version == GPI_PROFILE_CACHE_VERSION) {
        ...
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_Comment, "Loaded %d profiles from disk cache.\n", count);
    }
...
static GPIBool gpiSaveDiskProfile(GPConnection* connection, GPIProfile* profile, void* data)
{
    FILE* fp;
    GPIConnection* iconnection = (GPIConnection*)*connection;
...
static GPIBool gpiCheckProfileForUser(GPConnection* connection, GPIProfile* profile, void* udata)
{
...
GPResult
gpiFindProfileByUser(GPConnection* connection, char nick[GP_NICK_LEN], char email[GP_EMAIL_LEN], GPIProfile** profile)
{
...
GPIBool gpiProfileMap(GPConnection* connection, gpiProfileMapFunc func, void* data)
{
    ...
    return (!TableMapSafe2(iconnection->profileList.profileTable, gpiProfileMapCallback, &mapData)) ? GPITrue
                                                                                                    : GPIFalse;
}
...
static GPIBool gpiCheckForBuddy(GPConnection* connection, GPIProfile* profile, void* udata)
{
...
{
    return ((profile && !profile->cache && !profile->buddyStatus && !profile->buddyStatusInfo && !profile->peerSig
             && !profile->authSig && !profile->blocked))
               ? GPITrue
               : GPIFalse;
}
...
#ifndef _PS2
            gpiProfileMap(connection, gpiFixBuddyIndices, (void*)(uintptr_t)index);
#else
...
static GPIBool gpiFixBlockIndices(GPConnection* connection, GPIProfile* profile, void* data)
{
#ifndef _PS2
    int baseIndex = (int)(uintptr_t)data;
#else
...
#ifndef _PS2
        gpiProfileMap(connection, gpiFixBlockIndices, (void*)(uintptr_t)index);
#else
...
static GPIBool gpiCheckForBlock(GPConnection* connection, GPIProfile* profile, void* udata)
{
    GPIFindProfileData* data = (GPIFindProfileData*)udata;
    ...
    for (i = 0; i < num; i++) {
        if (i == 0) {
            ...
            if (!gpiValueForKeyWithIndex(input, ",", &index, buffer, sizeof(buffer)))
                CallbackFatalError(
                    connection, GP_NETWORK_ERROR, GP_PARSE, "Unexpected data was received from the server.");
        }
