...
void gpiDestroy(GPConnection* connection)
{
    ...
    if (iconnection->infoCaching && iconnection->connectState != GPI_NOT_CONNECTED) {
        if (gpiSaveDiskProfiles(connection) != GP_NO_ERROR) {
            gsDebugFormat(GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_HotError, "Error saving profiles to disk.");
        }
    }
#endif
...
static GPIBool gpiResetProfile(GPConnection* connection, GPIProfile* profile, void* data)
{
...
GPResult gpiProcessConnectionManager(GPConnection* connection)
{
    ...
    do {
        ...
        ///////////////////////////////////////////////
        result =
            gpiRecvToBuffer(connection, iconnection->cmSocket, &iconnection->socketBuffer, &len, &connClosed, "CM");
        if (result != GP_NO_ERROR) {
            if (result == GP_NETWORK_ERROR)
                CallbackFatalError(
                    connection, GP_NETWORK_ERROR, GP_NETWORK, "There was an error reading from the server.");
        ...
        while ((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL) {
            ...
            //////////////////////////////////
                ...
                if (gpiCheckForError(connection, iconnection->inputBuffer, GPITrue)) {
                    ...
                    /////////////////////////
                    gpiValueForKey(
                        iconnection->inputBuffer, "\\lt\\", iconnection->loginTicket, sizeof(iconnection->loginTicket));
...
GPResult gpiProcess(GPConnection* connection, int blockingOperationID)
{
    ...
    assert((iconnection->connectState == GPI_NOT_CONNECTED) || (iconnection->connectState == GPI_CONNECTING)
           || (iconnection->connectState == GPI_NEGOTIATING) || (iconnection->connectState == GPI_CONNECTED)
           || (iconnection->connectState == GPI_DISCONNECTED) || (iconnection->connectState == GPI_PROFILE_DELETING));
...
*/
    ...
    if (iconnection->connectState == GPI_CONNECTING) {
        do {
            ...
            //PANTS|07.10.00 - only sleep if looping
            loop = (((result == GP_NO_ERROR) && (blockingOperationID != 0)
...
static GPIBool gpiFreeProfileInfo(GPConnection* connection, GPIProfile* profile, void* data)
{
    GSI_UNUSED(data);
...
static GPIBool gpiReportProfile(GPConnection* connection, GPIProfile* profile, void* data)
{
    int temp;
