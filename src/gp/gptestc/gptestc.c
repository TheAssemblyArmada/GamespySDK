    ...
    if (arg->result == GP_NO_ERROR) {
        if (arg->numMatches > 0) {
            for (i = 0; i < arg->numMatches; i++) {
                result = gpGetInfo(
                    pconn, arg->matches[i].profile, GP_DONT_CHECK_CACHE, GP_BLOCKING, (GPCallback)Whois, NULL);
...
void RecvBuddyStatus(GPConnection* connection, void* arg_, void* param)
{
    ...
    static char* statusToString[6] = {"GP_OFFLINE", "GP_ONLINE", "GP_PLAYING", "GP_STAGING", "GP_CHATTING", "GP_AWAY"};
    ...
    if (arg->profile == other) {
        ...
        if (status.status == 1 && _tcscmp(status.statusString, "Ready") && _tcscmp(status.statusString, "BlockTime")) {
            if (appState < 1) {
                ...
                CHECK_GP_RESULT(gpSetStatus(pconn, (GPEnum)GP_ONLINE, _T("Ready"), _T("gptestc")),
                                "gpSetStatus failed");
...
int test_main(int argc, char** argv)
{
    ...
    printf("\nConnecting...\n");
    CHECK_GP_RESULT(
        gpConnect(pconn, nick, email, password, GPTC_FIREWALL_OPTION, GP_BLOCKING, (GPCallback)ConnectResponse, NULL),
        "gpConnect failed");
    ...
    CHECK_GP_RESULT(gpProfileSearch(pconn,
                                    _T(""),
                                    _T(""),
                                    _T("dan@gamespy.com"),
                                    _T(""),
                                    ...
                                    0,
                                    GP_BLOCKING,
                                    (GPCallback)ProfileSearchResponse,
                                    NULL),
                    "gpProfileSearch failed");
    printf(" Search on nick ('crt')\n");
    CHECK_GP_RESULT(
        gpProfileSearch(
            pconn, _T("crt"), _T(""), _T(""), _T(""), _T(""), 0, GP_BLOCKING, (GPCallback)ProfileSearchResponse, NULL),
        "gpProfileSearch failed");
    ...
    CHECK_GP_RESULT(
        gpProfileSearchUniquenick(
            pconn, GPTC_NICK1, namespaceIds, GP_MAX_NAMESPACEIDS, GP_BLOCKING, (GPCallback)ProfileSearchResponse, NULL),
    ...
    //////////
    printf("\nGetting Info\n");
    CHECK_GP_RESULT(gpGetInfo(pconn, GPTC_PID1, GP_DONT_CHECK_CACHE, GP_BLOCKING, (GPCallback)GetInfoResponse, NULL),
                    "gpGetInfo failed");
    ...
    if (noComLineArgs) {
        ...
        CHECK_GP_RESULT(gpSetStatus(pconn, (GPEnum)GP_ONLINE, _T("DoneBlockTesting"), _T("gptestc")),
                        "gpSetStatus failed");
