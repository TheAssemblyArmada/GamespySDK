...
//FUNCTIONS
///////////
static GPResult gpiSendAuthBuddyRequest(GPConnection* connection, GPIProfile* profile)
{
...
GPResult gpiProcessRecvBuddyMessage(GPConnection* connection, const char* input)
{
    ...
    int productID;
    GPIConnection* iconnection = (GPIConnection*)*connection;
    char strTemp[GSI_MAX(GP_STATUS_STRING_LEN, GP_LOCATION_STRING_LEN)];
...
GPResult gpiBuddyHandleKeyReply(GPConnection* connection, GPIPeer* peer, char* buffer)
{
    ...
    if (strcmp(buffer, "") == 0) {
        ...
        int decodedLen = 0, index = 0, numKeys, i;
        char keyName[512];
        char keyVal[512];
        char decodeKey[512];
        char decodeVal[512];
        ...
        if (numKeys == 0) {
            ...
            for (i = 0; i < numKeys; i++) {
                ...
                if (gpiStatusInfoCheckKey(connection, pProfile->buddyStatusInfo->extendedInfoKeys, decodeKey, &checkKey)
                        == GP_NO_ERROR
                    && checkKey == NULL) {
...
GPIBool gpiFixBuddyIndices(GPConnection* connection, GPIProfile* profile, void* data)
{
#ifndef _PS2
...
GPResult gpiDeleteBuddy(GPConnection* connection, GPProfile profile, GPIBool sendServerRequest)
{
    GPIProfile* pProfile;
    GPIConnection* iconnection = (GPIConnection*)*connection;
    int index;
