...
const int RemoteAuthProfiles[] =
    {58214083, 58214074, 58214075, 58214076, 58214077, 58214078, 58214079, 58214080, 58214081, 58214082};
...
void ConnectCallback(GPConnection* connection, GPConnectResponseArg* arg, Profile* profile)
{
    if (arg->result == GP_NO_ERROR) {
        ...
        if (arg->profile != RemoteAuthProfiles[profile->index % 10]) {
            ...
            sprintf(buffer,
                    "%d: XXX Connection ProfileID Error (%d != %d) [%d]: %d.%03ds\n",
                    totalConnected,
                    arg->profile,
                    RemoteAuthProfiles[profile->index % 10],
                    (profile->index % 10),
                    ms / 1000,
                    ms % 1000);
            Log(buffer);
...
void FindPlayersCallback(GPConnection* connection, GPFindPlayersResponseArg* arg, Profile* profile)
{
    ...
    for (i = 0; i < arg->numMatches; i++)
        gpGetInfo(
            &profile->gp, arg->matches[i].profile, GP_DONT_CHECK_CACHE, GP_NON_BLOCKING, GetInfoCallback, profile);
...
void StartOp(Profile* profile)
{
...
#else
        {
            const char* authtokens[] = {"GMT/61LHe4Yu+pHI0eDXu8GMdc51iXjsNvcz1GGoWJ9orpnbZIbp3CVBQddeSZOSq58",
            ...
            const char* partnerchallenges[] = {"WI[A)IuA",
                                               "RNd$)q)0",
                                               "LQTi;4#X",
                                               "AOA)(57=",
                                               "ob=0XiWJ",
            ...
            gpConnectPreAuthenticated(&profile->gp,
                                      authtokens[tokenIndex],
                                      partnerchallenges[tokenIndex],
                                      GP_FIREWALL,
                                      GP_NON_BLOCKING,
                                      ConnectCallback,
                                      profile);
    ...
    case OpGetInfo:
        ...
        if (profile->numOthers > 0)
            gpGetInfo(
                &profile->gp, RandomOther(profile), GP_DONT_CHECK_CACHE, GP_NON_BLOCKING, GetInfoCallback, profile);
        else
            profile->activeOp = OpNull;
        break;
    ...
    case OpProfileSearch:
        ...
        if (num == 0)
            gpProfileSearch(&profile->gp,
                            ...
                            0,
                            GP_NON_BLOCKING,
                            ProfileSearchCallback,
                            profile);
        else if (num == 1)
            gpProfileSearch(&profile->gp, "crt", NULL, NULL, NULL, 0, GP_NON_BLOCKING, ProfileSearchCallback, profile);
        else
            gpProfileSearch(
                &profile->gp, "STRESS-TEST", NULL, NULL, NULL, 0, GP_NON_BLOCKING, ProfileSearchCallback, profile);
        break;
...
void Frame(unsigned long now)
{
    ...
    ///////////////////////////////////////
    if (!shuttingDown && (numConnections < maxConnections)
        && ((numConnections - numConnected) < MAX_OUTSTANDING_CONNECTIONS)) {
        Profile* profile;
