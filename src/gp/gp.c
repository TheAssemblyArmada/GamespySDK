                    ...
                    void* param)
{
    ...
    /////////
    return gpiConnect(
        connection, nick, "", email, password, "", "", NULL, firewall, GPIFalse, blocking, callback, param);
}
#ifdef GSI_UNICODE
GPResult gpConnectW(GPConnection* connection,
                           ...
                           void* param)
{
    ...
    /////////
    return gpiConnect(
        connection, nick, uniquenick, email, password, "", "", cdkey, firewall, GPITrue, blocking, callback, param);
}
#ifdef GSI_UNICODE
GPResult gpConnectNewUserW(GPConnection* connection,
                           const unsigned short nick[GP_NICK_LEN],
                           ...
                           void* param)
{
    ...
    return gpConnectNewUserA(
        connection, nick_A, uniquenick_A, email_A, password_A, cdkey_A, firewall, blocking, callback, param);
}
#endif
                              ...
                              void* param)
{
    ...
    /////////
    return gpiConnect(
        connection, "", uniquenick, "", password, "", "", NULL, firewall, GPIFalse, blocking, callback, param);
}
#ifdef GSI_UNICODE
GPResult gpConnectUniqueNickW(GPConnection* connection,
                              const unsigned short uniquenick[GP_UNIQUENICK_LEN],
                              const unsigned short password[GP_PASSWORD_LEN],
                              GPEnum firewall,
                              GPEnum blocking,
...
GPResult gpConnectPreAuthenticatedA(GPConnection* connection,
                                    ...
                                    void* param)
{
    ...
    /////////
    return gpiConnect(
        connection, "", "", "", "", authtoken, partnerchallenge, NULL, firewall, GPIFalse, blocking, callback, param);
}
#ifdef GSI_UNICODE
GPResult gpConnectPreAuthenticatedW(GPConnection* connection,
...
GPResult gpCheckUserA(GPConnection* connection,
                      const char nick[GP_NICK_LEN],
                      const char email[GP_EMAIL_LEN],
                      const char password[GP_PASSWORD_LEN],
                      GPEnum blocking,
...
#ifdef GSI_UNICODE
GPResult gpCheckUserW(GPConnection* connection,
                      const unsigned short nick[GP_NICK_LEN],
                      const unsigned short email[GP_EMAIL_LEN],
                      const unsigned short password[GP_PASSWORD_LEN],
                      GPEnum blocking,
...
GPResult gpNewUserA(GPConnection* connection,
                    const char nick[GP_NICK_LEN],
                    const char uniquenick[GP_UNIQUENICK_LEN],
                    const char email[GP_EMAIL_LEN],
                    const char password[GP_PASSWORD_LEN],
...
#ifdef GSI_UNICODE
GPResult gpNewUserW(GPConnection* connection,
                    const unsigned short nick[GP_NICK_LEN],
                    const unsigned short uniquenick[GP_UNIQUENICK_LEN],
                    const unsigned short email[GP_EMAIL_LEN],
                    const unsigned short password[GP_PASSWORD_LEN],
...
#ifdef GSI_UNICODE
GPResult gpSuggestUniqueNickW(GPConnection* connection,
...
#ifdef GSI_UNICODE
GPResult gpRegisterUniqueNickW(GPConnection* connection,
                               const unsigned short uniquenick[GP_UNIQUENICK_LEN],
                               const unsigned short cdkey[GP_CDKEY_LEN],
                               GPEnum blocking,
                               GPCallback callback,
...
#ifdef GSI_UNICODE
GPResult gpRegisterCdKeyW(GPConnection* connection,
...
GPResult gpGetErrorStringA(GPConnection* connection, char errorString[GP_ERROR_STRING_LEN])
{
    GPIConnection* iconnection;
...
#ifdef GSI_UNICODE
GPResult gpGetErrorStringW(GPConnection* connection, unsigned short errorString[GP_ERROR_STRING_LEN])
{
...
GPResult gpNewProfileA(GPConnection* connection,
                       const char nick[GP_NICK_LEN],
                       GPEnum replace,
                       GPEnum blocking,
                       GPCallback callback,
...
#ifdef GSI_UNICODE
GPResult gpNewProfileW(GPConnection* connection,
                       const unsigned short nick[GP_NICK_LEN],
                       GPEnum replace,
                       GPEnum blocking,
                       GPCallback callback,
                          ...
                          void* param)
{
    ...
    // Do the search.
    /////////////////
    return gpiProfileSearch(
        connection, nick, uniquenick, email, firstname, lastname, icquin, 0, blocking, callback, param);
}
#ifdef GSI_UNICODE
GPResult gpProfileSearchW(GPConnection* connection,
                          const unsigned short nick[GP_NICK_LEN],
                          const unsigned short uniquenick[GP_UNIQUENICK_LEN],
                          const unsigned short email[GP_EMAIL_LEN],
                          ...
                          void* param)
{
    ...
    return gpProfileSearchA(
        connection, nick_A, uniquenick_A, email_A, firstname_A, lastname_A, icquin, blocking, callback, param);
}
#endif
...
#ifdef GSI_UNICODE
GPResult gpProfileSearchUniquenickW(GPConnection* connection,
                                    const unsigned short uniquenick[GP_UNIQUENICK_LEN],
                                    const int namespaceIDs[GP_MAX_NAMESPACEIDS],
                                    int numNamespaces,
                                    GPEnum blocking,
...
GPResult gpGetInfoNoWait(GPConnection* connection, GPProfile profile, GPGetInfoResponseArg* arg)
{
    GPIConnection* iconnection;
...
#ifdef GSI_UNICODE
GPResult gpSetInfosW(GPConnection* connection, GPEnum info, const unsigned short* value)
{
...
GPResult gpSendBuddyRequestA(GPConnection* connection, GPProfile profile, const char reason[GP_REASON_LEN])
{
    GPIConnection* iconnection;
...
#ifdef GSI_UNICODE
GPResult gpSendBuddyRequestW(GPConnection* connection, GPProfile profile, const unsigned short reason[GP_REASON_LEN])
{
...
GPResult gpGetBuddyStatusInfo(GPConnection* connection, int index, GPBuddyStatusInfo* statusInfo)
{
    GPIConnection* iconnection;
    int num;
    GPIProfile* profile;
...
GPResult gpSetBuddyAddr(GPConnection* connection, int index, unsigned int buddyIp, unsigned short buddyPort)
{
    GPIConnection* iconnection;
    int num;
    GPIProfile* profile;
...
GPResult gpGetNumBlocked(GPConnection* connection, int* numBlocked)
{
    GPIConnection* iconnection;
...
GPResult gpGetBlockedProfile(GPConnection* connection, int index, GPProfile* profile)
{
    GPIConnection* iconnection;
    int num;
    GPIProfile* pProfile;
...
gsi_bool gpIsBlocked(GPConnection* connection, GPProfile profile)
{
    GPIProfile* pProfile;
    GPIConnection* iconnection;
...
#ifndef GP_NEW_STATUS_INFO
GPResult gpSetStatusA(GPConnection* connection,
                      GPEnum status,
                      ...
                      const char locationString[GP_LOCATION_STRING_LEN])
{
    ...
    /////////////////////////////////////////////////
    if ((status == iconnection->lastStatusState) && (strcmp(statusStringFixed, iconnection->lastStatusString) == 0)
...
#ifdef GSI_UNICODE
GPResult gpSetStatusW(GPConnection* connection,
                      GPEnum status,
...
#ifdef GP_NEW_STATUS_INFO
GPResult gpSetStatusInfoA(GPConnection* connection,
                          GPEnum statusState,
                          unsigned int hostIp,
                          ...
                          int gameMapNameLen)
{
    ...
    // Don't send it if its the same as the previous.
    /////////////////////////////////////////////////
    if ((statusState == iconnection->lastStatusState) && (strcmp(richStatus, iconnection->richStatus) == 0)
        && (strcmp(gameTypeFixed, iconnection->gameType) == 0)
        && (strcmp(gameVariantFixed, iconnection->gameVariant) == 0)
        && (strcmp(gameMapNameFixed, iconnection->gameMapName) == 0) && (sessionFlags == iconnection->sessionFlags)
        && (hostIp == iconnection->hostIp) && (hostPrivateIp == iconnection->hostPrivateIp)
        && (queryPort == iconnection->queryPort) && (hostPort == iconnection->hostPort)) {
        return GP_NO_ERROR;
    }
#else
...
#ifdef GSI_UNICODE
GPResult gpSetStatusInfoW(GPConnection* connection,
                          GPEnum statusState,
                          unsigned int hostIp,
                          unsigned int hostPrivateIp,
                          unsigned short queryPort,
                          ...
                          int gameMapNameLen)
{
    ...
    if ((statusState == iconnection->lastStatusState) && (sessionFlags == iconnection->sessionFlags)
        && (hostIp == iconnection->hostIp) && (hostPrivateIp == iconnection->hostPrivateIp)
        && (queryPort == iconnection->queryPort) && (hostPort == iconnection->hostPort)
        && (_tcscmp(richStatus, iconnection->richStatus_W) == 0) && (_tcscmp(gameType, iconnection->gameType_W) == 0)
    ...
    aResult = gpSetStatusInfoA(connection,
                               statusState,
                               hostIp,
                               hostPrivateIp,
                               queryPort,
                               ...
                               sessionFlags,
                               richStatus_A,
                               (int)strlen(richStatus_A),
                               gameType_A,
                               (int)strlen(gameType_A),
                               gameVariant_A,
                               (int)strlen(gameVariant_A),
                               gameMapName_A,
                               (int)strlen(gameMapName_A));
...
GPResult gpGetBuddyStatusInfoKeys(GPConnection* connection, int index, GPCallback callback, void* userData)
{
    ...
    aResult =
        gpiSendBuddyMessage(connection, pProfile->profileId, GPI_BM_KEYS_REQUEST, "Keys?", GP_DONT_ROUTE, aPeerOp);
    return aResult;
}
#endif

GPResult gpSendBuddyMessageA(GPConnection* connection, GPProfile profile, const char* message)
{
    GPIConnection* iconnection;
...
#ifdef GSI_UNICODE
GPResult gpSendBuddyMessageW(GPConnection* connection, GPProfile profile, const unsigned short* message)
{
    char* message_A;
    GPResult result;
...
GPResult gpSendBuddyUTMA(GPConnection* connection, GPProfile profile, const char* message, int sendOption)
{
    GPIConnection* iconnection;
...
#ifdef GSI_UNICODE
GPResult gpSendBuddyUTMW(GPConnection* connection, GPProfile profile, const unsigned short* message, int sendOption)
{
    char* message_A;
    GPResult result;
...
#ifdef GSI_UNICODE
GPResult gpIsValidEmailW(GPConnection* connection,
                         const unsigned short email[GP_EMAIL_LEN],
                         GPEnum blocking,
                         GPCallback callback,
                         void* param)
...
#ifdef GSI_UNICODE
GPResult gpGetUserNicksW(GPConnection* connection,
                         const unsigned short email[GP_EMAIL_LEN],
                         const unsigned short password[GP_PASSWORD_LEN],
                         GPEnum blocking,
                         GPCallback callback,
...
GPResult
gpInvitePlayerA(GPConnection* connection, GPProfile profile, int productID, const char location[GP_LOCATION_STRING_LEN])
{
    GPIConnection* iconnection;
...
#ifdef GSI_UNICODE
GPResult gpInvitePlayerW(GPConnection* connection,
                         GPProfile profile,
                         int productID,
...
static GPResult gpiAddSendingFileA(GPConnection* connection, GPITransfer* transfer, const char* path, const char* name)
{
    GPIFile* file = NULL;
    int size = 0;
    gsi_time modTime = 0;
...
#ifdef GSI_UNICODE
static GPResult gpiAddSendingFileW(GPConnection* connection,
...
GPResult gpSendFilesA(GPConnection* connection,
                      GPTransfer* transfer,
                      GPProfile profile,
                      const char* message,
...
GPResult gpSendFilesW(GPConnection* connection,
                      GPTransfer* transfer,
                      GPProfile profile,
...
GPResult gpAcceptTransferA(GPConnection* connection, GPTransfer transfer, const char* message)
{
    GPITransfer* pTransfer;
    GPIConnection* iconnection;
...
GPResult gpAcceptTransferW(GPConnection* connection, GPTransfer transfer, const unsigned short* message)
{
    char* message_A = NULL;
    GPResult result;
...
GPResult gpRejectTransferA(GPConnection* connection, GPTransfer transfer, const char* message)
{
    GPITransfer* pTransfer;
...
GPResult gpRejectTransferW(GPConnection* connection, GPTransfer transfer, const unsigned short* message)
{
    char* message_A = NULL;
    GPResult result;
...
GPResult gpSetTransferDirectoryA(GPConnection* connection, GPTransfer transfer, const char* directory)
{
    GPITransfer* pTransfer;
    char lastChar;
...
GPResult gpSetTransferDirectoryW(GPConnection* connection, GPTransfer transfer, const unsigned short* directory)
{
...
GPResult gpSetTransferThrottle(GPConnection* connection, GPTransfer transfer, int throttle)
{
    ...
    CHECK_RESULT(gpiPeerStartTransferMessage(
        connection, pTransfer->peer, GPI_BM_FILE_TRANSFER_THROTTLE, (GPITransferID_st)&pTransfer->transferID));
    ...
    if (pTransfer->sender) {
        ...
        if (arg) {
            ...
            arg->num = throttle;
            gpiAddCallback(
                connection, iconnection->callbacks[GPI_TRANSFER_CALLBACK], arg, NULL, GPI_ADD_TRANSFER_CALLBACK);
        }
    }
