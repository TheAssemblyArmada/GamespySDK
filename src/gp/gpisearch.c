...
//FUNCTIONS
///////////
static GPResult gpiStartProfileSearch(GPConnection* connection, GPIOperation* operation)
{
    ...
    if (rcode == 0)
        CallbackFatalError(
            connection, GP_NETWORK_ERROR, GP_NETWORK, "There was an error making a socket non-blocking.");
...
static GPResult gpiInitSearchData(GPConnection* connection, GPISearchData** searchData, int type)
{
    GPISearchData* data;
...
static GPResult
gpiStartSearch(GPConnection* connection, GPISearchData* data, GPEnum blocking, GPCallback callback, void* param)
{
...
GPResult gpiProfileSearch(GPConnection* connection,
...
GPResult gpiProfileSearchUniquenick(GPConnection* connection,
                                    const char uniquenick[GP_UNIQUENICK_LEN],
...
GPResult gpiIsValidEmail(GPConnection* connection,
                         const char email[GP_EMAIL_LEN],
                         GPEnum blocking,
                         GPCallback callback,
                         void* param)
...
GPResult gpiGetUserNicks(GPConnection* connection,
                         const char email[GP_EMAIL_LEN],
...
GPResult gpiFindPlayers(GPConnection* connection, int productID, GPEnum blocking, GPCallback callback, void* param)
{
    GPISearchData* data;
...
GPResult gpiCheckUser(GPConnection* connection,
                      const char nick[GP_NICK_LEN],
                      const char email[GP_EMAIL_LEN],
                      const char password[GP_PASSWORD_LEN],
                      GPEnum blocking,
...
GPResult gpiNewUser(GPConnection* connection,
                    const char nick[GP_NICK_LEN],
                    const char uniquenick[GP_UNIQUENICK_LEN],
                    const char email[GP_EMAIL_LEN],
                    const char password[GP_PASSWORD_LEN],
...
GPResult gpiOthersBuddy(GPConnection* connection, GPEnum blocking, GPCallback callback, void* param)
{
    GPISearchData* data;
...
GPResult gpiOthersBuddyList(GPConnection* connection,
                            int* profiles,
                            int numOfProfiles,
                            GPEnum blocking,
                            GPCallback callback,
...
static GPResult gpiProcessSearch(GPConnection* connection, GPIOperation* operation)
{
    int state;
    GPISearchData* data;
    char key[512];
                ...
                if ((data->type == GPI_SEARCH_PROFILE) || (data->type == GPI_SEARCH_PROFILE_UNIQUENICK)) {
                    ...
                    do {
                        ...
                        if (strcmp(key, "bsrdone") == 0) {
                            ...
                            //////////////////////
                            arg.numMatches++;
                            arg.matches = (GPProfileSearchMatch*)gsirealloc(
                                arg.matches, sizeof(GPProfileSearchMatch) * arg.numMatches);
                            if (arg.matches == NULL)
                                Error(connection, GP_MEMORY_ERROR, "Out of memory.");
                            ...
                            do {
                                ...
                                oldIndex = index;
                                CHECK_RESULT(
                                    gpiReadKeyAndValue(connection, data->inputBuffer.buffer, &index, key, value));
                            ...
                            CallbackFatalError(
                                connection, GP_NETWORK_ERROR, GP_PARSE, "Error reading from the search server.");
                        }
                    } while (!done);
                    ...
                    if ((more == GP_MORE) && (arg.more == GP_MORE))
                        CHECK_RESULT(gpiProfileSearch(connection,
                                                      ...
                                                      data->lastname,
                                                      data->icquin,
                                                      arg.numMatches + data->skip,
                                                      (GPEnum)operation->blocking,
                                                      operation->callback.callback,
                                                      operation->callback.param));
                    ...
                    if (callback.callback != NULL) {
                        ...
                        do {
                            ...
                            if (strcmp(key, "psrdone") == 0) {
                                ...
                                //////////////////////
                                arg->numMatches++;
                                arg->matches = (GPFindPlayerMatch*)gsirealloc(
                                    arg->matches, sizeof(GPFindPlayerMatch) * arg->numMatches);
                                if (arg->matches == NULL)
                                    Error(connection, GP_MEMORY_ERROR, "Out of memory.");
                                match = &arg->matches[arg->numMatches - 1];
                    ...
                    if (callback.callback != NULL) {
                        ...
                        do {
                            ...
                            if (strcmp(key, "odone") == 0) {
                                ...
                                //////////
                                tempPtr =
                                    gsirealloc(arg->profiles, sizeof(GPProfileSearchMatch) * (arg->numProfiles + 1));
                                if (tempPtr == NULL)
                                    Error(connection, GP_MEMORY_ERROR, "Out of memory.");
                    ...
                    if (callback.callback != NULL) {
                        ...
                        /////////////////
                        arg =
                            (GPGetReverseBuddiesListResponseArg*)gsimalloc(sizeof(GPGetReverseBuddiesListResponseArg));
                        if (arg == NULL)
                            Error(connection, GP_MEMORY_ERROR, "Out of memory.");
                        arg->result = GP_NO_ERROR;
                        ...
                        do {
                            ...
                            if (strcmp(key, "oldone") == 0) {
                                ...
                                //////////
                                tempPtr =
                                    gsirealloc(arg->matches, sizeof(GPUniqueMatch) * (arg->numOfUniqueMatchs + 1));
                                if (tempPtr == NULL)
                                    Error(connection, GP_MEMORY_ERROR, "Out of memory.");
                        ...
                        /////////
                        CHECK_RESULT(
                            gpiAddCallback(connection, callback, arg, operation, GPI_ADD_REVERSE_BUDDIES_LIST));
                    }
...
#else
                                arg->suggestedNicks[count] =
                                    (unsigned short*)gsimalloc(GP_UNIQUENICK_LEN * sizeof(unsigned short));
                                if (arg->suggestedNicks[count] == NULL)
                                    Error(connection, GP_MEMORY_ERROR, "Out of memory.");
