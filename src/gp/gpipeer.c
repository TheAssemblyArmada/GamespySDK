...
//FUNCTIONS
///////////
static GPResult gpiProcessPeerInitiatingConnection(GPConnection* connection, GPIPeer* peer)
{
...
static GPResult gpiProcessPeerAcceptingConnection(GPConnection* connection, GPIPeer* peer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GSUdpPeerState aPeerState;
    char* str;
    ...
    if (str != NULL) {
        ...
        if (strncmp(peer->inputBuffer.buffer, "\\auth\\", 6) == 0) {
            ...
            //////////////////////////////////
            sprintf(buffer, "%s%d%d", iconnection->password, iconnection->profileid, pid);
...
static GPResult gpiProcessPeerConnected(GPConnection* connection, GPIPeer* peer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    //int len;
    GSUdpPeerState aPeerState;
...
void gpiCheckTimedOutPeerOperations(GPConnection* connection, GPIPeer* peer)
{
    ...
    while (anIterator && anIterator != peer->peerOpQueue.last) {
        if (anIterator->state != GPI_PEER_OP_STATE_FINISHED && current_time() > anIterator->timeout
            && anIterator->callback) {
            ...
            if (anIterator->type == GPI_BM_KEYS_REQUEST) {
                ...
                GPGetBuddyStatusInfoKeysArg* arg =
                    (GPGetBuddyStatusInfoKeysArg*)gsimalloc(sizeof(GPGetBuddyStatusInfoKeysArg));
    ...
    if (iconnection->peerList == peer) {
        ...
        for (pprev = iconnection->peerList; pprev->pnext != peer; pprev = pprev->pnext) {
            if (pprev->pnext == NULL) {
                ...
                assert(0);
                gsDebugFormat(
                    GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError, "Tried to remove peer not in list.");
                return;
            }
        }
    ...
    while (ArrayLength(peer->messages)) {
        ...
        if (message->type < 100)
            gpiSendServerBuddyMessage(
                connection, peer->profile, message->type, message->buffer.buffer + message->start);
	...
	*/
    ...
    for (peer = iconnection->peerList; peer != NULL; peer = nextPeer) {
        ...
        if (peer->state == GPI_PEER_DISCONNECTED) {
            ...
            //gsDebug
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Notice, "Peer disconnected, pid: %d", peer->profile);
...
GPIPeer* gpiGetPeerByAddr(const GPConnection* connection, unsigned int ip, unsigned short port)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPIPeer* pcurr;
	...
	*/
    ...
    if (profile->buddyStatusInfo) {
        ...
        if (aPeerState != GS_UDP_PEER_CONNECTED || aPeerState != GS_UDP_PEER_CONNECTING) {
            anError = gsUdpEngineStartTalkingToPeer(profile->buddyStatusInfo->buddyIp,
                                                    profile->buddyStatusInfo->buddyPort,
                                                    iconnection->mHeader,
                                                    GPI_PEER_TIMEOUT);
            if (anError != GS_UDP_ADDRESS_ALREADY_IN_USE)
                CallbackError(
                    connection, GP_NETWORK_ERROR, GP_NETWORK, "There was an error starting communication with a peer.");
        }
...
GPResult
gpiPeerStartTransferMessage(GPConnection* connection, GPIPeer* peer, int type, const struct GPITransferID_s* transferID)
{
    char buffer[64];
    GPITransferID tid;
...
void gpiPeerMessageCallback(unsigned int ip,
                            unsigned short port,
                            ...
                            void* userData)
{
    ...
    if (!aPeer) {
        ...
        if (aPeer) {
            ...
            gsDebugFormat(GSIDebugCat_GP,
                          ...
                          "gpiPeerMessageCallback: out of memory when allocating peer, addr: %s:%d",
                          inet_ntoa(anAddr),
                          port);
            return;
        }
    ...
    if ((int)messageLength > (size - writePos)) {
        ...
        size = (writePos + GSI_MAX(GPI_READ_SIZE, (int)messageLength));
        reallocedBuff = (unsigned char*)gsirealloc(buff, (unsigned int)size + 1);
        if (reallocedBuff == NULL) {
            gsDebugFormat(GSIDebugCat_GP,
                          GSIDebugType_Memory,
                          GSIDebugLevel_HotError,
                          "gpiPeerMessageCallback: out of memory when reallocating buffer, addr: %s:%d",
                          inet_ntoa(anAddr),
                          port);
            gsifree(buff);
...
void gpiPeerAcceptedCallback(unsigned int ip,
                             unsigned short port,
