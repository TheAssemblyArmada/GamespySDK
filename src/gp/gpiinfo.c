...
static GPResult gpiDateToInt(GPConnection* connection, int* date, int day, int month, int year)
{
    int temp;
...
static GPResult gpiIntToDate(GPConnection* connection, int date, int* day, int* month, int* year)
{
    int d;
    int m;
    int y;
...
GPResult gpiProcessGetInfo(GPConnection* connection, GPIOperation* operation, const char* input)
{
    ...
    if (!gpiValueForKey(input, "\\birthday\\", buffer, sizeof(buffer))) {
        ...
        CHECK_RESULT(
            gpiIntToDate(connection, atoi(buffer), &infoCache.birthday, &infoCache.birthmonth, &infoCache.birthyear));
    }
...
static GPResult gpiSendLocalInfo(GPConnection* connection, const char* info, const char* value)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
...
static GPResult gpiSendUserInfo(GPConnection* connection, const char* info, const char* value)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
...
GPResult gpiGetInfo(GPConnection* connection,
                    GPProfile profile,
                    GPEnum checkCache,
                    GPEnum blocking,
                    GPCallback callback,
