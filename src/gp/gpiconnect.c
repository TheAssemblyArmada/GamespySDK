...
static GPResult gpiStartConnect(GPConnection* connection, GPIOperation* operation)
{
    ...
    if (!gsUdpEngineIsInitialized()) {
        ...
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_Notice, "Initializing UDP Layer\n");
        ...
        if (anError != GS_UDP_NO_ERROR) {
            ...
            if (anError != GS_UDP_NO_ERROR) {
                ...
                CallbackFatalError(
                    connection, GP_NETWORK_ERROR, GP_UDP_LAYER, "There was error starting the UDP layer.");
            }
        }
    ...
    anError = gsUdpEngineAddMsgHandler(iconnection->mHeader,
                                       iconnection->mHeader,
                                       NULL,
...
GPResult gpiProcessConnect(GPConnection* connection, GPIOperation* operation, const char* input)
{
    ...
    case GPI_REQUESTING:
        ...
        if (!gpiValueForKey(input, "\\userid\\", buffer, sizeof(buffer)))
            CallbackFatalError(
                connection, GP_NETWORK_ERROR, GP_PARSE, "Unexepected data was received from the server.");
...
GPResult gpiCheckConnect(GPConnection* connection)
{
    ...
    if (state == GPI_DISCONNECTED)
        CallbackFatalError(
            connection, GP_SERVER_ERROR, GP_LOGIN_CONNECTION_FAILED, "The server has refused the connection.");
...
static GPIBool gpiDisconnectCleanupProfile(GPConnection* connection, GPIProfile* profile, void* data)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GSI_UNUSED(data);
    ...
    if ((!profile->cache) || (iconnection->infoCachingBuddyAndBlockOnly == GPITrue && !profile->buddyOrBlockCache)) {
