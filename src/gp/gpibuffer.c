...
GPResult
gpiAppendStringToBufferLen(GPConnection* connection, GPIBuffer* outputBuffer, const char* string, int stringLen)
{
    int len;
    int size;
    char* output;
...
GPResult gpiAppendShortToBuffer(GPConnection* connection, GPIBuffer* outputBuffer, short num)
{
    char shortVal[8];
...
GPResult gpiAppendUShortToBuffer(GPConnection* connection, GPIBuffer* outputBuffer, unsigned short num)
{
    char shortVal[8];
...
static GPResult gpiSendData(GPConnection* connection,
                            SOCKET sock,
                            const char* buffer,
                            int bufferLen,
                            GPIBool* closed,
                            ...
                            char id[3])
{
    ...
    if (gsiSocketIsError(rcode)) {
        ...
        *closed = GPIFalse;
        ...
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_Comment, "SENDXXXX(%s): Connection closed\n", id);
...
GPResult gpiSendOrBufferStringLenToPeer(GPConnection* connection, GPIPeer_st peer, const char* string, int stringLen)
{
    ...
    /////////////////////////////////////////////////////////////////////
    if (!(peer->outputBuffer.len - peer->outputBuffer.pos) && !ArrayLength(peer->messages)) {
        if ((int)remaining <= (gsUdpEngineGetPeerOutBufferFreeSpace(peer->ip, peer->port) - GS_UDP_RELIABLE_MSG_HEADER
                               - GS_UDP_MSG_HEADER_LEN)) {
            gsUdpEngineSendMessage(
                peer->ip, peer->port, iconnection->mHeader, (unsigned char*)string, remaining, gsi_true);
            total = remaining;
            remaining = 0;
        } else {
            ...
            if (freeSpace > (GS_UDP_MSG_HEADER_LEN + GS_UDP_RELIABLE_MSG_HEADER)) {
                ...
                gsUdpEngineSendMessage(
                    peer->ip, peer->port, iconnection->mHeader, (unsigned char*)string, sent, gsi_true);
                total = sent;
                remaining -= sent;
            }
        }
...
GPResult gpiSendOrBufferString(GPConnection* connection, GPIPeer_st peer, char* string)
{
...
GPResult gpiRecvToBuffer(GPConnection* connection,
                         SOCKET sock,
                         ...
                         char id[3])
{
    ...
    if (total) {
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_RawDump, "RECVTOTL(%s): %d\n", id, total);
    }
...
GPResult gpiSendFromBuffer(GPConnection* connection,
                           SOCKET sock,
...
GPResult gpiSendBufferToPeer(GPConnection* connection,
                             unsigned int ip,
                             unsigned short port,
                             GPIBuffer* outputBuffer,
                             GPIBool* closed,
                             GPIBool clipSentData)
{
    ...
    if ((int)remaining
        <= (gsUdpEngineGetPeerOutBufferFreeSpace(ip, port) - GS_UDP_RELIABLE_MSG_HEADER - GS_UDP_MSG_HEADER_LEN)) {
        ...
        do {
            ...
            if (sendAmount <= (GS_UDP_MSG_HEADER_LEN + GS_UDP_RELIABLE_MSG_HEADER))
                break;
            if (gsUdpEngineSendMessage(ip, port, iconnection->mHeader, &buffer[pos + total], sendAmount, gsi_true)
                == GS_UDP_SEND_FAILED)
                break;
            total += sendAmount;
...
GPResult
gpiReadMessageFromBuffer(GPConnection* connection, GPIBuffer* inputBuffer, char** message, int* type, int* plen)
{
    char* str;
    int len;
    char intValue[16];
