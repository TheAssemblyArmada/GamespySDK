...
static GPResult gpiFinishTransferMessage(GPConnection* connection, GPITransfer* transfer, const char* message, int len)
{
...
static GPResult gpiNewTransfer(GPConnection* connection, GPITransfer** transfer, GPProfile profile, GPIBool sender)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
...
GPResult gpiNewSenderTransfer(GPConnection* connection, GPITransfer** transfer, GPProfile profile)
{
...
static GPResult
gpiNewReceiverTransfer(GPConnection* connection, GPITransfer** transfer, GPProfile profile, GPITransferID* transferID)
{
#ifdef WIN32
...
void gpiFreeTransfer(GPConnection* connection, GPITransfer* transfer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    int pos;
...
void gpiCancelTransfer(GPConnection* connection, GPITransfer* transfer)
{
    ...
    if (transfer->peer) {
        // Start the message.
        /////////////////////
        if (gpiPeerStartTransferMessage(
                connection, transfer->peer, GPI_BM_FILE_TRANSFER_CANCEL, (GPITransferID_st)&transfer->transferID)
            == GP_NO_ERROR) {
            // Finish the message.
            //////////////////////
...
void gpiTransferError(GPConnection* connection, const GPITransfer* transfer)
{
...
GPIFile* gpiAddFileToTransfer(GPITransfer* transfer, const char* path, const char* name)
{
    GPIFile file;
    char* str;
...
static GPResult gpiSendTransferRequest(GPConnection* connection, GPITransfer* transfer)
{
    char buffer[32];
    GPIFile* file;
    int i;
    ...
    CHECK_RESULT(gpiPeerStartTransferMessage(
        connection, transfer->peer, GPI_BM_FILE_SEND_REQUEST, (GPITransferID_st)&transfer->transferID));
...
static GPITransfer* gpiFindTransferByTransferID(GPConnection* connection, GPITransferID* transferID)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    int i;
    int num;
...
GPITransfer* gpiFindTransferByLocalID(GPConnection* connection, int localID)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    int i;
    int num;
...
int gpiGetTransferLocalIDByIndex(GPConnection* connection, int index)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPITransfer* transfer;
    int num;
...
void gpiSkipFile(GPConnection* connection, GPITransfer* transfer, int file, int reason)
{
    char buffer[32];

    if (gpiPeerStartTransferMessage(connection, transfer->peer, GP_FILE_SKIP, (GPITransferID_st)&transfer->transferID)
        != GP_NO_ERROR)
        return;
...
void gpiSkipCurrentFile(GPConnection* connection, GPITransfer* transfer, int reason)
{
...
static GPIBool gpiHandleSendRequest(GPConnection* connection,
                                    GPIPeer* peer,
                                    ...
                                    int bufferLen)
{
    ...
    for (i = 0; i < numFiles; i++) {
        ...
        len = strlen(name);
        if (strstr(name, "//") || strstr(name, "./") || (name[len - 1] == '.') || (name[0] == '/')
            || (strcspn(name, ":*?\"<>|\n") != len)) {
            gpiFreeTransfer(connection, transfer);
            return GPIFalse;
        }
...
static GPIBool gpiHandleBegin(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPTransferCallbackArg* arg;
    GPIFile* file;
...
static GPIBool gpiHandleEnd(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPTransferCallbackArg* arg;
    GPIFile* file;
    ...
    if (file->flags & GPI_FILE_DIRECTORY) {
        ...
        ///////////////////////
        if (gpiPeerStartTransferMessage(
                connection, transfer->peer, GPI_BM_FILE_END, (GPITransferID_st)&transfer->transferID)
            != GP_NO_ERROR)
            return GPIFalse;
        gpiFinishTransferMessage(connection, transfer, NULL, 0);
#endif
...
static GPIBool
gpiHandleData(GPConnection* connection, GPITransfer* transfer, const char* headers, const char* buffer, int bufferLen)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPTransferCallbackArg* arg;
    GPIFile* file;
    ...
    gsDebugFormat(GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_RawDump, "HNDLDATA(PT): %d\n", bufferLen);
    ...
    if (writeFailed) {
        ...
        //////////////////////////
        if (gpiPeerStartTransferMessage(
                connection, transfer->peer, GPI_BM_FILE_DATA, (GPITransferID_st)&transfer->transferID)
            != GP_NO_ERROR)
            return GPIFalse;
...
static GPIBool gpiHandleSkip(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPTransferCallbackArg* arg;
    GPIFile* file;
...
static GPIBool gpiHandleTransferThrottle(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    int throttle;
    char intValue[16];
    ...
    if (transfer->sender) {
        if (gpiPeerStartTransferMessage(
                connection, transfer->peer, GPI_BM_FILE_TRANSFER_THROTTLE, (GPITransferID_st)&transfer->transferID)
            != GP_NO_ERROR)
            return GPIFalse;
...
static GPIBool gpiHandleTransferCancel(GPConnection* connection, GPITransfer* transfer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPTransferCallbackArg* arg;
...
static GPIBool gpiHandleTransferKeepalive(GPConnection* connection, GPITransfer* transfer)
{
...
static GPResult gpiSendFileEnd(GPConnection* connection, GPITransfer* transfer, GPIFile* file)
{
    CHECK_RESULT(gpiPeerStartTransferMessage(
        connection, transfer->peer, GPI_BM_FILE_END, (GPITransferID_st)&transfer->transferID));
...
static GPResult gpiSendFileBegin(GPConnection* connection, GPITransfer* transfer, GPIFile* file)
{
    char buffer[64];
    ...
    CHECK_RESULT(gpiPeerStartTransferMessage(
        connection, transfer->peer, GPI_BM_FILE_BEGIN, (GPITransferID_st)&transfer->transferID));
...
static GPResult gpiSendFileData(GPConnection* connection, GPITransfer* transfer, unsigned char* data, size_t len)
{
    CHECK_RESULT(gpiPeerStartTransferMessage(
        connection, transfer->peer, GPI_BM_FILE_DATA, (GPITransferID_st)&transfer->transferID));
...
GPResult gpiProcessCurrentFile(GPConnection* connection, GPITransfer* transfer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPIFile* file;
    GPTransferCallbackArg* arg;
        ...
        if (file->flags & GPI_FILE_DIRECTORY) {
            ...
            if (!file->file) {
                ...
                if (file->file) {
                    ...
                    if (arg) {
                        ...
                        arg->index = transfer->currentFile;
                        arg->type = GP_FILE_BEGIN;
                        gpiAddCallback(connection,
                                       iconnection->callbacks[GPI_TRANSFER_CALLBACK],
                                       arg,
                                       NULL,
            ...
            for (i = 0; (file->progress < file->size) && !transfer->peer->outputBuffer.len /*&& (i < 20)*/; i++) {
                ...
                if (num) {
                    ...
                    // Send the data.
                    /////////////////
                    CHECK_RESULT(gpiSendFileData(connection, transfer, (unsigned char*)buffer, (int)num));

                    // Update progress.
                    ///////////////////
                    transfer->progress += (int)num;
                    file->progress += (int)num;
                    total += (int)num;
            ...
            if (total) {
                gsDebugFormat(GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_RawDump, "SENTTOTL(PT): %d\n", total);
            }
...
GPResult gpiProcessTransfer(GPConnection* connection, GPITransfer* transfer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    int currentFile;
    int len;
    ...
    if (!transfer->peer) {
        ...
        if ((now - transfer->lastSend) > GPI_KEEPALIVE_TIME) {
            ...
            ////////////////////
            CHECK_RESULT(gpiPeerStartTransferMessage(
                connection, transfer->peer, GPI_BM_FILE_TRANSFER_KEEPALIVE, (GPITransferID_st)&transfer->transferID));
            gpiFinishTransferMessage(connection, transfer, NULL, 0);
        }
    }
...
GPResult gpiProcessTransfers(GPConnection* connection)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    int len;
    int i;
...
void gpiTransferPeerDestroyed(GPConnection* connection, GPIPeer* peer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPTransferCallbackArg* arg;
    GPITransfer* transfer;
...
void gpiTransfersHandlePong(GPConnection* connection, GPProfile profile, GPIPeer* peer)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPITransfer* transfer;
    int i;
...
GPResult gpiSendTransferReply(GPConnection* connection,
...
void gpiHandleTransferMessage(GPConnection* connection,
                              GPIPeer* peer,
                              int type,
                              const char* headers,
                              const char* buffer,
