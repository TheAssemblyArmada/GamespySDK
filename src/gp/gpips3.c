...
SceNpCommunicationId gpi_communication_id = {{'N', 'P', 'X', 'S', '0', '0', '0', '0', '5'}, '\0', 0, 0};
...
GPResult gpiSyncNpBuddies(GPConnection* connection)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        gpiProfileSearchUniquenick(connection,
                                   npId.handle.data,
                                   &iconnection->namespaceID,
                                   1,
                                   GP_NON_BLOCKING,
                                   (GPCallback)gpiSyncNpBuddiesCallback,
                                   NULL);
    }
...
void gpiSyncNpBuddiesCallback(GPConnection* pconnection, GPProfileSearchResponseArg* arg, void* param)
{
    if (arg->result == GP_NO_ERROR) {
        if (arg->numMatches == 1) {
            ...
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Comment, "PS3BuddySync: No suitable match found\n");
    } else
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError, "PS3BuddySync: Buddy Search FAILED!\n");
...
GPResult gpiSyncNpBlockList(GPConnection* connection)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        gpiProfileSearchUniquenick(connection,
                                   ...
                                   1,
                                   GP_NON_BLOCKING,
                                   (GPCallback)gpiSyncNpBlockListCallback,
                                   NULL);
    }
...
void gpiSyncNpBlockListCallback(GPConnection* pconnection, GPProfileSearchResponseArg* arg, void* param)
{
    ...
    if (arg->result == GP_NO_ERROR) {
        if (arg->numMatches == 1) {
            ...
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Comment, "PS3BlockSync: No suitable match found\n");
    } else
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError, "PS3BlockSync: Block Entry Search FAILED!\n");
...
GPResult gpiAddToNpBlockList(GPConnection* connection, int profileid)
{
    ...
    /////////////////////////////////////////////////////////////////
    gpiGetInfo(
        connection, profileid, GP_CHECK_CACHE, GP_NON_BLOCKING, (GPCallback)gpiAddToNpBlockListInfoCallback, NULL);
        ...
        if (_tcslen(arg->uniquenick) != 0) {
            ...
            if (ArrayLength(iconnection->npTransactionList) < GPI_NP_NUM_TRANSACTIONS) {
                ...
                if (ret < 0) {
                    ...
                    /////////////////////////////////////
                    ret = sceNpLookupNpIdAsync(transaction.npTransId, &onlineId, transaction.npIdForAdd, 0, NULL);
                    if (ret < 0) {
                        gsDebugFormat(GSIDebugCat_GP,
                                      GSIDebugType_Misc,
                                      GSIDebugLevel_HotError,
