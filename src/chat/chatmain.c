...
#define ASSERT_NICK()                                                                                                  \
    assert(nick != NULL);                                                                                              \
    assert(nick[0] != '\0');                                                                                           \
    assert(strlen(nick) < MAX_NICK);
#define ASSERT_USER(user)                                                                                              \
    assert(user != NULL);                                                                                              \
    assert(user[0] != '\0');                                                                                           \
    assert(strlen(user) < MAX_USER);
#define ASSERT_MESSAGE()                                                                                               \
    assert(message != NULL);                                                                                           \
    assert(message[0] != '\0');
#define ASSERT_TYPE(type)                                                                                              \
    assert((type == CHAT_MESSAGE) || (type == CHAT_ACTION) || (type == CHAT_NOTICE) || (type == CHAT_UTM)              \
           || (type == CHAT_ATM));
#define ASSERT_PASSWORD()                                                                                              \
...
void ciHandleDisconnect(CHAT chat, const char* reason)
{
    ...
    //////////////////////////////////////
        ...
        ciAddCallback(chat,
                      ...
                      (void*)connection->globalCallbacks.disconnected,
                      &params,
                      connection->globalCallbacks.param,
                      0,
                      NULL);
    }
    ...
    if (connection->chatSocket.connectState == ciConnected) {
        ...
        while ((message = ciSocketRecv(&connection->chatSocket)) != NULL) {
            ...
            if (connection->globalCallbacks.raw != NULL) {
                ...
                ciAddCallback(chat,
                              CALLBACK_RAW,
                              (void*)connection->globalCallbacks.raw,
                              &params,
                              connection->globalCallbacks.param,
                              0,
                              NULL);
...
void ciSendLogin(CHAT chat)
{
    ...
    if (connection->loginType == CIPreAuthLogin) {
        ciSocketSendf(
            &connection->chatSocket, "LOGINPREAUTH %s %s", connection->authtoken, connection->partnerchallenge);
    ...
    /////////////////////////////////////////////////////////////////
    MD5Digest((unsigned char*)connection->password, (unsigned int)strlen(connection->password), passwordHash);

    // Send the login message based on type.
    ////////////////////////////////////////
    if (connection->loginType == CIUniqueNickLogin) {
        ciSocketSendf(
            &connection->chatSocket, "LOGIN %d %s %s", connection->namespaceID, connection->uniquenick, passwordHash);
                  ...
                  CHATBool blocking)
{
    ...
    CHAT aChat = chatConnectA(
        serverAddress_A, port, nick_A, user_A, name_A, callbacks, nickErrorCallback, connectCallback, param, blocking);
                         ...
                         CHATBool blocking)
{
    ...
    char* nick_A = (char*)UCS2ToUTF8StringAlloc(nick);
    char* name_A = (char*)UCS2ToUTF8StringAlloc(name);
    CHAT aChat = chatConnectSpecialA(serverAddress_A,
                                     port,
                                     nick_A,
                                     name_A,
                                     callbacks,
                        ...
                        CHATBool blocking)
{
    ...
    CHAT aChat = chatConnectSecureA(serverAddress_A,
                                    port,
                                    nick_A,
                                    name_A,
                                    gamename_A,
                       ...
                       CHATBool blocking)
{
    ...
    char* gamename_A = (char*)UCS2ToUTF8StringAlloc(gamename);
    char* secretKey_A = (char*)UCS2ToUTF8StringAlloc(secretKey);
    CHAT aChat = chatConnectLoginA(serverAddress_A,
                                   port,
                                   namespaceID,
                                   email_A,
                                   profilenick_A,
                         ...
                         CHATBool blocking)
{
    ...
    char* gamename_A = (char*)UCS2ToUTF8StringAlloc(gamename);
    char* secretKey_A = (char*)UCS2ToUTF8StringAlloc(secretKey);
    CHAT aChat = chatConnectPreAuthA(serverAddress_A,
                                     port,
                                     authtoken_A,
                                     partnerchallenge_A,
                                     name_A,
...
#ifdef GSI_UNICODE
void chatRetryWithNickW(CHAT chat, const unsigned short* nick)
{
    char* nick_A = (char*)UCS2ToUTF8StringAlloc(nick);
...
#ifdef GSI_UNICODE
void chatRegisterUniqueNickW(CHAT chat, int namespaceID, const unsigned short* uniquenick, const unsigned short* cdkey)
{
    char* uniquenick_A = (char*)UCS2ToUTF8StringAlloc(uniquenick);
...
#ifdef GSI_UNICODE
void chatSendRawW(CHAT chat, const unsigned short* command)
{
...
#ifdef GSI_UNICODE
void chatFixNickW(unsigned short* newNick, const unsigned short* oldNick)
{
...
#ifdef GSI_UNICODE
const unsigned short* chatTranslateNickW(unsigned short* nick, const unsigned short* extension)
{
...
static void ciSetQuietModeEnumJoinedChannelsA(CHAT chat, int index, const char* channel, void* param)
{
    // Setup a filter.
    //////////////////
...
#ifdef GSI_UNICODE
static void ciSetQuietModeEnumJoinedChannelsW(CHAT chat, int index, const unsigned short* channel, void* param)
{
...
#ifdef GSI_UNICODE
void chatLeaveChannelW(CHAT chat, const unsigned short* channel, const unsigned short* reason)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
void chatSendChannelMessageA(CHAT chat, const char* channel, const char* message, int type)
{
    ...
    if (callbacks != NULL) {
        ...
        params.type = type;
        ciAddCallback(
            chat, CALLBACK_CHANNEL_MESSAGE, (void*)callbacks->channelMessage, &params, callbacks->param, 0, channel);
    }
}
#ifdef GSI_UNICODE
void chatSendChannelMessageW(CHAT chat, const unsigned short* channel, const unsigned short* message, int type)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
#ifdef GSI_UNICODE
void chatSetChannelTopicW(CHAT chat, const unsigned short* channel, const unsigned short* topic)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
#ifdef GSI_UNICODE
void chatSetChannelModeW(CHAT chat, const unsigned short* channel, CHATChannelMode* mode)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
#ifdef GSI_UNICODE
void chatSetChannelPasswordW(CHAT chat, const unsigned short* channel, CHATBool enable, const unsigned short* password)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
    char* password_A = (char*)UCS2ToUTF8StringAlloc(password);
...
#ifdef GSI_UNICODE
void chatSetChannelLimitW(CHAT chat, const unsigned short* channel, int limit)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
#ifdef GSI_UNICODE
void chatAddChannelBanW(CHAT chat, const unsigned short* channel, const unsigned short* ban)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
#ifdef GSI_UNICODE
void chatRemoveChannelBanW(CHAT chat, const unsigned short* channel, const unsigned short* ban)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
    char* ban_A = (char*)UCS2ToUTF8StringAlloc(ban);
...
#ifdef GSI_UNICODE
void chatSetChannelGroupW(CHAT chat, const unsigned short* channel, const unsigned short* group)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
#ifdef GSI_UNICODE
CHATBool chatInChannelW(CHAT chat, const unsigned short* channel)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
...
**********/
static void
ciEnumUsersCallback(CHAT chat, const char* channel, int numUsers, const char** users, int* modes, void* param)
{
...
#ifdef GSI_UNICODE
void chatSendUserMessageW(CHAT chat, const unsigned short* user, const unsigned short* message, int type)
{
    char* user_A = (char*)UCS2ToUTF8StringAlloc(user);
    char* message_A = (char*)UCS2ToUTF8StringAlloc(message);
...
#ifdef GSI_UNICODE
void chatInviteUserW(CHAT chat, const unsigned short* channel, const unsigned short* user)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
    char* user_A = (char*)UCS2ToUTF8StringAlloc(user);
...
#ifdef GSI_UNICODE
void chatKickUserW(CHAT chat, const unsigned short* channel, const unsigned short* user, const unsigned short* reason)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
    char* user_A = (char*)UCS2ToUTF8StringAlloc(user);
    char* reason_A = (char*)UCS2ToUTF8StringAlloc(reason);
...
#ifdef GSI_UNICODE
void chatBanUserW(CHAT chat, const unsigned short* channel, const unsigned short* user)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
    char* user_A = (char*)UCS2ToUTF8StringAlloc(user);
...
#ifdef GSI_UNICODE
void chatSetUserModeW(CHAT chat, const unsigned short* channel, const unsigned short* user, int mode)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
    char* user_A = (char*)UCS2ToUTF8StringAlloc(user);
...
#ifdef GSI_UNICODE
CHATBool chatGetUserModeNoWaitW(CHAT chat, const unsigned short* channel, const unsigned short* user, int* mode)
{
    char* channel_A = (char*)UCS2ToUTF8StringAlloc(channel);
    char* user_A = (char*)UCS2ToUTF8StringAlloc(user);
...
void chatGetUdpRelayA(CHAT chat, const char* channel, chatGetUdpRelayCallback callback, void* param, CHATBool blocking)
{
    int ID;
    CONNECTION;
    CONNECTED;
...
#ifdef GSI_UNICODE
void chatSetGlobalKeysW(CHAT chat, int num, const unsigned short** keys, const unsigned short** values)
{
...
static void ciSendGetKey(CHAT chat, const char* target, const char* cookie, int num, const char** keys)
{
    char buffer[512];
    int len;
    int i;
...
static CHATBool
ciSendGetChannelKey(CHAT chat, const char* channel, const char* nick, const char* cookie, int num, const char** keys)
{
    char buffer[512];
    int len;
    int i;
                         ...
                         CHATBool blocking)
{
    ...
    if (!user || !user[0])
        ...
        ID = ciAddGETCKEYFilter(
            chat, cookie, num, keys, (CHATBool)(strcmp(user, "*") == 0), getBroadcastKeys, callback, param);
...
void ciNickError(CHAT chat, int type, const char* nick, int numSuggestedNicks, char** suggestedNicks)
{
    ...
    if (connection->nickErrorCallback) {
        ...
        ciAddCallback(chat,
                      CALLBACK_NICK_ERROR,
                      (void*)connection->nickErrorCallback,
                      &params,
