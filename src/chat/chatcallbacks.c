...
#define ASSERT_DATA(data)                                                                                              \
    assert(data != NULL);                                                                                              \
    assert(data->type >= 0);                                                                                           \
    assert(data->type < CALLBACK_NUM);                                                                                 \
    assert(data->callback != NULL);                                                                                    \
    assert(data->callbackParams != NULL);                                                                              \
    assert(data->ID >= 0);
...
#define COPY(param)                                                                                                    \
    ...
    } // remove dangling "if" danger
#define COPY_MODE()                                                                                                    \
    if (srcParams->mode != NULL) {                                                                                     \
...
#define FREE_STRING_ARRAY(array, num)                                                                                  \
    if (num > 0 && array != NULL) {                                                                                    \
        int i = 0;                                                                                                     \
        for (; i < num; i++)                                                                                           \
            gsifree(array[i]);                                                                                         \
...
static void ciFreeCallbackData(ciCallbackData* data)
{
    ...
    case CALLBACK_GET_CHANNEL_BASIC_USER_INFO: {
        ciCallbackGetChannelBasicUserInfoParams* callbackParams =
            (ciCallbackGetChannelBasicUserInfoParams*)data->callbackParams;
        gsifree(CHANNEL);
        gsifree(NICK);
        gsifree(USER);
        gsifree(ADDRESS);
    ...
    case CALLBACK_BROADCAST_KEY_CHANGED: {
        ciCallbackBroadcastKeyChangedParams* callbackParams =
            (ciCallbackBroadcastKeyChangedParams*)data->callbackParams;
        gsifree(CHANNEL);
        gsifree(USER);
        gsifree(KEY);
        gsifree(VALUE);
...
CHATBool ciAddCallback_(CHAT chat,
                        int type,
                        void* callback,
                        void* callbackParams,
                        void* param,
                        ...
                        size_t callbackParamsSize)
{
    ...
    case CALLBACK_GET_CHANNEL_BASIC_USER_INFO: {
        ciCallbackGetChannelBasicUserInfoParams* destParams =
            (ciCallbackGetChannelBasicUserInfoParams*)data.callbackParams;
...
static void ciCallCallback(CHAT chat, ciCallbackData* data)
{
    ...
    case CALLBACK_ENUM_CHANNELS_ALL: {
        ...
        callback(chat,
                 SUCCESS,
                 NUM_CHANNELS,
                 (const unsigned short**)channels_W,
                 (const unsigned short**)topics_W,
                 NUM_USERS,
                 param);
    ...
    case CALLBACK_GET_USER_INFO: {
        ...
        unsigned short** channels_W = UTF8ToUCS2StringArrayAlloc((const char**)CHANNELS, NUM_CHANNELS);
        callback(
            chat, SUCCESS, nick_W, user_W, name_W, address_W, NUM_CHANNELS, (const unsigned short**)channels_W, param);
        gsifree(nick_W);
        gsifree(user_W);
        gsifree(name_W);
        gsifree(address_W);
    ...
    case CALLBACK_NICK_ERROR: {
        ...
        unsigned short** suggestedNicks_W =
            UTF8ToUCS2StringArrayAlloc((const char**)SUGGESTED_NICKS, NUM_SUGGESTED_NICKS);
