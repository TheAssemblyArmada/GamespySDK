...
#define ASSERT_CHANNEL()                                                                                               \
    assert(channel != NULL);                                                                                           \
    assert(channel[0] != '\0');                                                                                        \
    assert(strlen(channel) < MAX_CHANNEL);
#define ASSERT_USER(user)                                                                                              \
    assert(user != NULL);                                                                                              \
    assert(user[0] != '\0');                                                                                           \
    assert(strlen(user) < MAX_NAME);
...
{
    connection->channelTable = TableNew2(sizeof(ciChatChannel),
                                         CHANNEL_HASH_BUCKETS,
                                         2,
                                         ciChannelTableHashFn,
                                         ciChannelTableCompareFn,
...
void ciChannelEntered(CHAT chat, const char* channel, chatChannelCallbacks* callbacks)
{
    ...
    strzcpy(chatChannel.name, channel, MAX_CHANNEL);
    chatChannel.users = TableNew2(
        sizeof(ciChatUser), USER_HASH_BUCKETS, 2, ciUserTableHashFn, ciUserTableCompareFn, ciUserTableElementFreeFn);
...
** USER FUNCTIONS **
*******************/
void ciUserEnteredChannel(CHAT chat,
                          const char* name,
                          const char* channel,
                          int mode,
                          const char* user,
...
static void ciUserChangeNickMap(void* elem, void* clientData)
{
    ...
    if (user != NULL) {
        ...
        ////////////////////////////////
        if (ciWasJoinCallbackCalled(data->chat,
                                    channel->name)) //PANTS - 03.01.00 - check if the join callback was called
        {
            // Add the callback.
            ////////////////////
            if (channel->callbacks.userChangedNick != NULL) {
                ...
                ciAddCallback(data->chat,
                              CALLBACK_USER_CHANGED_NICK,
                              (void*)channel->callbacks.userChangedNick,
                              &params,
                              channel->callbacks.param,
                              0,
                              channel->name);
            }
        }
...
void ciUserChangedMode(CHAT chat, const char* user, const char* channel, int mode, CHATBool enabled)
{
    ...
    if (chatChannel->callbacks.userModeChanged != NULL) {
        ...
        ciAddCallback(chat,
                      ...
                      (void*)chatChannel->callbacks.userModeChanged,
                      &params,
                      chatChannel->callbacks.param,
                      0,
                      channel);
    }
}
...
static void ciClearAllUsersChannelMap(void* elem, void* clientData)
{
    ...
    if (channel->callbacks.userListUpdated != NULL) {
        ...
        ciAddCallback(chat,
                      ...
                      (void*)channel->callbacks.userListUpdated,
                      &params,
                      channel->callbacks.param,
                      0,
                      channel->name);
