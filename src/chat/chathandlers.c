...
** GLOBALS **
************/
ciServerMessageType serverMessageTypes[] = {{"PRIVMSG", ciPrivmsgHandler},
            ...
            if (filter->type == matches[i].type) {
                ...
                if ((!name && !filter->name) || (name && filter->name && (strcasecmp(name, filter->name) == 0))) {
                    if ((!name2 && !filter->name2)
                        || (name2 && filter->name2 && (strcasecmp(name2, filter->name2) == 0))) {
    ...
    if (filter->type == TYPE_LIST) {
        ...
        if (filter->callback)
            ciAddCallback_(chat,
                           ...
                           filter->ID,
                           NULL,
                           sizeof(ciCallbackGetUdpRelayParams));
...
static int ciAddFilter(CHAT chat,
                       int type,
                       const char* name,
                       const char* name2,
                       void* callback,
...
int ciAddGETKEYFilter(CHAT chat,
                      const char* cookie,
                      int num,
                      const char** keys,
                      const char* channel,
...
int ciAddGETCKEYFilter(CHAT chat,
                       const char* cookie,
                       int num,
                       const char** keys,
                       CHATBool channel,
...
int ciAddGETCHANKEYFilter(CHAT chat,
                          const char* cookie,
                          int num,
                          const char** keys,
                          CHATBool getBroadcastKeys,
    ...
    if (strcasecmp(target, connection->nick) == 0) {
        if (connection->globalCallbacks.privateMessage != NULL) {
            ...
            ciAddCallback(chat,
                          ...
                          (void*)connection->globalCallbacks.privateMessage,
                          &params,
        ...
        if ((callbacks != NULL) && (callbacks->channelMessage != NULL)) {
            ...
            if (action)
                ...
                params.type = CHAT_MESSAGE;
            ciAddCallback(
                chat, CALLBACK_CHANNEL_MESSAGE, (void*)callbacks->channelMessage, &params, callbacks->param, 0, target);
        }
    }
}
    ...
    if (ciWasJoinCallbackCalled(chat, channel)) //PANTS - 03.01.00 - check if the join callback was called
    {
        ...
        if (callbacks != NULL) {
            if (callbacks->userJoined != NULL) {
                ...
                params.mode = mode;
                ciAddCallback(
                    chat, CALLBACK_USER_JOINED, (void*)callbacks->userJoined, &params, callbacks->param, 0, channel);
            }
            ...
            if (callbacks->userListUpdated != NULL) {
                ...
                ciAddCallback(chat,
                              ...
                              (void*)callbacks->userListUpdated,
                              &params,
                              callbacks->param,
                              0,
                              channel);
    ...
    if (ciWasJoinCallbackCalled(chat, channel)) {
        ...
        if (callbacks != NULL) {
            ...
            if (callbacks->userParted != NULL) {
                ...
                params.kicker = NULL;
                ciAddCallback(
                    chat, CALLBACK_USER_PARTED, (void*)callbacks->userParted, &params, callbacks->param, 0, channel);
            }
...
void ciTopicHandler(CHAT chat, const ciServerMessage* message)
{
    ...
    if ((callbacks != NULL) && (callbacks->topicChanged != NULL)) {
        ...
        params.topic = topic;
        ciAddCallback(
            chat, CALLBACK_TOPIC_CHANGED, (void*)callbacks->topicChanged, &params, callbacks->param, 0, channel);
    }
}
...
void ciInviteHandler(CHAT chat, const ciServerMessage* message)
{
    ...
    if (connection->globalCallbacks.invited != NULL) {
        ...
        ciAddCallback(chat,
                      CALLBACK_INVITED,
                      (void*)connection->globalCallbacks.invited,
                      &params,
                      connection->globalCallbacks.param,
                      0,
                      NULL);
...
void ciRplWhoReplyHandler(CHAT chat, const ciServerMessage* message)
{
    ...
    if ((filter->type == TYPE_CWHO) && filter->callback) {
        ...
        ciAddCallback(
            chat, CALLBACK_GET_CHANNEL_BASIC_USER_INFO, filter->callback, &params, filter->param, filter->ID, NULL);
            ...
            while (*flags) {
                ...
                *flags = '\0';
                ...
                ciAddCallback(chat,
                              ...
                              (void*)callbacks->broadcastKeyChanged,
                              &params,
                              callbacks->param,
                              0,
                              channel);
...
void ciRplUserIPHandler(CHAT chat, const ciServerMessage* message)
{
    ...
    if (IP) {
        ...
        if (connection->fillInUserCallback) {
            ...
            UCS2ToAsciiString(
                user, connection->user); // convert the unicode user to ascii (nicknames cannot currently be UTF-8!!)
#endif
        }
    }
...
void ciRplListHandler(CHAT chat, const ciServerMessage* message)
{
    ...
    if (filter != NULL) {
        if (filter->callback != NULL) {
            ...
            ciAddCallback(
                chat, CALLBACK_ENUM_CHANNELS_EACH, filter->callback, &params, filter->param, filter->ID, NULL);
    ...
    if (!ciGetChannelMode(chat, channel, &dummyMode)) {
        ...
        if ((callbacks != NULL) && (callbacks->channelModeChanged != NULL)) {
            ...
            ciAddCallback(chat,
                          ...
                          (void*)callbacks->channelModeChanged,
                          &params,
                          callbacks->param,
                          0,
                          channel);
