...
static GVBool gviFillDeviceInfo(AudioDeviceID deviceID, GVDeviceInfo* device, GVDeviceType types)
{
    ...
    result = AudioDeviceGetProperty(deviceID,
                                    0,
                                    (supportedTypes & GV_CAPTURE) ? true : false,
    ...
    if (data->m_capturing) {
        ...
        do {
            ...
            result = AudioConverterFillComplexBuffer(
                data->m_captureConverter, gviAudioConverterCaptureProc, &converterData, &size, &bufferList, NULL);
                                              ...
                                              void* inUserData)
{
    ...
    if (data->m_playing) {
        ...
        wroteToBuffer =
            gviWriteSourcesToBuffer(data->m_playbackSources, data->m_playbackClock, data->m_playbackBuffer, 1);
    }
                                          ...
                                          void* inClientData)
{
    ...
    result = AudioConverterFillComplexBuffer(
        data->m_playbackConverter, gviAudioConverterPlaybackProc, device, &size, &bufferList, NULL);
    if (result != noErr)
        return (OSStatus)1;
...
static void gviSetDeviceVolume(GVDevice device, GVBool isInput, GVScalar volume)
{
    ...
    for (channel = 0; channel < 3; channel++)
        AudioDeviceSetProperty(
            device->m_deviceID, NULL, channel, isInput, kAudioDevicePropertyVolumeScalar, sizeof(Float32), &vol);
}
...
static GVBool gviGetChannelVolume(GVDevice device, GVBool isInput, int channel, Float32* volume)
{
    ...
    result =
        AudioDeviceGetProperty(device->m_deviceID, channel, isInput, kAudioDevicePropertyVolumeScalar, &size, volume);
    ...
    *len = 0;
    ...
    do {
        ...
        if (overThreshold) {
            ...
            overThreshold = ((GVFrameStamp)(frame->m_frameStamp - data->m_captureLastCrossedThresholdTime)
...
static void gviHardwarePlayPacket(GVDevice device,
                                  const GVByte* packet,
                                  int len,
                                  GVSource source,
...
static int gviHardwareGetNumChannels(GVDevice device, GVDeviceType type)
{
    ...
    result = AudioDeviceGetPropertyInfo(
...
static void gviHardwareGetChannelName(GVDevice device, GVDeviceType type, int index, gsi_char name[GV_CHANNEL_NAME_LEN])
{
    ...
    result = AudioDeviceGetProperty(
        device->m_deviceID, 0, (type & GV_CAPTURE) ? true : false, kAudioDevicePropertyStreams, &size, streamIDs);
    if (result == noErr) {
...
static GVBool gviHardwareInitCapture(GVIDevice* device)
{
    ...
    result = AudioDeviceGetProperty(
        device->m_deviceID, 0, true, kAudioDevicePropertyStreamFormat, &size, &data->m_captureStreamDescriptor);
    if (result != noErr)
        return GVFalse;
    ...
    AudioDeviceAddPropertyListener(
        device->m_deviceID, 0, true, kAudioDevicePropertyDeviceIsAlive, gviPropertyListener, device);
...
static GVBool gviHardwareInitPlayback(GVIDevice* device)
{
    ...
    size = sizeof(AudioStreamBasicDescription);
    result = AudioDeviceGetProperty(
        device->m_deviceID, 0, false, kAudioDevicePropertyStreamFormat, &size, &data->m_playbackStreamDescriptor);
    if (result != noErr) {
        gviFreeSourceList(data->m_playbackSources);
        return GVFalse;
    }
    ...
    result =
        AudioConverterSetProperty(data->m_playbackConverter, kAudioConverterPrimeMethod, sizeof(UInt32), &primeMethod);
    if (result != noErr) {
        AudioConverterDispose(data->m_playbackConverter);
        gviFreeSourceList(data->m_playbackSources);
        return GVFalse;
    ...
    // add property listener
    AudioDeviceAddPropertyListener(
        device->m_deviceID, 0, false, kAudioDevicePropertyDeviceIsAlive, gviPropertyListener, device);
