...
static void EEStreamClearSPUBuffer()
{
    ...
    sceSdRemote(1, rSdVoiceTrans, AUTODMA_CH, SD_TRANS_MODE_WRITE | SD_TRANS_BY_DMA, IOPBuffer, CORE_INPUT_L, 0x800);
    sceSdRemote(1,
                rSdVoiceTrans,
                AUTODMA_CH,
                SD_TRANS_MODE_WRITE | SD_TRANS_BY_DMA,
                IOPBuffer + IOP_BUFF_SIZE,
                CORE_INPUT_R,
                0x800);
...
static void EEStreamConvertAndTransfer(GVIDevice* device, int which)
{
    ...
    if (data->m_playbackBufferPos) {
        EEStreamDataPush(data->m_playbackBuffer + data->m_playbackBufferPos,
                         GVISamplesPerFrame - data->m_playbackBufferPos);
...
static void gviPS2Spu2PlayPacket(GVIDevice* device,
                                 const GVByte* packet,
                                 int len,
                                 GVSource source,
