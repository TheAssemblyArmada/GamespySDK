                              ...
                              GVFrameStamp currentPlayClock)
{
    ...
    if (!gviSource) {
        ...
        if ((clockOffset < gviSource->m_clockOffsetAverage)
            && gviIsFrameStampGT(clockOffset, (GVFrameStamp)gviSource->m_clockOffsetAverage)) {
            unwrappedClockOffset = (clockOffset + GVI_FRAMESTAMP_MAX);
        } else if ((gviSource->m_clockOffsetAverage < clockOffset)
        ...
        gviSource->m_clockOffsetAverage = (float)((gviSource->m_clockOffsetAverage * GVI_CLOCK_OFFSET_AVERAGING_FACTOR)
...
GVBool gviWriteSourcesToBuffer(GVISourceList sourceList, GVFrameStamp startTime, GVSample* sampleBuffer, int numFrames)
{
    GVISource* source;
    ...
    for (i = 0; i < GVI_MAX_SOURCES; i++) {
        ...
        while (source->m_frameHead.m_next) {
            ...
            if (gviIsFrameStampGTE(frame->m_frameStamp, startTime)) {
                ...
                GVSample* writePtr =
                    (sampleBuffer + ((GVFrameStamp)(frame->m_frameStamp - startTime) * GVISamplesPerFrame));
#if GVI_PRE_DECODE
                int j;
