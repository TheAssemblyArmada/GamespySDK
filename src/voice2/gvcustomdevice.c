...
static GVBool
gviCustomCapturePacket(GVDevice device, GVByte* packet, int* len, GVFrameStamp* frameStamp, GVScalar* volume)
{
    // not supported with custom devices
    assert(0);
    GSI_UNUSED(device);
...
static void gviCustomPlayPacket(GVIDevice* device,
                                const GVByte* packet,
                                int len,
                                GVSource source,
...
GVBool gviGetCustomPlaybackAudio(GVIDevice* device, GVSample* audio, int numSamples)
{
    ...
    // fill it
    wroteToBuffer = gviWriteSourcesToBuffer(data->m_playbackSources, data->m_playbackClock, audio, numFrames);
    ...
    if (device->m_playbackFilterCallback) {
        for (i = 0; i < numFrames; i++)
            device->m_playbackFilterCallback(
                device, audio + (GVISamplesPerFrame * i), (GVFrameStamp)(data->m_playbackClock + i));
    }
...
GVBool gviSetCustomCaptureAudio(GVDevice device,
        ...
        if (overThreshold) {
            ...
            for (i = 0; i < numFrames; i++) {
                ...
                if (device->m_captureFilterCallback)
                    device->m_captureFilterCallback(
                        device, (GVSample*)audio + (i * GVISamplesPerFrame), (GVFrameStamp)(data->m_captureClock + i));
