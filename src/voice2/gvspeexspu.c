...
GVBool gviSpeexInitialize(int quality, GVRate sampleRate)
{
    ...
    if (issueSampleTaskEncodeInit(
            quality, sampleRate, &gSpeexTaskOutput, gviSpeexEncoderStateBuffer, SPEEX_ENCODER_STATE_BUFFER_SIZE)
        != 0)
        return GVFalse;
...
void gviSpeexEncode(GVByte* out, const GVSample* in)
{
    ...
    immediateReturn = issueSampleTaskEncode((short*)in,
                                            gviSpeexSamplesPerFrame,
                                            gviSpeexEncodedFrameSize,
                                            (char*)gviSpeexEncodedBuffer,
                                            GVI_SPEEX_ENCODED_BUFFER,
                                            &gSpeexTaskOutput,
                                            gviSpeexEncoderStateBuffer,
...
void gviSpeexDecodeAdd(GVSample* out, const GVByte* in, GVDecoderData data)
{
    ...
    immediateReturn = issueSampleTaskDecodeAdd(data,
                                               SPEEX_DECODER_STATE_BUFFER_SIZE,
                                               gviSpeexEncodedBuffer,
                                               GVI_SPEEX_ENCODED_BUFFER,
                                               gviSpeexEncodedFrameSize,
                                               gviSpeexDecodedBuffer,
                                               gviSpeexSamplesPerFrame,
                                               &gSpeexTaskOutput);
    for (i = 0; i < gviSpeexSamplesPerFrame; i++)
...
void gviSpeexDecodeSet(GVSample* out, const GVByte* in, GVDecoderData data)
{
    ...
    memset(gviSpeexEncodedBuffer, 0, GVI_SPEEX_ENCODED_BUFFER);
    memcpy(gviSpeexEncodedBuffer, in, gviSpeexEncodedFrameSize);
    immediateReturn = issueSampleTaskDecodeSet(data,
                                               SPEEX_DECODER_STATE_BUFFER_SIZE,
                                               gviSpeexEncodedBuffer,
                                               GVI_SPEEX_ENCODED_BUFFER,
                                               gviSpeexEncodedFrameSize,
