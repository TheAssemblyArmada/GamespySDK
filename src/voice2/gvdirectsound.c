...
static GVBool gviPlaybackDeviceThink(GVIDevice* device)
{
    ...
    newBytes =
        (((playCursor + data->m_playbackBufferSize) - data->m_playbackBufferPosition) % data->m_playbackBufferSize);
    ...
    result = IDirectSoundBuffer_Lock(
        data->m_playbackBuffer, lockPosition, newBytes, &audioPtr1, &audioLen1, &audioPtr2, &audioLen2, 0);
    if (FAILED(result))
        return GVFalse;
    ...
    wroteToBuffer =
        gviWriteSourcesToBuffer(data->m_playbackSources, data->m_playbackClock, (GVSample*)audioPtr1, numFrames);
    if (!wroteToBuffer)
    ...
    if (device->m_playbackFilterCallback) {
        for (i = 0; i < numFrames; i++)
            device->m_playbackFilterCallback(
                device, (GVSample*)audioPtr1 + (GVISamplesPerFrame * i), (GVFrameStamp)(data->m_playbackClock + i));
    }
    ...
    if (audioPtr2) {
        ...
        wroteToBuffer =
            gviWriteSourcesToBuffer(data->m_playbackSources, data->m_playbackClock, (GVSample*)audioPtr2, numFrames);
        if (!wroteToBuffer)
        ...
        if (device->m_playbackFilterCallback) {
            for (i = 0; i < numFrames; i++)
                device->m_playbackFilterCallback(
                    device, (GVSample*)audioPtr2 + (GVISamplesPerFrame * i), (GVFrameStamp)(data->m_playbackClock + i));
        }
...
static GVBool gviStartPlaybackDevice(GVIHardwareData* data)
{
    ...
    // lock the buffer
    result = IDirectSoundBuffer_Lock(
        data->m_playbackBuffer, 0, 0, &audioPtr1, &audioLen1, &audioPtr2, &audioLen2, DSBLOCK_ENTIREBUFFER);
    if (FAILED(result))
        return GVFalse;
...
static int gviHardwareGetAvailableCaptureBytes(GVDevice device)
{
    ...
    // get the number of new bytes
    newBytes =
        (((readPosition + data->m_captureBufferSize) - data->m_captureBufferPosition) % data->m_captureBufferSize);
...
static GVBool
gviHardwareCapturePacket(GVDevice device, GVByte* packet, int* len, GVFrameStamp* frameStamp, GVScalar* volume)
{
    GVIHardwareData* data = (GVIHardwareData*)device->m_data;
    HRESULT result;
    DWORD readPosition;
    ...
    *len = 0;
    ...
    if (data->m_capturing) {
        // lock the buffer
        result = IDirectSoundCaptureBuffer_Lock(data->m_captureBuffer,
                                                data->m_captureBufferPosition,
                                                newBytes,
                                                &audioPtr1,
                                                &audioLen1,
                                                &audioPtr2,
        ...
        if (volume) {
            ...
            if (!overThreshold && audioPtr2)
                overThreshold =
                    gviIsOverThreshold(audioPtr2, audioLen2 / GV_BYTES_PER_SAMPLE, data->m_captureThreshold);
        }
        ...
        if (overThreshold) {
            ...
            overThreshold = ((GVFrameStamp)(data->m_captureClock - data->m_captureLastCrossedThresholdTime)
                             < GVI_HOLD_THRESHOLD_FRAMES);
        }
...
static void gviHardwarePlayPacket(GVIDevice* device,
                                  const GVByte* packet,
                                  int len,
                                  GVSource source,
...
static LPDIRECTPLAYVOICETEST GetVoiceTestInstance(void)
{
    ...
    result = CoCreateInstance(
        &CLSID_DirectPlayVoiceTest, NULL, CLSCTX_INPROC_SERVER, &IID_IDirectPlayVoiceTest, (LPVOID*)&voiceTest);
...
GVBool gvAreDevicesSetup(GVDeviceID captureDevice, GVDeviceID playbackDevice)
{
    ...
    // run the dialog
    result =
        IDirectPlayVoiceTest_CheckAudioSetup(voiceTest, &playbackDevice, &captureDevice, GVIHwnd, DVFLAGS_QUERYONLY);
