...
static GVBool Initialize(const char* remoteIP)
{
    ...
    printf(
        "SamplesPerFrame=%d EncodedFrameSize=%d BitsPerSecond=%d\n", SamplesPerFrame, EncodedFrameSize, BitsPerSecond);
...
static GVBool PSPStartup(void)
{
    ...
    PSPPlaybackThreadID =
        sceKernelCreateThread("playback", PSPPlaybackThread, SCE_KERNEL_USER_HIGHEST_PRIORITY, 4096, 0, NULL);
