...
static GVBool
gviPS2EyetoyCapturePacket(GVDevice device, GVByte* packet, int* len, GVFrameStamp* frameStamp, GVScalar* volume)
{
    GVIPS2EyetoyData* data = (GVIPS2EyetoyData*)device->m_data;
    ...
    *frameStamp = data->m_captureClock;
    ...
    if (numFrames) {
        ...
        if (data->m_captureBufferBytes)
            memmove(data->m_captureBuffer,
                    data->m_captureBuffer + (numFrames * GVIBytesPerFrame),
                    (unsigned int)data->m_captureBufferBytes);
    }
