...
static int gviPSPCaptureThread(SceSize args, void* argp)
{
    ...
    while (!data->m_captureThreadStop) {
        ...
        if (data->m_capturing) {
            // get some input
            rcode = sceAudioInputBlocking(
                GVI_INPUT_LEN, GV_SAMPLES_PER_SECOND, data->m_captureBuffer + data->m_captureBufferWritePos);
            if (rcode < 0) {
...
static GVBool gviPSPAudioInitDevice(GVIDevice* device)
{
    ...
    data->m_captureThreadID = sceKernelCreateThread(
        "capture", gviPSPCaptureThread, SCE_KERNEL_USER_HIGHEST_PRIORITY, GVI_CAPTURE_THREAD_STACK_SIZE, 0, NULL);
