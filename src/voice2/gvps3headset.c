...
static GVBool gviPS3HeadsetInitHeadphone(GVIPS3HeadsetData* data)
{
    ...
    while (aCount < 10) {
        result = sys_event_queue_create(
            &data->m_playbackQueue, &aQueueAttr, data->m_playbackQueueKey, GVI_AUDIO_QUEUE_DEPTH);
        if (result == CELL_OK) {
            break;
        }
        // search unused key
...
GVBool gviPS3HeadsetInitMic(GVIPS3HeadsetData* data)
{
    ...
    while ((aCount++) < 100) {
        result = sys_event_queue_create(&data->m_captureCallbackQueue, &equeue_attr, data->m_captureEventQueueKey, 32);
        if (result == CELL_OK)
            break;
    ...
    result = sys_event_queue_receive(data->m_captureCallbackQueue, &data->m_captureCallbackEvent, 10000);
    if (result == ETIMEDOUT)
        return GVFalse;
    ...
    if (aMsg == CELLMIC_ATTACH && aDevNum == data->m_deviceNum) {
        ...
        result = cellMicOpenEx(data->m_deviceNum,
                               GVI_PLAYBACK_SAMPLE_RATE,
                               1,
                               GV_SAMPLES_PER_SECOND,
                               GVI_PS3_MIC_BUFFER_MS,
                               CELLMIC_SIGTYPE_DSP);
...
static void
gviProcessCapturedFrame(GVDevice device, GVSample* frameIn, GVByte* frameOut, GVScalar* volume, GVBool* threshold)
{
    GVIPS3HeadsetData* data = (GVIPS3HeadsetData*)device->m_data;
...
// captures a packet
static GVBool
gviPS3HeadsetCapturePacket(GVDevice device, GVByte* packet, int* len, GVFrameStamp* frameStamp, GVScalar* volume)
{
    GVIPS3HeadsetData* data = (GVIPS3HeadsetData*)device->m_data;
    ...
    *frameStamp = data->m_captureClock;
    ...
    for (numFrames = 0; numFrames < framesAvailable; numFrames++) {
        ...
        result = sys_event_queue_receive(data->m_captureCallbackQueue, &data->m_captureCallbackEvent, 1);
        if (result == ETIMEDOUT) {
            break;
        }
    ...
    if (overThreshold) {
        ...
        overThreshold =
            ((GVFrameStamp)(*frameStamp - data->m_captureLastCrossedThresholdTime) < GVI_HOLD_THRESHOLD_FRAMES);
    }
...
static void gviPS3HeadsetPlayPacket(GVIDevice* device,
                                    const GVByte* packet,
                                    int len,
                                    GVSource source,
