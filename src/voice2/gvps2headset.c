...
static void
gviProcessCapturedFrame(GVDevice device, GVByte* frameOut, GVSample* frameIn, GVScalar* volume, GVBool* threshold)
{
    GVIPS2HeadsetData* data = (GVIPS2HeadsetData*)device->m_data;
...
static GVBool
gviPS2HeadsetCapturePacket(GVDevice device, GVByte* packet, int* len, GVFrameStamp* frameStamp, GVScalar* volume)
{
    GVIPS2HeadsetData* data = (GVIPS2HeadsetData*)device->m_data;
    ...
    *frameStamp = data->m_captureClock;
    ...
    for (numFrames = 0; numFrames < framesAvailable; numFrames++) {
        ...
        result = lgAudRead(data->m_handle,
                           LGAUD_BLOCKMODE_NOT_BLOCKING,
                           ((u_char*)data->m_captureBuffer) + data->m_captureBufferBytes,
                           &readSize);
        if (LGAUD_FAILED(result)) {
            gviDeviceUnplugged(device);
            return GVFalse;
...
static void gviPS2HeadsetPlayPacket(GVIDevice* device,
                                    const GVByte* packet,
                                    int len,
                                    GVSource source,
