...
////////////////////////////////////////////////////////////////
static int ghiFindFreeSlot(void)
{
    int i;
...
//handle used for asynch DNS lookups
    ...
    if (bResult)
        bResult = ghiInitBuffer(
            connection, &connection->encodeBuffer, ENCODE_BUFFER_INITIAL_SIZE, ENCODE_BUFFER_INCREMENT_SIZE);
    if (bResult)
        bResult =
            ghiInitBuffer(connection, &connection->recvBuffer, RECV_BUFFER_INITIAL_SIZE, RECV_BUFFER_INCREMENT_SIZE);
    if (bResult)
        bResult = ghiInitBuffer(
            connection, &connection->decodeBuffer, DECODE_BUFFER_INITIAL_SIZE, DECODE_BUFFER_INCREMENT_SIZE);
