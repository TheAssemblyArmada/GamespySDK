...
static gsi_char* stateStrings[] = {_T("Socket Init"),
                                   _T("Host Lookup"),
                                   _T("Lookup Pending"),
                                   _T("Connecting"),
                                   _T("Securing Session"),
...
static gsi_char* resultStrings[] = {_T("GHTTPSuccess"),
                                    _T("GHTTPOutOfMemory"),
                          ...
                          va_list theParamList)
{
...
#if defined(_WIN32) && !defined(_XBOX)
    {
        ...
        sprintf(buffer, "[%s][%s] ", gGSIDebugCatStrings[theCat], gGSIDebugTypeStrings[theType]);
...
static GHTTPBool
CompletedCallback(GHTTPRequest request, GHTTPResult result, char* buffer, GHTTPByteCount bufferLen, void* param)
{
...
static void ProgressCallback(GHTTPRequest request,
                             GHTTPState state,
                             const char* buffer,
                             ...
                             void* param)
{
    ...
    if (state == GHTTPReceivingFile) {
        ...
        //////////////////////////////////////////////
        if (totalSize != -1)
            _tprintf(_T(" (%" PRI_BYTECOUNT " / %" PRI_BYTECOUNT " bytes)\n"), bytesReceived, totalSize);
        else
            _tprintf(_T(" (%" PRI_BYTECOUNT "bytes)\n"), bytesReceived);
    } else
        _tprintf(_T("\n"));
...
int test_main(int argc, char** argv)
{
    ...
    // get a file
    request = ghttpGet(_T("http://www.gamespy.net/images/dev_serv_main.jpg"),
                       GHTTPFalse,
                       CompletedCallback,
                       (void*)(uintptr_t)pendingRequests);
    CheckRequest(request, pendingRequests);
    pendingRequests++;

    // put a file into our own buffer, with progress updates
    // also gets redirected
    request = ghttpGetEx(_T("http://google.com/intl/en_ALL/images/logo.gif"),
                         NULL,
                         buffer,
                         sizeof(buffer),
                         NULL,
    ...
    // download a file
#if !defined(NOFILE)
    request = ghttpSave(_T("http://www.gamespy.net/images/dev_serv_logo.jpg"),
                        _T("logo.jpg"),
                        GHTTPFalse,
                        CompletedCallback,
                        (void*)(uintptr_t)pendingRequests);
    ...
    // get a header
    request = ghttpHead(_T("http://sdkdev.gamespy.com/games/st_ladder/web/index.html"),
                        GHTTPFalse,
                        CompletedCallback,
                        (void*)(uintptr_t)pendingRequests);
    CheckRequest(request, pendingRequests);
