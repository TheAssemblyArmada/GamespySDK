...
GHTTPBool ghttpSetRequestEncryptionEngine(GHTTPRequest request, GHTTPEncryptionEngine engine)
{
    ...
    if (connection->encryptor.mInterface != NULL && connection->encryptor.mEngine != engine) {
        return GHTTPFalse;
    }
...
int verify_callback(int preverify, X509_STORE_CTX* x509_ctx)
{
    ...
    if (preverify == 0) {
        if (err == X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY)
            ...
            gsDebugFormat(
                GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_WarmError, "  Error = X509_V_ERR_CERT_UNTRUSTED\n");
            ...
            gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_WarmError, "  Error = X509_V_OK\n");
        else
            gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_WarmError, "  Error = %d\n", err);
    }
...
// Init the engine
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    gsDebugFormat(
        GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_Debug, "GameSpy SSL (OpenSSL engine) initialized\r\n");

    return GHIEncryptionResult_Success;
}

// Destroy the engine
GHIEncryptionResult ghiEncryptorSslCleanupFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    gsOpenSSLInterface* sslInterface = NULL;
...
GHIEncryptionResult ghiEncryptorSslStartFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
                                               ...
                                               int* theEncryptedLength)
{
    ...
    encodeResult = matrixSslEncode(
        connection->encryptor.mInterface, (unsigned char*)thePlainTextBuffer, *thePlainTextLength, &encryptedBuf);
                                               ...
                                               int* theDecryptedLength)
{
    ...
    while (decryptMore != GHTTPFalse && ((inBuf.end - inBuf.start) > 0)) {
        ...
        decodeResult =
            matrixSslDecode(theEncryptor->mInterface, &inBuf, &decryptedBuf, &error, &alertlevel, &alertdescription);
        switch (decodeResult) {
        case SSL_SUCCESS:
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
...

    {
        ...
        verifyOption = SSL_VERIFY_COMMON_NAME | SSL_VERIFY_ROOT_CA | SSL_VERIFY_DATE | SSL_VERIFY_CHAIN
...
GHIEncryptionResult ghiEncryptorSslCleanupFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    gsDebugFormat(
        GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_Debug, "GameSpy SSL (RevoEx) engine cleanup called\r\n");
...
ghiEncryptorProcessSSLHandshake(struct GHIConnection* connection, struct GHIEncryptor* encryptor, GHIBuffer* data);
...
static void ghiEncryptorGenerateEncryptionKeys(gsSSL* sslInterface)
{
    ...
    RC4Init(
        &sslInterface->sendRC4, (const unsigned char*)sslInterface->clientWriteKey, sslInterface->clientWriteKeyLen);
    RC4Init(&sslInterface->recvRC4, (const unsigned char*)sslInterface->clientReadKey, sslInterface->clientReadKeyLen);

    gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_Debug, "Generated SSL session keys\r\n");
}
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_Debug, "GameSpy SSL engine initialized\r\n");
...
GHIEncryptionResult ghiEncryptorSslStartFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    ghiEncryptorWriteNBOLength(
        helloMsg.header.lengthNBO, sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg), 2);
...
#pragma message("!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
    ...
    if (GHTTPFalse
        == ghiAppendDataToBuffer(&connection->sendBuffer, (const char*)&helloMsg, sizeof(gsSSLClientHelloMsg))) {
        // assert or just return?
        return GHIEncryptionResult_BufferTooSmall;
    }

    gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_Debug, "GameSpy SSL engine handshake started\r\n");
    ...
    if (theEncryptor != NULL) {
        ...
        if (sslInterface == NULL || theEncryptor->mSessionEstablished == GHTTPFalse) {
            ...
            *theEncryptedLength += thePlainTextLength; // number of bytes written
            ...
            MD5Update(
                &md5, (unsigned char*)thePlainTextBuffer, (unsigned int)thePlainTextLength); // **cast-away const**
                                               ...
                                               int* theDecryptedLength)
{
    ...
    while (readPos < *theEncryptedLength) {
        ...
        case GS_SSL_CONTENT_HANDSHAKE: {
            ...
            if (connection->encryptor.mSessionEstablished)
                RC4Encrypt(&sslInterface->recvRC4,
                           ...
                           (unsigned char*)&theEncryptedBuffer[readPos],
                           length);

            ghiInitReadOnlyBuffer(connection, &data, &theEncryptedBuffer[readPos], length);

            gsDebugFormat(
                GSIDebugCat_HTTP, GSIDebugType_Misc, GSIDebugLevel_Debug, "SSL handshake message received\r\n");
        ...
        case GS_SSL_CONTENT_APPLICATIONDATA: {
            ...
            memcpy(
                theDecryptedBuffer + writePos, &theEncryptedBuffer[readPos], (size_t)(length - GS_CRYPT_MD5_HASHSIZE));
        ...
        case GS_SSL_CONTENT_ALERT:
            ...
            gsDebugFormat(
                GSIDebugCat_HTTP, GSIDebugType_Network, GSIDebugLevel_WarmError, "SSL received unhandled ALERT\r\n");
            // server alert
            break;
...
ghiEncryptorProcessSSLHandshake(struct GHIConnection* connection, struct GHIEncryptor* encryptor, GHIBuffer* data)
{
    ...
    while (data->pos < data->len) {
        ...
        if (messageType == GS_SSL_HANDSHAKE_SERVERHELLO) {
            ...
            MD5Update(
                &sslInterface->finishHashMD5, (unsigned char*)&data->data[messageStart], (unsigned int)totalMsgLen);
            SHA1Input(
                &sslInterface->finishHashSHA1, (unsigned char*)&data->data[messageStart], (unsigned int)totalMsgLen);
            ...
            while (data->pos < certListEndPos) {
                ...
                if (certCount == 1) {
                    ...
                    sslInterface->serverpub.modulus.mLength =
                        (unsigned int)((temp - 1)
                                       / GS_LARGEINT_DIGIT_SIZE_BYTES); //-1 to subtract the previous 0x00 byte
                    gsLargeIntSetFromMemoryStream(
                        &sslInterface->serverpub.modulus, (const gsi_u8*)&data->data[data->pos], (gsi_u32)temp - 1);
                    data->pos += temp - 1;
                    ...
                    sslInterface->serverpub.exponent.mLength =
                        (unsigned int)(((temp - 1) / GS_LARGEINT_DIGIT_SIZE_BYTES) + 1); // ceiling of temp/4
                    gsLargeIntSetFromMemoryStream(
                        &sslInterface->serverpub.exponent, (const gsi_u8*)&data->data[data->pos], (gsi_u32)temp);
                    data->pos += temp;
                }
...
#pragma message("!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
            ...
            ghiEncryptorWriteNBOLength(
                clientKeyExchange->header.lengthNBO,
                (int)(sslInterface->serverpub.modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES + 4),
                2);
            clientKeyExchange->handshakeType = GS_SSL_HANDSHAKE_CLIENTKEYEXCHANGE;
            ghiEncryptorWriteNBOLength(clientKeyExchange->lengthNBO,
                                       (int)(sslInterface->serverpub.modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES),
                                       3);
            //    encrypt the preMasterSecret using the server's public key (store result in sendbuffer)
            gsCryptRSAEncryptBuffer(&sslInterface->serverpub,
                                    sslInterface->premastersecret,
                                    GS_SSL_MASTERSECRET_LEN,
                                    (unsigned char*)&connection->sendBuffer.data[connection->sendBuffer.len]);
            connection->sendBuffer.len += sslInterface->serverpub.modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES;

            // add it to the running handshake hash
            MD5Update(&sslInterface->finishHashMD5,
                      (unsigned char*)clientKeyExchange + sizeof(gsSSLRecordHeaderMsg),
            ...
            MD5Update(
                &sslInterface->finishHashMD5, (unsigned char*)sslInterface->mastersecret, GS_SSL_MASTERSECRET_LEN);
            ...
            SHA1Input(
                &sslInterface->finishHashSHA1, (unsigned char*)sslInterface->mastersecret, GS_SSL_MASTERSECRET_LEN);
