...
static void piQRServerKeyCallback(int key, qr2_buffer_t buffer, PEER peer)
{
    PEER_CONNECTION;

    // check if we should report it
    if (connection->inRoom[StagingRoom]
        && (!connection->playing || (connection->reportingOptions & PEER_REPORT_INFO))) {
...
static void piQRPlayerKeyCallback(int key, int index, qr2_buffer_t buffer, PEER peer)
{
    piPlayer* player;
    PEER_CONNECTION;
    ...
    // check if we should report it
    if (connection->inRoom[StagingRoom]
        && (!connection->playing || (connection->reportingOptions & PEER_REPORT_PLAYERS))) {
...
static void piQRTeamKeyCallback(int key, int index, qr2_buffer_t buffer, PEER peer)
{
    PEER_CONNECTION;
...
static void piQRKeyListCallback(qr2_key_type type, qr2_keybuffer_t keyBuffer, PEER peer)
{
    PEER_CONNECTION;
...
static int piQRCountCallback(qr2_key_type type, PEER peer)
{
    PEER_CONNECTION;
...
static void piQRAddErrorCallbackA(qr2_error_t error, char* errorString, PEER peer)
{
    PEER_CONNECTION;
...
#ifdef GSI_UNICODE
static void piQRAddErrorCallbackW(qr2_error_t error, unsigned short* errorString, PEER peer)
{
...
static void piQRNatNegotiateCallback(int cookie, PEER peer)
{
    PEER_CONNECTION;
...
static void piQRPublicAddressCallback(unsigned int ip, unsigned short port, PEER peer)
{
    PEER_CONNECTION;
...
PEERBool piStartReporting(PEER peer, SOCKET socket, unsigned short port)
{
    ...
    if (socket == INVALID_SOCKET) {
        rcode = qr2_initA(&connection->queryReporting,
                          NULL,
                          PI_QUERYPORT,
                          connection->title,
                          connection->qrSecretKey,
                          1,
                          connection->natNegotiate,
        ...
        rcode = qr2_init_socketA(&connection->queryReporting,
                                 socket,
                                 port,
                                 connection->title,
                                 connection->qrSecretKey,
...
PEERBool piStartAutoMatchReporting(PEER peer)
{
    ...
    if (operation->socket == INVALID_SOCKET) {
        rcode = qr2_initA(&connection->autoMatchReporting,
                          NULL,
                          PI_QUERYPORT,
                          autoMatchTitle,
                          connection->qrSecretKey,
        ...
        rcode = qr2_init_socketA(&connection->autoMatchReporting,
                                 operation->socket,
                                 operation->port,
                                 autoMatchTitle,
                                 connection->qrSecretKey,
