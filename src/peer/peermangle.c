...
static const char* piStagingRoomHash(unsigned int publicIP, unsigned int privateIP, unsigned short port, char* buffer)
{
    ...
    result = (((privateIP >> 24) & 0xFF) | ((privateIP >> 8) & 0xFF00) | ((privateIP << 8) & 0xFF0000)
              | ((privateIP << 24) & 0xFF000000));
    result ^= publicIP;
    result ^= (port | (port << 16));
...
void piMangleTitleRoom(char buffer[PI_ROOM_MAX_LEN], const char* title)
{
    ...
    sprintf(buffer, "#GSP" PI_SEPERATOR "%s", title);
}
...
void piMangleStagingRoom(char buffer[PI_ROOM_MAX_LEN],
                         const char* title,
...
void piMangleUser(char buffer[PI_USER_MAX_LEN], unsigned int IP, int profileID)
{
    ...
    sprintf(buffer, "X%sX|%d", EncodeIP(IP, NULL, PEERFalse), profileID);
}
