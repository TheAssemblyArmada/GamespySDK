...
**********/
static void
piSBGamesListCallback(SBServerListPtr serverlist, SBListCallbackReason reason, SBServer server, void* instance)
{
...
static void
piSBGamesEngineCallback(SBQueryEnginePtr engine, SBQueryEngineCallbackReason reason, SBServer server, void* instance)
{
    PEER peer = (PEER)instance;
    //PEER_CONNECTION;
...
PEERBool piSBStartListingGames(PEER peer, const unsigned char* fields, int numFields, const char* filter)
{
    ...
    ///////////////////////////
    if (SBServerListConnectAndQuery(&connection->gameList, fullFields, smartSpyFilter, PUSH_UPDATES, 0)
        != sbe_noerror) {
        piSBStopListingGames(peer);
        return PEERFalse;
    }
...
***********/
static void
piSBGroupsListCallback(SBServerListPtr serverlist, SBListCallbackReason reason, SBServer server, void* instance)
{
    ...
    case slc_serveradded: {
        ...
        //#endif
        piAddListGroupRoomsCallback(peer,
                                    PEERTrue,
                                    groupID,
                                    server,
                                    name,
...
static void
piSBAutoMatchListCallback(SBServerListPtr serverlist, SBListCallbackReason reason, SBServer server, void* instance)
{
    PEER peer = (PEER)instance;
    PEER_CONNECTION;
    ...
    case slc_serveradded:
        ...
        if (!SBServerDirectConnect(server))
            SBGetServerRulesFromMaster(&connection->autoMatchList,
    ...
    case slc_serverupdated:
        ...
        if (!SBServerHasFullKeys(server) && SBServerDirectConnect(server)
    ...
    case slc_initiallistcomplete:
        ...
        if (!SBServerListCount(&connection->autoMatchList) || (0 == connection->autoMatchEngine.querylist.count)) {
            piSetAutoMatchStatus(peer, PEERWaiting);
        }
        break;
    case slc_queryerror:
...
static void piSBAutoMatchEngineCallback(SBQueryEnginePtr engine,
...
PEERBool piSBStartListingAutoMatches(PEER peer)
{
    ...
    // start updating the list.
    if (SBServerListConnectAndQuery(&connection->autoMatchList, NULL, connection->autoMatchFilter, PUSH_UPDATES, 0)
        != sbe_noerror) {
        piSBStopListingAutoMatches(peer);
        connection->autoMatchSBFailed = PEERTrue;
        return PEERFalse;
...
PEERBool piSBInit(PEER peer)
{
    ...
    SBServerListInit(&connection->gameList,
                     ...
                     connection->sbGameVersion,
                     SBFalse,
                     piSBGamesListCallback,
                     peer);
    SBQueryEngineInit(
        &connection->gameEngine, connection->sbMaxUpdates, piSBQueryVersion, SBFalse, piSBGamesEngineCallback, peer);

    // init the group list
    memset(&connection->groupList, 0, sizeof(connection->groupList));
    SBServerListInit(&connection->groupList,
                     connection->title,
                     connection->sbName,
                     connection->sbSecretKey,
                     connection->sbGameVersion,
                     SBFalse,
                     piSBGroupsListCallback,
                     peer);
    ...
    SBServerListInit(&connection->autoMatchList,
                     autoMatchTitle,
                     connection->sbName,
                     connection->sbSecretKey,
                     connection->sbGameVersion,
                     SBFalse,
                     piSBAutoMatchListCallback,
                     peer);
    SBQueryEngineInit(&connection->autoMatchEngine,
                      connection->sbMaxUpdates,
                      piSBQueryVersion,
                      SBFalse,
                      piSBAutoMatchEngineCallback,
                      peer);
...
void piSBUpdateGame(PEER peer, SBServer server, PEERBool fullUpdate, PEERBool forceUpdateByMaster, PEERBool icmpEcho)
{
    ...
    if (server->flags & UNSOLICITED_UDP_FLAG) {
        ...
        SBQueryEngineUpdateServer(
            &connection->gameEngine, server, 1, fullUpdate ? QTYPE_FULL : QTYPE_BASIC, usequerychallenge);
    } else {
