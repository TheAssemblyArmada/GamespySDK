...
static int WatchKeysHash(const void* elem, int numBuckets)
{
...
static int GS_STATIC_CALLBACK WatchKeysCompare(const void* elem1, const void* elem2)
{
...
static void WatchKeysFree(void* elem)
{
    piWatchKey* key = (piWatchKey*)elem;
...
static int WatchCacheHash(const void* elem, int numBuckets)
{
...
static int GS_STATIC_CALLBACK WatchCacheCompare(const void* elem1, const void* elem2)
{
    int rcode;
...
static void WatchCacheFree(void* elem)
{
    piCacheKey* key = (piCacheKey*)elem;
...
PEERBool piKeysInit(PEER peer)
{
    ...
    /////////////////////////
    connection->globalWatchCache =
        TableNew(sizeof(piCacheKey), PI_WATCH_CACHE_NUM_BUCKETS, WatchCacheHash, WatchCacheCompare, WatchCacheFree);
    if (!connection->globalWatchCache)
        return PEERFalse;
    for (roomType = 0; roomType < NumRooms; roomType++) {
        connection->globalWatchKeys[roomType] =
            TableNew(sizeof(piWatchKey), PI_WATCH_KEYS_NUM_BUCKETS, WatchKeysHash, WatchKeysCompare, WatchKeysFree);
...
static const char* piGetWatchKeyA(const char* nick, const char* key, HashTable watchCache)
{
    piCacheKey keyTemp;
    piCacheKey* cacheKey;
...
static const unsigned short* piGetWatchKeyW(const unsigned short* nick, const unsigned short* key, HashTable watchCache)
{
#ifndef GSI_UNICODE
    GSI_UNUSED(nick);
    GSI_UNUSED(key);
...
piGetRoomWatchKeyW(PEER peer, RoomType roomType, const unsigned short* nick, const unsigned short* key)
{
    PEER_CONNECTION;
...
static void piCleanseGlobalCacheMap(void* elem, void* clientData)
{
    piPlayer* player;
    int roomType;
...
static void piCleanseRoomCacheMap(void* elem, void* clientData)
{
    piPlayer* player;
    piCacheKey* cacheKey = (piCacheKey*)elem;
...
static void piGetRoomKeysCallbackA(CHAT chat,
                                   CHATBool success,
                                   const char* channel,
                                   const char* user,
                                   int num,
...
#ifdef GSI_UNICODE
static void piGetRoomKeysCallbackW(CHAT chat,
                                   CHATBool success,
...
static void piRemoveExistingKeysMap(void* elem, void* clientData)
{
    int i;
...
static PEERBool piKeyChanged(PEER peer,
                             const char* nick,
                             const char* key,
                             const char* value,
                             HashTable watchKeys,
...
void piGlobalKeyChanged(PEER peer, const char* nick, const char* key, const char* value)
{
    ...
    if (piKeyChanged(peer,
                     nick,
                     key,
                     value,
                     connection->globalWatchKeys[TitleRoom],
                     connection->globalWatchCache,
                     PEERFalse,
                     (RoomType)0)
        || piKeyChanged(peer,
                        ...
                        key,
                        value,
                        connection->globalWatchKeys[GroupRoom],
                        connection->globalWatchCache,
                        PEERFalse,
                        (RoomType)0)
        || piKeyChanged(peer,
                        ...
                        key,
                        value,
                        connection->globalWatchKeys[StagingRoom],
                        connection->globalWatchCache,
                        PEERFalse,
                        (RoomType)0))
...
static void piPlayerChangedNickMap(void* elem, void* clientData)
{
    piCacheKey* key = (piCacheKey*)elem;
...
static void piSetupKeysMap(void* elem, void* clientData)
{
    piWatchKey* key = (piWatchKey*)elem;
...
static void piKeyCacheRefresh(PEER peer, RoomType roomType, const char* nick)
{
    ...
    if (!nick) {
        ...
        if (num) {
            ...
            assert(data.next == num);
            chatGetChannelKeysA(connection->chat,
                                ROOM,
                                nick ? nick : "*",
                                num,
                                (const char**)data.keys,
                                piGetRoomKeysCallback,
                                peer,
                                CHATFalse);
            gsifree(data.keys);
        }
