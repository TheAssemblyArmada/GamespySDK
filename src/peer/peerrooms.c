...
void piSetLocalFlags(PEER peer)
{
    ...
    if (titleRoom) {
        ...
        if (nFlags != connection->oldFlags[TitleRoom]) {
            chatSetChannelKeysA(
                connection->chat, connection->rooms[TitleRoom], connection->nick, 1, &key, (const char**)&titleRoom);
    ...
    if (groupRoom) {
        ...
        if (nFlags != connection->oldFlags[GroupRoom]) {
            chatSetChannelKeysA(
                connection->chat, connection->rooms[GroupRoom], connection->nick, 1, &key, (const char**)&groupRoom);
    ...
    if (stagingRoom) {
        ...
        if (nFlags != connection->oldFlags[StagingRoom]) {
            chatSetChannelKeysA(connection->chat,
                                connection->rooms[StagingRoom],
                                connection->nick,
                                1,
                                &key,
                                (const char**)&stagingRoom);
