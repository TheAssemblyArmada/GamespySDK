...
** FUNCTIONS **
**************/
static int piXpingTableHashFn(const void* param, int numBuckets)
{
...
static int GS_STATIC_CALLBACK piXpingTableCompareFn(const void* param1, const void* param2)
{
...
static void piXpingTableElementFreeFn(void* param)
{
    piXping* xping = (piXping*)param;
    assert(xping);
    assert(xping->nicks[0][0]);
...
static void piPinged(unsigned int IP, unsigned short port, int ping, const char* data, int len, PEER peer)
{
    piPlayer* player;
...
PEERBool piPingInit(PEER peer)
{
    ...
    ////////////////////////
    connection->xpings = TableNew(
        sizeof(piXping), PI_XPING_NUM_BUCKETS, piXpingTableHashFn, piXpingTableCompareFn, piXpingTableElementFreeFn);
...
static void
piPingerReply(unsigned int IP, unsigned short port, int ping, const char* pingData, int pingDataLen, PEER peer)
{
...
static void piPickPingPlayersMap(void* elem, void* clientData)
{
    ...
    for (i = (data->max - 1); i >= 0; i--) {
        ...
        if ((!other->numPings && player->numPings) ||                       // Not pinged.
            ...
            (piIsPlayerVIP(other, StagingRoom) && !piIsPlayerVIP(player, StagingRoom))
            ||                                           // Op or voice in the same staging room.
...
static void piPickXpingPlayerMap(void* elem, void* clientData)
{
    piPlayer* player = (piPlayer*)elem;
...
static void piPingPlayerLeftRoomTableMapFn(void* elem, void* clientdata)
{
