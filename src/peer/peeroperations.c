...
static piOperation*
piAddOperation(PEER peer, piOperationType type, void* data, PEERCBType callback, void* callbackParam, int opID)
{
    piOperation* operation;
    piOperationContainer container;
...
/* Connect.
**********/
static void piConnectConnectCallback(CHAT chat, CHATBool success, int failureReason, void* param)
{
    PEER_CONNECTION_OP;
    ...
    // Add the callback.
    ////////////////////
    piAddConnectCallback(peer,
                         (PEERBool)success,
                         failureReason,
                         (peerConnectCallback)operation->callback,
...
static void piConnectNickErrorCallbackA(CHAT chat,
                                        int type,
                                        const char* nick,
                                        ...
                                        void* param)
{
    ...
    /////////////////////////
    piAddNickErrorCallback(
        peer, type, nick, numSuggestedNicks, suggestedNicks, operation->callbackParam, operation->ID);

    GSI_UNUSED(chat);
}
#ifdef GSI_UNICODE
static void piConnectNickErrorCallbackW(CHAT chat,
                                        int type,
...
static void piConnectFillInUserCallbackA(CHAT chat, unsigned int IP, char user[128], void* param)
{
    int pid;
...
#ifdef GSI_UNICODE
static void piConnectFillInUserCallbackW(CHAT chat,
                                         unsigned int IP,
...
PEERBool piNewConnectOperation(PEER peer,
                               ...
                               int opID)
{
    ...
    if (connectType == PI_CONNECT) {
        connection->chat = chatConnectSecureA(PI_CHAT_SERVER_ADDRESS,
                                              PI_CHAT_SERVER_PORT,
                                              nick,
                                              encodedUniqueID,
                                              connection->sbName,
        ...
        connection->chat = chatConnectLoginA(PI_CHAT_SERVER_ADDRESS,
                                             PI_CHAT_SERVER_PORT,
                                             namespaceID,
                                             email,
                                             profilenick,
        ...
        connection->chat = chatConnectPreAuthA(PI_CHAT_SERVER_ADDRESS,
                                               PI_CHAT_SERVER_PORT,
                                               authtoken,
                                               partnerchallenge,
                                               encodedUniqueID,
...
/* Create Staging Room.
**********************/
static PEERJoinResult piEnterResultToJoinResult(CHATEnterResult result)
{
    switch (result) {
...
static void piCreateStagingRoomEnumUsersCallbackA(CHAT chat,
                                                  CHATBool success,
                                                  const char* channel,
                                                  int numUsers,
                                                  const char** users,
                                                  ...
                                                  void* param)
{
    ...
    // Add the callback.
    ////////////////////
    piAddJoinRoomCallback(peer,
                          (PEERBool)success,
                          success ? PEERJoinSuccess : PEERJoinFailed,
                          StagingRoom,
                          (peerJoinRoomCallback)operation->callback,
                          operation->callbackParam,
                          operation->ID);
...
#ifdef GSI_UNICODE
static void piCreateStagingRoomEnumUsersCallbackW(CHAT chat,
                                                  CHATBool success,
...
static void piCreateStagingRoomGetChannelModeCallback(CHAT chat,
                                                      CHATBool success,
...
static void piCreateStagingRoomEnterChannelCallbackA(CHAT chat,
                                                     CHATBool success,
                                                     ...
                                                     void* param)
{
    ...
    if (success) {
        ...
        chatGetChannelMode(connection->chat,
                           peerGetRoomChannel(peer, StagingRoom),
                           piCreateStagingRoomGetChannelModeCallback,
                           (void*)connection,
                           CHATFalse);
...
#ifdef GSI_UNICODE
static void piCreateStagingRoomEnterChannelCallbackW(CHAT chat,
                                                     CHATBool success,
                                                     CHATEnterResult result,
                                                     const unsigned short* channel,
                                                     void* param)
...
PEERBool piNewCreateStagingRoomOperation(PEER peer,
                                         const char* name,
                                         const char* password,
                                         int maxPlayers,
                                         SOCKET socket,
                                         ...
                                         int opID)
{
    ...
    if (socket == INVALID_SOCKET) {
        ...
        addr.s_addr =
            0; //crt -- don't bind to privateIP (for clients that have both public and private)  connection->privateIP;
        if (port)
            privatePort = port;
        else
    ...
    chatEnterChannelA(
        connection->chat, room, NULL, &channelCallbacks, piCreateStagingRoomEnterChannelCallback, operation, CHATFalse);
...
/* Join Room.
************/
static void piJoinRoomEnumUsersCallbackA(CHAT chat,
                                         CHATBool success,
                                         const char* channel,
                                         int numUsers,
                                         const char** users,
...
#ifdef GSI_UNICODE
static void piJoinRoomEnumUsersCallbackW(CHAT chat,
                                         CHATBool success,
                                         const unsigned short* channel,
                                         int numUsers,
                                         const unsigned short** users,
...
static void
piJoinRoomEnterChannelCallbackA(CHAT chat, CHATBool success, CHATEnterResult result, const char* channel, void* param)
{
    PEER_CONNECTION_OP;
...
#ifdef GSI_UNICODE
static void piJoinRoomEnterChannelCallbackW(CHAT chat,
                                            CHATBool success,
                                            CHATEnterResult result,
                                            const unsigned short* channel,
                                            void* param)
...
PEERBool piNewJoinRoomOperation(PEER peer,
                                RoomType roomType,
                                const char* channel,
                                const char* password,
                                peerJoinRoomCallback callback,
                                ...
                                int opID)
{
    ...
    chatEnterChannelA(
        connection->chat, channel, password, &channelCallbacks, piJoinRoomEnterChannelCallback, operation, CHATFalse);
...
*******************/
PEERBool
piNewListGroupRoomsOperation(PEER peer, const char* fields, peerListGroupRoomsCallback callback, void* param, int opID)
{
    piOperation* operation;
    ...
    operation = connection->listingGroupsOperation =
        piAddOperation(peer, PI_LIST_GROUP_ROOMS_OPERATION, NULL, (PEERCBType)callback, param, opID);
    if (!operation)
        return PEERFalse;
...
/* Get Player Info.
*******************/
static void piGetPlayerInfoCallbackA(CHAT chat,
                                     CHATBool success,
                                     const char* nick,
                                     const char* user,
                                     const char* address,
                                     void* param)
{
    ...
    if (operation->callback) {
        if (operation->type == PI_GET_PLAYER_INFO_OPERATION)
            piAddGetPlayerInfoCallback(peer,
                                       (PEERBool)success,
                                       nick,
                                       IP,
                                       profileID,
                                       (peerGetPlayerInfoCallback)operation->callback,
                                       operation->callbackParam,
                                       operation->ID);
        else if (operation->type == PI_GET_PROFILE_ID_OPERATION)
            piAddGetPlayerProfileIDCallback(peer,
                                            (PEERBool)success,
                                            nick,
                                            profileID,
                                            (peerGetPlayerProfileIDCallback)operation->callback,
                                            operation->callbackParam,
                                            operation->ID);
        else if (operation->type == PI_GET_IP_OPERATION)
            piAddGetPlayerIPCallback(peer,
                                     (PEERBool)success,
                                     nick,
                                     IP,
                                     (peerGetPlayerIPCallback)operation->callback,
                                     operation->callbackParam,
                                     operation->ID);
        else
            assert(0);
...
#ifdef GSI_UNICODE
static void piGetPlayerInfoCallbackW(CHAT chat,
                                     CHATBool success,
                                     const unsigned short* nick,
...
PEERBool
piNewGetPlayerInfoOperation(PEER peer, const char* nick, peerGetPlayerInfoCallback callback, void* param, int opID)
{
    piOperation* operation;
...
PEERBool
piNewGetProfileIDOperation(PEER peer, const char* nick, peerGetPlayerProfileIDCallback callback, void* param, int opID)
{
    piOperation* operation;
...
PEERBool piNewGetIPOperation(PEER peer, const char* nick, peerGetPlayerIPCallback callback, void* param, int opID)
{
    piOperation* operation;
...
/* Change Nick.
**************/
static void piChangeNickCallbackA(CHAT chat, CHATBool success, const char* oldNick, const char* newNick, void* param)
{
    PEER_CONNECTION_OP;
    ...
    ////////////////////
    if (operation->callback)
        piAddChangeNickCallback(peer,
                                (PEERBool)success,
                                oldNick,
                                newNick,
                                (peerChangeNickCallback)operation->callback,
                                operation->callbackParam,
                                operation->ID);
...
#ifdef GSI_UNICODE
static void piChangeNickCallbackW(CHAT chat,
                                  CHATBool success,
...
PEERBool
piNewChangeNickOperation(PEER peer, const char* newNick, peerChangeNickCallback callback, void* param, int opID)
{
    piOperation* operation;
...
/* Get Global Keys.
******************/
static void piGetGlobalKeysCallbackA(CHAT chat,
                                     CHATBool success,
                                     const char* user,
                                     int num,
                                     const char** keys,
                                     ...
                                     void* param)
{
    ...
    ////////////////////
    if (operation->callback)
        piAddGetGlobalKeysCallback(peer,
                                   (PEERBool)success,
                                   user,
                                   num,
                                   keys,
                                   values,
                                   (peerGetGlobalKeysCallback)operation->callback,
                                   operation->callbackParam,
                                   operation->ID);
...
#ifdef GSI_UNICODE
static void piGetGlobalKeysCallbackW(CHAT chat,
                                     CHATBool success,
                                     const unsigned short* user,
                                     int num,
...
PEERBool piNewGetGlobalKeysOperation(PEER peer,
                                     const char* target,
                                     int num,
                                     const char** keys,
...
/* Get Room Keys.
****************/
static void piGetChannelKeysCallbackA(CHAT chat,
                                      CHATBool success,
                                      const char* channel,
                                      const char* user,
                                      int num,
                                      ...
                                      void* param)
{
    ...
    if (operation->callback)
        piAddGetRoomKeysCallback(peer,
                                 ...
                                 keys,
                                 values,
                                 (peerGetRoomKeysCallback)operation->callback,
                                 operation->callbackParam,
                                 operation->ID);
...
#ifdef GSI_UNICODE
static void piGetChannelKeysCallbackW(CHAT chat,
                                      CHATBool success,
                                      const unsigned short* channel,
                                      const unsigned short* user,
                                      int num,
                                      ...
                                      void* param)
{
    ...
    char** values_A = UCS2ToUTF8StringArrayAlloc(values, num);
    int i;
    piGetChannelKeysCallbackA(
        chat, success, channel_A, user_A, num, (const char**)keys_A, (const char**)values_A, param);
    gsifree(channel_A);
    gsifree(user_A);
    for (i = 0; i < num; i++) {
        gsifree(keys_A[i]);
...
PEERBool piNewGetRoomKeysOperation(PEER peer,
                                   RoomType roomType,
                                   const char* nick,
                                   int num,
                                   const char** keys,
...
/* Authenticate CD Key
*********************/
static void piAuthenticateCDKeyCallbackA(CHAT chat, int result, const char* message, void* param)
{
    ...
    ////////////////////
    if (operation->callback)
        piAddAuthenticateCDKeyCallback(peer,
                                       result,
                                       message,
                                       (peerAuthenticateCDKeyCallback)operation->callback,
                                       operation->callbackParam,
                                       operation->ID);
...
#ifdef GSI_UNICODE
static void piAuthenticateCDKeyCallbackW(CHAT chat, int result, const unsigned short* message, void* param)
{
...
PEERBool piNewAuthenticateCDKeyOperation(PEER peer,
                                         const char* cdkey,
...
/* AutoMatch
***********/
PEERBool piNewAutoMatchOperation(PEER peer,
                                 SOCKET socket,
                                 unsigned short port,
