...
static int GS_STATIC_CALLBACK piPlayersTableCompareFn(const void* elem1, const void* elem2)
{
...
PEERBool piPlayersInit(PEER peer)
{
    ...
    //////////////////////////
    connection->players = TableNew(sizeof(piPlayer),
                                   PI_PLAYERS_NUM_BUCKETS,
                                   piPlayersTableHashFn,
                                   piPlayersTableCompareFn,
...
static piPlayer* piAddPlayer(PEER peer, const char* nick, PEERBool initialize)
{
    piPlayer* player;
...
static int piFindPlayersByIPMap(void* elem, void* clientdata)
{
    piPlayer* player = (piPlayer*)elem;
    unsigned int IP;
...
static void piSetNewPlayerFlags(PEER peer, const char* nick, RoomType roomType, int flags)
{
    piPlayer* player;
    int oldFlags;
...
PEERBool piIsPlayerHost(piPlayer* player)
{
    ...
    return (PEERBool)((player->flags[StagingRoom] & (PEER_FLAG_OP | PEER_FLAG_HOST))
                      == (PEER_FLAG_OP | PEER_FLAG_HOST));
}
