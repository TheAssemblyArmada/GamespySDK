...
static PINGERBool piSocketInit(const char* localAddress, unsigned short localPort)
{
    int rcode;
...
static void piQueueCallback(unsigned int IP,
                            unsigned short port,
                            int ping,
                            const char* data,
                            int len,
...
static void piCallCallbacks(void)
{
    ...
    while (ArrayLength(piCallbacks) > 0) {
        ...
        ///////////
        callback->callback(
            callback->IP, callback->port, callback->ping, callback->data, callback->len, callback->param);
...
static void piProcessTrip2(piUDPPing* udpPing, const char* data, SOCKADDR_IN* from, gsi_time recvTime)
{
    ...
    if (activePing->reply != NULL) {
        ...
        // Add the callback.
        ////////////////////
        piQueueCallback(from->sin_addr.s_addr,
                        from->sin_port,
                        ping,
                        data,
                        PI_DATA_MAX_LEN,
                        activePing->replyParam,
                        activePing->reply);
    }
            ...
            if (activePing->timeout != 0) {
                ...
                if (activePing->timeout <= now) {
                    ...
                    if ((activePing->originator) && (activePing->reply != NULL)) {
                        // Queue the callback.
                        //////////////////////
                        piQueueCallback(activePing->remoteIP,
                                        activePing->remotePort,
                                        PINGER_TIMEOUT,
                                        NULL,
                                        0,
                                        activePing->replyParam,
