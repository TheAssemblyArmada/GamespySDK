...
static SObject* AddObject(ObjectType type)
{
    int i;
    SObject* object;
...
static void RemoveObject(SObject* object)
{
    assert(object);
    if (!object)
        return;
...
static void ClientKilled(Client* killed, Client* killer)
{
#if DEATH_MESSAGES
...
static void RocketThink(SObject* rocket)
{
    assert(rocket);

    // Move the rocket.
    ///////////////////
    ComputeNewPosition(rocket->position, rocket->position, FORWARD, rocket->rotation, Diff, ROCKET_SPEED, GT2False);

    // Check if we should disappear.
    ////////////////////////////////
    if ((rocket->position[0] < (MAP_MIN - MAP_EXTRA)) || (rocket->position[0] > (MAP_MAX + MAP_EXTRA))
        || (rocket->position[1] < (MAP_MIN - MAP_EXTRA)) || (rocket->position[1] > (MAP_MAX + MAP_EXTRA))) {
        RemoveObject(rocket);
    }
}

static void RocketTouchObject(SObject* rocket, SObject* object)
{
...
static void RocketTouchClient(SObject* rocket, Client* client)
{
    // Check for death.
    ///////////////////
    if (client->dead)
...
static void MineThink(SObject* mine)
{
    // Check for detonate.
    //////////////////////
...
static void MineTouchObject(SObject* mine, SObject* object)
{
    // Ignore explosions.
    /////////////////////
    if (object->type == ObjectExplosion)
...
static void MineTouchClient(SObject* mine, Client* client)
{
...
static void AsteroidThink(SObject* asteroid)
{
    // Rotate the asteroid.
    ///////////////////////
    ...
    /////////////////////
    ComputeNewPosition(
        asteroid->position, asteroid->position, FORWARD, asteroid->heading, Diff, asteroid->speed, GT2False);
...
static void AsteroidTouchObject(SObject* asteroid, SObject* object)
{
...
static void AsteroidTouchClient(SObject* asteroid, Client* client)
{
    // Check for death.
    ///////////////////
    if (client->dead)
...
static void AsteroidDie(SObject* asteroid)
{
    // One less asteroid.
    /////////////////////
    NumAsteroids--;
...
static void ExplosionThink(SObject* explosion)
{
    // Check for done.
    //////////////////
...
static void ExplosionTouchObject(SObject* explosion, SObject* object)
{
}

static void ExplosionTouchClient(SObject* explosion, Client* client)
{
...
static SObject* AddExplosion(V2f position, int owner)
{
    SObject* explosion;
...
static void ClientThink(Client* client)
{
    // Dead?
    ////////
    if (client->dead) {
...
void ClientPress(int clientIndex, const char* button)
{
    SObject* object;
    Client* client;
    V2f forward;
...
void ObjectThink(SObject* object)
{
    int i;
    SObject* other;
    Client* client;
...
void ObjectsThink(unsigned long now, unsigned long diff)
{
    Client* client;
    SObject* object;
    int i;
