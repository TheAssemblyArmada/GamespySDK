...
static void Broadcast(const char* message, int len, int exceptIndex, GT2Bool reliable)
{
    int i;
...
void BroadcastText(const char* message, int exceptIndex, GT2Bool reliable)
{
    ...
    // Encode the message.
    //////////////////////
    rcode = gtEncode(MSG_S_CHAT, MSG_S_CHAT_STR, buffer, sizeof(buffer), messageCopy);
    if (rcode != -1) {
...
static void SendAddClient(int sendClientIndex, int newClientIndex)
{
    char buffer[256];
    Client* sendClient;
    Client* newClient;
    ...
    // Encode the message.
    /////////////////////
    rcode = gtEncode(MSG_S_ADDCLIENT, MSG_S_ADDCLIENT_STR, buffer, sizeof(buffer), newClientIndex, newClient->nick);
    if (rcode != -1) {
        // Send the message.
        ////////////////////
...
static void SendDelClient(int sendClient, int delClient)
{
    char buffer[32];
    int rcode;
    ...
    // Encode the message.
    /////////////////////
    rcode = gtEncode(MSG_S_DELCLIENT, MSG_S_DELCLIENT_STR, buffer, sizeof(buffer), delClient);
    if (rcode != -1) {
        // Send the message.
        ////////////////////
...
static void SendStart(int sendClient)
{
    char buffer[32];
    int rcode;
    ...
    // Encode the message.
    /////////////////////
    rcode = gtEncode(MSG_S_START, MSG_S_START_STR, buffer, sizeof(buffer), sendClient);
    if (rcode != -1) {
        // Send the message.
        ////////////////////
        gt2Send(clients[sendClient].connection, (const GT2Byte*)buffer, rcode, GT2True);
...
void SendSound(int sound, int sendClient)
{
    char buffer[32];
    int rcode;
    ...
    // Encode the message.
    /////////////////////
    rcode = gtEncode(MSG_S_SOUND, MSG_S_SOUND_STR, buffer, sizeof(buffer), sound);
    if (rcode != -1) {
        // Send the message.
        ////////////////////
        if (sendClient == -1)
...
void SendNumAsteroids(int total, int sendClient)
{
    char buffer[32];
    int rcode;
    ...
    // Encode the message.
    /////////////////////
    rcode = gtEncode(MSG_S_NUMASTEROIDS, MSG_S_NUMASTEROIDS_STR, buffer, sizeof(buffer), (byte)total);
    ;
    if (rcode != -1) {
        // Send the message.
        ////////////////////
...
static void SendUpdates(void)
{
    ...
    ///////////////////////////
    rcode = gtEncode(MSG_S_UPDATE, MSG_S_UPDATE_STR, buffer, len, nextServerUpdateID++, numClients, numObjects);
    if (rcode == -1)
        return;
    buffer += rcode;
    len -= rcode;
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i].used) {
            ...
            rcode = gtEncodeNoType(MSG_S_UPDATE_CLIENT_STR,
                                   buffer,
                                   len,
                                   i,
    ...
    for (i = 0; i < MAX_OBJECTS; i++) {
        if (sObjects[i].used) {
            ...
            rcode = gtEncodeNoType(MSG_S_UPDATE_OBJECT_STR,
                                   buffer,
                                   len,
                                   object->type,
...
static void ServerReceivedCallback(GT2Connection connection, GT2Byte* message, int len, GT2Bool reliable)
{
    int clientIndex;
    Client* client;
    GTMessageType type;
    ...
    if (type == MSG_C_UPDATE) {
        ...
        /////////////
        rcode = gtDecode(
            MSG_C_UPDATE_STR, (char*)message, len, &updateID, &packedRotation, &forward, &backward, &right, &left);
        if (rcode == -1)
            return;
        ...
        if (client->lastUpdate && !client->dead && client->motion) {
            ...
            ////////////////////////////
            ComputeNewPosition(
                client->position, client->position, client->motion, client->rotation, diff, PLAYER_SPEED, GT2True);
        }
    ...
    /////////
        ...
        // Decode it.
        /////////////
        rcode = gtDecode(MSG_C_PRESS_STR, (char*)message, len, press);
        if (rcode == -1)
            return;
    ...
    ////////
        ...
        ///////////////////////////////
        rcode = gtDecode(MSG_C_CHAT_STR, (char*)message, len, buffer + strlen(buffer));
        if (rcode == -1)
            return;
...
static void ServerClosedCallback(GT2Connection connection, GT2CloseReason reason)
{
    int clientIndex;
    int i;
    SObject* object;
...
static void ServerConnectAttemptCallback(GT2Socket socket,
                                         ...
                                         int len)
{
    ...
    // Decode the data.
    ///////////////////
    rcode = gtDecode(MSG_C_INITIAL_STR, (char*)message, len, nick);
    if (rcode == -1) {
...
static void ServerSocketErrorCallback(GT2Socket socket)
{
    printf("Server socket error\n");
}

GT2Bool InitializeServer(void)
{
    GT2Result result;
    int i;
...
void ServerThink(unsigned long now)
{
