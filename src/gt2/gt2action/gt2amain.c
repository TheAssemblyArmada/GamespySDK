...
static GHTTPBool
NaminatorCallback(GHTTPRequest request, GHTTPResult result, char* buffer, GHTTPByteCount bufferLen, void* param)
{
...
static void ParseArgs(int argc, char** argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        // Nick.
        ////////
        else if ((strcasecmp(argv[i], "-nick") == 0) || (strcasecmp(argv[i], "-name") == 0)) {
            if (++i < argc) {
                char* str;
...
static GT2Bool Initialize(void)
{
    // Init the display.
    ////////////////////
    InitializeDisplay();
