...
static void ClientSocketErrorCallback(GT2Socket socket)
{
    printf("Server socket error\n");
    GSI_UNUSED(socket);
}

static void ClientConnectedCallback(GT2Connection connection, GT2Result result, GT2Byte* message, int len)
{
...
static void ClientReceivedCallback(GT2Connection connection, GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    if (type == MSG_S_ADDCLIENT) {
        ...
        // Decode it.
        /////////////
        rcode = gtDecode(MSG_S_ADDCLIENT_STR, (char*)message, len, &newPlayerIndex, nick);
        if (rcode == -1)
            return;
    ...
    /////////////////
        ...
        // Decode it.
        /////////////
        rcode = gtDecode(MSG_S_DELCLIENT_STR, (char*)message, len, &delPlayerIndex);
        if (rcode == -1)
            return;
    ...
    ///////////////////////////////
        ...
        // Decode it.
        /////////////
        rcode = gtDecode(MSG_S_START_STR, (char*)message, len, &index);
        if (rcode == -1)
            return;
    ...
    /////////////////
        ...
        // Decode the header.
        /////////////////////
        rcode = gtDecode(MSG_S_UPDATE_STR, (char*)message, len, &updateID, &updatedClients, &updatedObjects);
        if (rcode == -1)
            return;
        message += rcode;
        len -= rcode;
        ...
        for (i = 0; i < updatedClients; i++) {
            // Decode the update.
            /////////////////////
            rcode = gtDecodeNoType(MSG_S_UPDATE_CLIENT_STR,
                                   (char*)message,
                                   len,
                                   &index,
                                   &packedPosition[0],
        ...
        for (i = 0; i < updatedObjects; i++) {
            // Decode the object.
            /////////////////////
            rcode = gtDecodeNoType(MSG_S_UPDATE_OBJECT_STR,
                                   (char*)message,
                                   len,
                                   &type,
                                   &packedPosition[0],
    ...
    ////////
        ...
        // Decode it.
        /////////////
        rcode = gtDecode(MSG_S_CHAT_STR, (char*)message, len, &buffer);
        if (rcode == -1)
            return;
    ...
    /////////
        ...
        // Decode it.
        /////////////
        rcode = gtDecode(MSG_S_SOUND_STR, (char*)message, len, &sound);
        if (rcode == -1)
            return;
    ...
    ////////////////
        ...
        // Decode it.
        /////////////
        rcode = gtDecode(MSG_S_NUMASTEROIDS_STR, (char*)message, len, &total);
        if (rcode == -1)
            return;
...
static void ClientClosedCallback(GT2Connection connection, GT2CloseReason reason)
{
...
void SendPress(const char* value)
{
    if (connected) {
        char buffer[32];
        int rcode;
        ...
        // Encode the message.
        //////////////////////
        rcode = gtEncode(MSG_C_PRESS, MSG_C_PRESS_STR, buffer, sizeof(buffer), value);
        if (rcode != -1) {
            // Send the message.
            ////////////////////
...
void SendChat(const char* message)
{
    if (connected) {
        ...
        // Encode the message.
        //////////////////////
        rcode = gtEncode(MSG_C_CHAT, MSG_C_CHAT_STR, buffer, sizeof(buffer), message);
...
void ClientThink(unsigned long now)
{
...
static void ClientSendMonitor(GT2Connection connection, int filterID, const GT2Byte* message, int len, GT2Bool reliable)
{
    // Update stats.
    ////////////////
    if (reliable) {
...
static void ClientReceiveMonitor(GT2Connection connection, int filterID, GT2Byte* message, int len, GT2Bool reliable)
{
    // Update stats.
    ////////////////
    if (reliable) {
...
GT2Bool InitializeClient(void)
{
    ...
    //////////////////////////
    rcode = gtEncode(MSG_C_INITIAL, MSG_C_INITIAL_STR, buffer, sizeof(buffer), localNick);
    if (rcode == -1)
        return GT2False;
