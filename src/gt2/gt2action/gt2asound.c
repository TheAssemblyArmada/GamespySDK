...
const char* soundFiles[] =
    {"sounds\\explosion.wav", "sounds\\mine.wav", "sounds\\die.wav", "sounds\\rocket.wav", "sounds\\pickup.wav", NULL};
...
// Adapted from a Microsoft DirectSound sample.
///////////////////////////////////////////////
static HRESULT WaveOpenFile(CHAR* strFileName, HMMIO* phmmioIn, WAVEFORMATEX** ppwfxInfo, MMCKINFO* pckInRIFF)
{
    HRESULT hr;
    HMMIO hmmioIn = NULL;
...
// Adapted from a Microsoft DirectSound sample.
///////////////////////////////////////////////
static HRESULT WaveStartDataRead(HMMIO* phmmioIn, MMCKINFO* pckIn, MMCKINFO* pckInRIFF)
{
    // Seek to the data
...
////////////////////////////////////////////
static GT2Bool LoadSound(int sound, const char* filename, LPDIRECTSOUNDBUFFER restoreBuffer)
{
    int rcode;
    HRESULT result;
    ...
    if (!restoreBuffer) {
        ...
        // Create the buffer.
        /////////////////////
        result = IDirectSound_CreateSoundBuffer(DirectSound, &bufferDescriptor, &soundEffect.SoundBuffers[0], NULL);
        if (FAILED(result)) {
    ...
    ///////////////////
    result =
        IDirectSoundBuffer_Lock(soundBuffer, 0, 0, &lockedBuffer, &lockedBufferLen, NULL, NULL, DSBLOCK_ENTIREBUFFER);
    if (FAILED(result)) {
        mmioClose(IOHandle, 0);
        return GT2False;
    }
    ...
    /////////////////////////////////
    rcode = WaveReadFile(IOHandle, lockedBufferLen, (BYTE*)lockedBuffer, &dataChunkInfo, &numBytes);
    if (rcode) {
        mmioClose(IOHandle, 0);
        return GT2False;
    }
    ...
    // Unlock the buffer.
    /////////////////////
    IDirectSoundBuffer_Unlock(soundBuffer, lockedBuffer, lockedBufferLen, NULL, 0);
    ...
    if (i == soundEffect->numDuplicates) {
        ...
        if (i == MAX_DUPLICATES) {
            ...
            result = IDirectSound_DuplicateSoundBuffer(
                DirectSound, soundEffect->SoundBuffers[0], &soundEffect->SoundBuffers[i]);
            if (FAILED(result))
                return;
