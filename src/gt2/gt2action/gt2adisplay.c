...
static void RemoveOldestChat(void)
{
    int i;
...
static void AddChatLine(const char* message)
{
    // Do scrolling.
    ////////////////
    ScrollChat();
...
static void DrawString(const char* string, int x, int y, const V3b color, float scale)
{
    scale *= TEXT_SCALE;
    glColor3ubv(color);
    glPushMatrix();
...
static int GetCharacterWidth(char ch)
{
    int width;
...
static void DrawCharacter(char ch, int x, int y, const V3b color)
{
    glColor3ubv(color);
    glPushMatrix();
    glTranslatef(x, y, 0);
...
static void DrawBoundingCircle(float radius)
{
...
static void DrawPlayer(Player* player, const V2f position, float rotation)
{
    int width;
...
static void Predict(V2f newPosition, float* rotation, Player* player)
{
    GT2Bool local;
    ...
    if (DrawWithPredictionOption && !player->dead && player->motion) {
        ...
        ////////////////////////
        ComputeNewPosition(
            newPosition, player->position, player->motion, player->rotation, Diff, PLAYER_SPEED, GT2True);
...
static void DrawRocket(CObject* rocket)
{
    V2f position;
    ...
    if (DrawWithPredictionOption) {
        // Predict new position.
        ////////////////////////
        ComputeNewPosition(position, rocket->position, FORWARD, rocket->rotation, Diff, ROCKET_SPEED, GT2False);
    } else {
        // Just use the actual position.
        ////////////////////////////////
...
static void DrawMine(CObject* mine)
{
    glMatrixMode(GL_MODELVIEW);
...
static void DrawAsteroid(CObject* asteroid)
{
    glMatrixMode(GL_MODELVIEW);
...
static void DrawExplosion(CObject* explosion)
{
...
static int ReadLittleInt(FILE* file)
{
    byte bytes[4];
    int i;
...
static GT2Bool LoadTexture(const char* filename, GLuint* texture, GT2Bool useAlpha)
{
    int width;
    int height;
    GLubyte* bytes;
...
static void InitStar(unsigned long now, Star* star)
{
    int color;
...
static void DrawFPS(unsigned long diff)
{
    ...
    {
        char buf[64];
        sprintf(buf, "%d asteroid%s", ClientNumAsteroids, (ClientNumAsteroids == 1) ? "" : "s");
...
static void DrawRadar(void)
{
    ...
    //////////////////////////////////
    glTranslatef(WINDOW_MAX - (RADAR_SIZE * RadarScaleOption), WINDOW_MAX - (RADAR_SIZE * RadarScaleOption), 0);

    // Scale so we can draw in map space.
    /////////////////////////////////////
    glScalef(RADAR_SIZE / MAP_MAX * RadarScaleOption, RADAR_SIZE / MAP_MAX * RadarScaleOption, 1);
...
static void SetWindowView(void)
{
    glLoadIdentity();
    glViewport(0, 0, screenWidth, screenHeight);
