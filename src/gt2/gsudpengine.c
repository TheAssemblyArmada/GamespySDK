...
{
    GSUdpMsgHandler *msgHandler1 = (GSUdpMsgHandler*)theFirstHandler, *msgHandler2 = (GSUdpMsgHandler*)theSecondHandler;
    int initCmp;
...
{
    GSUdpRemotePeer *aPeer1 = (GSUdpRemotePeer*)theFirstPeer, *aPeer2 = (GSUdpRemotePeer*)theSecondPeer;
...
void gsUdpSocketError(GT2Socket theSocket)
{
    ...
    gsDebugFormat(GSIDebugCat_Common,
...
void gsUdpClosedRoutingCB(GT2Connection theConnection, GT2CloseReason reason)
{
    ...
    gsDebugFormat(GSIDebugCat_Common,
                  ...
                  gt2AddressToString(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), anAddr));
    len = ArrayLength(aUdp->mMsgHandlers);
    for (index = 0; index < len; index++) {
        ...
        if (aHandler->mClosed) {
            ...
            aHandler->mClosed(
                gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), aReason, aHandler->mUserData);
        }
    }
...
void gsUdpConnectedRoutingCB(GT2Connection theConnection, GT2Result theResult, GT2Byte* theMessage, int theMessageLen)
{
    ...
    for (aIndex = 0; aIndex < len; aIndex++) {
        ...
        if (aRemotePeerIdx != NOT_FOUND) {
            if (aTempHandler->mConnected) {
                ...
                aTempHandler->mConnected(gt2GetRemoteIP(theConnection),
                                         gt2GetRemotePort(theConnection),
                                         aCode,
                                         theResult == GT2Rejected ? gsi_true : gsi_false,
                                         aTempHandler->mUserData);
            }
    ...
    if (aUdp->mAppPendingConnections > 0) {
        if (aUdp->mAppConnected) {
            ...
            aUdp->mAppConnected(gt2GetRemoteIP(theConnection),
                                gt2GetRemotePort(theConnection),
                                aCode,
                                theResult == GT2Rejected ? gsi_true : gsi_false,
                                aUdp->mAppUserData);
...
void gsUdpPingRoutingCB(GT2Connection theConnection, int theLatency)
{
    ...
    for (index = 0; index < len; index++) {
        ...
        if (aHandler->mPingReply) {
            ...
            aHandler->mPingReply(gt2GetRemoteIP(theConnection),
                                 gt2GetRemotePort(theConnection),
                                 (unsigned int)theLatency,
                                 aHandler->mUserData);
            return;
        }
    }
    ...
    if (aUdp->mAppPingReply) {
        gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Passed to app\n");
        aUdp->mAppPingReply(gt2GetRemoteIP(theConnection),
                            gt2GetRemotePort(theConnection),
                            (unsigned int)theLatency,
                            aUdp->mAppUserData);
    }
...
void gsUdpReceivedRoutingCB(GT2Connection theConnection, GT2Byte* theMessage, int theMessageLen, GT2Bool reliable)
{
        ...
        if (index != NOT_FOUND) {
            ...
            if (aHandlerFound->mReceived) {
                gsDebugFormat(GSIDebugCat_Common,
                              ...
                              GSIDebugLevel_Comment,
                              "[Udp Engine] Passed to message handler\n");
                aHandlerFound->mReceived(gt2GetRemoteIP(theConnection),
                                         gt2GetRemotePort(theConnection),
                                         theMessage + GS_UDP_MSG_HEADER_LEN,
                                         (unsigned int)(theMessageLen - GS_UDP_MSG_HEADER_LEN),
                                         reliable,
                                         aHandlerFound->mUserData);
                return;
            }
        }
    }
    ...
    if (aUdp->mAppRecvData) {
        gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Passed to app\n");
        aUdp->mAppRecvData(gt2GetRemoteIP(theConnection),
                           gt2GetRemotePort(theConnection),
                           theMessage,
                           (unsigned int)theMessageLen,
                           reliable,
                           aUdp->mAppUserData);
    }
    GSI_UNUSED(anAddr);
...
GT2Bool gsUdpUnrecognizedMsgCB(GT2Socket theSocket,
                               unsigned int theIp,
                               ...
                               int theMsgLen)
{
    ...
    if (aUdp->mAppUnknownMessage) {
        ...
        aRet = aUdp->mAppUnknownMessage(
            theIp, thePort, (unsigned char*)theMessage, (unsigned int)theMsgLen, aUdp->mAppUserData);
...
void gsUdpConnAttemptCB(GT2Socket socket,
...
// functions below.
GSUdpErrorCode gsUdpEngineInitialize(unsigned short thePort,
                                     int theIncomingBufSize,
                                     int theOutgoingBufSize,
                                     gsUdpErrorCallback theAppNetworkError,
                                     ...
                                     void* theAppUserData)
{
    int incomingBufferSize, outgoingBufferSize;
    char anAddr[GS_IP_ADDR_AND_PORT];
    ...
    aGt2Result = gt2CreateSocket(&aUdp->mSocket, anAddr, outgoingBufferSize, incomingBufferSize, gsUdpSocketError);
    ...
    if (aUdp->mRemotePeers == NULL) {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Memory, GSIDebugLevel_HotError, "[Udp Engine] No more memory!!!\n");
...
GSUdpErrorCode gsUdpEngineGetPeerState(unsigned int theIp, unsigned short thePort, GSUdpPeerState* thePeerState)
{
    ...
    if (!aUdp->mInitialized) {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_State, GSIDebugLevel_Debug, "[Udp Engine] Engine not initialized\n");
...
// IP and port.
GSUdpErrorCode gsUdpEngineStartTalkingToPeer(unsigned int theIp,
                                             unsigned short thePort,
                                             ...
                                             int timeOut)
{
    ...
    if (!aUdp->mInitialized) {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Debug, "[Udp Engine] Engine not initialized\n");
        return GS_UDP_NETWORK_ERROR;
    }
    ...
    if (index != NOT_FOUND) {
        ...
        gt2Connect(aUdp->mSocket,
                   &aRemotePeer.mConnection,
                   anAddr,
                   (unsigned char*)theInitMsg,
                   GS_UDP_MSG_HEADER_LEN,
                   timeOut,
                   &aCallbacks,
                   GT2False);
        ...
        index = ArraySearch(aUdp->mMsgHandlers, &aHandler, gsUdpMsgHandlerCompare, 0, 0);
        if (index != NOT_FOUND) {
            GSUdpRemotePeer* aRemotePeerPtr =
                (GSUdpRemotePeer*)ArrayNth(aUdp->mRemotePeers, ArrayLength(aUdp->mRemotePeers) - 1);
            GSUdpMsgHandler* aHandlerFound = (GSUdpMsgHandler*)ArrayNth(aUdp->mMsgHandlers, index);
...
// freeSpace = 1460 - 16 - 7
GSUdpErrorCode gsUdpEngineSendMessage(unsigned int theIp,
                                      unsigned short thePort,
                                      char theHeader[GS_UDP_MSG_HEADER_LEN],
                                      unsigned char* theMsg,
                                      unsigned int theMsgLen,
                                      gsi_bool theReliable)
{
    GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
...
SOCKET gsUdpEngineGetSocket()
{
    ...
    if (!aUdp->mInitialized) {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_HotError, "[Udp Engine] Engine not initialized\n");
                                        ...
                                        void* theUserData)
{
    ...
    if (!theInitMsg[0]) {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Debug, "[Udp Engine] Invalid init message\n");
        return GS_UDP_PARAMETER_ERROR;
    }

    if (!theHeader[0]) {
        gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Debug, "[Udp Engine] Invalid header\n");
        return GS_UDP_PARAMETER_ERROR;
    }
...
GSUdpErrorCode gsUdpEngineRemoveMsgHandler(char theHeader[GS_UDP_MSG_HEADER_LEN])
{
    ...
    if (!theHeader || !theHeader[0]) {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Debug, "[Udp Engine] invalid or empty header\n");
        return GS_UDP_PARAMETER_ERROR;
    }
