...
** UTILITY FUNCTIONS **
**********************/
GT2Bool ParseMessage(const char* message, int len)
{
    const char* keyStart;
...
const char* GetConnectionName(GT2Connection connection)
{
    return gt2AddressToString(gt2GetRemoteIP(connection), 0, NULL);
}

void TellBackupHost(int index)
{
    char buffer[32];
...
GT2Bool StartConnecting(const char* server)
{
    GT2Connection connection;
    GT2Socket socket;
    GT2Result result;
