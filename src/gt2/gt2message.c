...
static GT2Bool gti2HandleClientResponse(GT2Connection connection, GT2Byte* message, int len)
{
    ...
    if (!gti2ConnectAttemptCallback(connection->socket,
                                    connection,
                                    connection->ip,
                                    connection->port,
                                    latency,
                                    message + GTI2_RESPONSE_LEN,
                                    len - GTI2_RESPONSE_LEN))
        return GT2False;
...
static GT2Bool gti2BufferIncomingMessage(GT2Connection connection,
...
restart:
    ...
    for (i = (num - 1); i >= 0; i--) {
        ...
        if (message->serialNumber == connection->expectedSerialNumber) {
            // deliver it
            if (!gti2DeliverReliableMessage(
                    connection, message->type, connection->incomingBuffer.buffer + message->start, message->len))
                return GT2False;
...
static GT2Bool gti2HandleNack(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if ((gti2SNDiff(messageInfo->serialNumber, SNMin) >= 0)
            && (gti2SNDiff(messageInfo->serialNumber, SNMax) <= 0)) {
...
static GT2Bool gti2HandleMessage(GT2Socket socket, GT2Byte* message, int len, unsigned int ip, unsigned short port)
{
    ...
    magicString = ((actualLength > GTI2_MAGIC_STRING_LEN)
                   && (memcmp(actualMessage, GTI2_MAGIC_STRING, GTI2_MAGIC_STRING_LEN) == 0));
    ...
    if (magicString
        && ((actualLength >= (GTI2_MAGIC_STRING_LEN * 2))
            && (memcmp(actualMessage + GTI2_MAGIC_STRING_LEN, GTI2_MAGIC_STRING, GTI2_MAGIC_STRING_LEN) == 0))) {
        message[3] = message[1];
        message[2] = message[0];
...
#ifdef RECV_LOG
            // log it
            gti2LogMessage(address.sin_addr.s_addr, ntohs(address.sin_port), socket->ip, socket->port, buffer, rcode);
#endif
            // handle the message
...
static GT2Bool gti2BeginReliableMessage(GT2Connection connection, GTI2MessageType type, int len, GT2Bool* overflow)
{
    ...
    if (connection->socket->protocolType == GTI2VdpProtocol)
        gti2BufferWriteData(
            &connection->outgoingBuffer, (const GT2Byte*)&vdpDataLength, connection->socket->protocolOffset);
...
GT2Bool gti2SendClientChallenge(GT2Connection connection, const char challenge[GTI2_CHALLENGE_LEN])
{
    ...
    // begin the message
    if (!gti2BeginReliableMessage(
            connection, GTI2MsgClientChallenge, totalLen + connection->socket->protocolOffset, &overflow))
        return GT2False;
...
GT2Bool gti2SendServerChallenge(GT2Connection connection,
                                ...
                                const char challenge[GTI2_CHALLENGE_LEN])
{
    ...
    // begin the message
    if (!gti2BeginReliableMessage(
            connection, GTI2MsgServerChallenge, totalLen + connection->socket->protocolOffset, &overflow))
        return GT2False;
...
GT2Bool
gti2SendClientResponse(GT2Connection connection, const char response[GTI2_RESPONSE_LEN], const char* message, int len)
{
    ...
    // begin the message
    if (!gti2BeginReliableMessage(
            connection, GTI2MsgClientResponse, totalLen + connection->socket->protocolOffset, &overflow))
        return GT2False;
...
GT2Bool gti2SendKeepAlive(GT2Connection connection)
{
    ...
    // begin the message
    if (!gti2BeginReliableMessage(
            connection, GTI2MsgKeepAlive, totalLen + connection->socket->protocolOffset, &overflow))
        return GT2False;
...
GT2Bool gti2SendAppUnreliable(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    gti2BufferWriteData(&connection->outgoingBuffer,
                        message + connection->socket->protocolOffset,
...
GT2Bool gti2ResendMessage(GT2Connection connection, GTI2OutgoingBufferMessage* message)
{
    ...
    type = (GTI2MessageType)connection->outgoingBuffer
               .buffer[message->start + connection->socket->protocolOffset + GTI2_MAGIC_STRING_LEN];
