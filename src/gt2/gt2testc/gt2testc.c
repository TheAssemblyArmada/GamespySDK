...
static void ReceivedCallback(GT2Connection connection, GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    if (server) {
        ...
        if ((pcurr->reliable != reliable) || (pcurr->len != len) || (memcmp(pcurr->data, message, (size_t)len) != 0)) {
            ...
            if (!hardcore || !(messageCount % 100)) {
                printf(
                    "Message %d validated (%d bytes, %s)", messageCount + 1, len, reliable ? "reliable" : "unreliable");
                if (server)
                    printf("\n");
                else
...
static void ConnectAttemptCallback(GT2Socket socket,
                                   GT2Connection connection,
                                   unsigned int ip,
                                   unsigned short port,
                                   int latency,
...
static void SendDumpCallback(GT2Socket socket,
                             GT2Connection connection,
                             unsigned int ip,
                             unsigned short port,
                             GT2Bool reset,
...
static void ReceiveDumpCallback(GT2Socket socket,
                                GT2Connection connection,
                                unsigned int ip,
                                unsigned short port,
                                GT2Bool reset,
...
static void EncodeTest()
{
    const char encodestr[] = {
        GT_INT, GT_UINT, GT_SHORT, GT_USHORT, GT_CHAR, GT_UCHAR, GT_FLOAT, GT_DOUBLE, GT_CSTR, GT_DBSTR, GT_RAW, 0};
...
int test_main(int argc, char** argv)
{
    ...
    do {
        ...
        result = gt2Connect(socket,
                            &Connection,
                            remoteAddress,
                            (GT2Byte*)initialMessage.data,
                            initialMessage.len,
                            10000,
                            &connectionCallbacks,
                            GT2True);
