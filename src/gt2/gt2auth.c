...
GT2Byte* gti2GetResponse(GT2Byte* buffer, const GT2Byte* challenge)
{
    ...
    for (i = 0; i < GTI2_RESPONSE_LEN; i++) {
        ...
        if (!valid || i == 0 || i == 13)
            buffer[i] = (GT2Byte)(33 + rand() % 93); //use chars in the range 33 - 125
        else {                                       //set the character to look back at, never use the random ones!
            if (i == 1 || i == 14)
                cchar = (char)challenge[i];
            else
                cchar = (char)challenge[i - 1];
            chalrand =
                abs((challenge[((i * challenge[i]) + GT2ChallengeKey[(i + challenge[i]) % keylen]) % GTI2_CHALLENGE_LEN]
                     ^ GT2ChallengeKey[(i * 17991 * cchar) % keylen]));
