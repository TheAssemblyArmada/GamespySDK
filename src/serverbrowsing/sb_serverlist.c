...
static int prevKeyCompare(SBServer server1, SBServer server2)
{
    ...
    case sbcm_int:
        diff = SBServerGetIntValueA(server1, prevsortkey, 0) - SBServerGetIntValueA(server2, prevsortkey, 0);
        break;
    case sbcm_float:
        f = SBServerGetFloatValueA(server1, prevsortkey, 0) - SBServerGetFloatValueA(server2, prevsortkey, 0);
...
static int GS_STATIC_CALLBACK IntKeyCompare(const void* entry1, const void* entry2)
{
    ...
    diff = SBServerGetIntValueA(server1, currsortkey, 0) - SBServerGetIntValueA(server2, currsortkey, 0);
...
static int GS_STATIC_CALLBACK FloatKeyCompare(const void* entry1, const void* entry2)
{
    ...
    f = SBServerGetFloatValueA(server1, currsortkey, 0) - SBServerGetFloatValueA(server2, currsortkey, 0);
...
static int GS_STATIC_CALLBACK StrCaseKeyCompare(const void* entry1, const void* entry2)
{
    ...
    diff = strcmp(SBServerGetStringValueA(server1, currsortkey, ""), SBServerGetStringValueA(server2, currsortkey, ""));
...
{
    slist->servers = ArrayNew(
        sizeof(SBServer), SERVER_GROWBY, NULL); //don't free the server automatically - it goes into the dead list
    slist->deadlist = NULL;
}
...
#define CALCULATEODDMODE(buffer, i, oddmode)                                                                           \
    ((buffer[i - 1] & 1) ^ (i & 1) ^ oddmode ^ (buffer[0] & 1) ^ ((buffer[0] < 79) ? 1 : 0)                            \
     ^ ((buffer[i - 1] < buffer[0]) ? 1 : 0));
...
SBError SBServerListConnectAndQuery(SBServerList* slist,
...
static void InitCryptKey(SBServerList* slist, char* key, int keylen)
{
    ...
    for (i = 0; i < keylen; i++) {
        slist->mychallenge[(i * seckey[i % seckeylen]) % LIST_CHALLENGE_LEN] ^=
            (char)((slist->mychallenge[i % LIST_CHALLENGE_LEN] ^ key[i]) & 0xFF);
    }
...
static int ParseServer(SBServerList* slist, SBServer server, char* buf, int len, int usepopularlist)
{
    ...
    // basic key and full key states
    {
        ...
        if ((flags & (HAS_FULL_RULES_FLAG | HAS_KEYS_FLAG)) == 0
            && ((state & STATE_BASICKEYS) | (state & STATE_FULLKEYS))) {
...
static SBError ProcessAdHocData(SBServerList* slist)
{
    ...
    while (slist->inbufferlen
           >= 3) //while there is at least 1 message in there.. (2 bytes for the size + 1 for the type)
    {
...
SBError SBSendPlayerSearchRequest(SBServerList* slist,
                                  char* searchName,
                                  int searchOptions,
                                  int maxResults,
                                  SBPlayerSearchCallbackFn callback)
{
...
#if defined(_PS3)
        error = (int)recvfrom(
            slist->slsocket, indata, sizeof(indata) - 1, 0, (struct sockaddr*)&saddr, (socklen_t*)&saddrlen);
#else
