...
static void QEStartQuery(SBQueryEngine* engine, SBServer server)
{
    ...
    if (server->state & STATE_PENDINGICMPQUERY) //send an ICMP ping request
    {
        ...
        memcpy(queryBuffer + sizeof(SBICMPHeader),
               &server->publicip,
               4); //put some data in the echo packet that we can use to verify the reply
...
void SBQueryEngineInit(SBQueryEngine* engine,
                       int maxupdates,
                       int queryversion,
                       SBBool lanBrowse,
                       SBEngineCallbackFn callback,
                       void* instance)
{
...
void SBQueryEngineUpdateServer(SBQueryEngine* engine,
                               SBServer server,
                               int addfront,
                               int querytype,
                               SBBool usequerychallenge)
{
    ...
    server->state &= (unsigned char)~(STATE_PENDINGBASICQUERY | STATE_PENDINGFULLQUERY | STATE_PENDINGICMPQUERY
                                      | STATE_PENDINGQUERYCHALLENGE | STATE_QUERYFAILED); //clear out these flags
...
SBServer SBQueryEngineUpdateServerByIP(SBQueryEngine* engine,
                                       const char* ip,
...
static void ProcessIncomingICMPReplies(SBQueryEngine* engine)
{
    ...
    for (server = engine->querylist.first; server != NULL; server = server->next) {
        if ((server->state & STATE_PENDINGICMPQUERY) == 0 || (server->flags & ICMP_IP_FLAG) == 0)
...
static void ProcessIncomingReplies(SBQueryEngine* engine, SBBool icmpSocket)
{
    ...
    while (CanReceiveOnSocket(recvSock)) {
        ...
        for (server = engine->querylist.first; server != NULL; server = server->next) {
            if ((icmpSocket && (server->flags & ICMP_IP_FLAG) && server->icmpip == saddr.sin_addr.s_addr)
                || //if it's an ICMP query and it matches the ICMP address
                (server->publicip == saddr.sin_addr.s_addr && (server->publicport == saddr.sin_port || icmpSocket))
                || //if it matches public - port doesnt need to match for ICMP
                (server->publicip == engine->mypublicip && (server->flags & PRIVATE_IP_FLAG)
                 && server->privateip == saddr.sin_addr.s_addr
                 && server->privateport == saddr.sin_port)) //or has a private, and matches
            {
                if (icmpSocket) {
    ...
    while (engine->querylist.first != NULL) {
        if (ctime > engine->querylist.first->updatetime + MAX_QUERY_MSEC) {
            ...
            engine->querylist.first->flags &=
                (unsigned char)~(STATE_PENDINGBASICQUERY | STATE_PENDINGFULLQUERY | STATE_PENDINGICMPQUERY);
