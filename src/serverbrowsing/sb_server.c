...
HashTable SBRefStrHash(SBServerList* slist)
{
    if (g_SBRefStrList == NULL)
        g_SBRefStrList = TableNew2(
            sizeof(SBRefString), LIST_NUMKEYBUCKETS, LIST_NUMKEYCHAINS, RefStringHash, RefStringCompare, RefStringFree);
...
SBServerGetPlayerStringValueW(SBServer server, int playernum, const unsigned short* key, const unsigned short* sdefault)
{
...
#ifdef GSI_UNICODE
const unsigned short*
SBServerGetTeamStringValueW(SBServer server, int teamnum, const unsigned short* key, const unsigned short* sdefault)
{
    char keyname_A[UKEY_LENGTH_MAX];
    char default_A[UKEY_LENGTH_MAX];
    const char* value_A = NULL;
...
static int GS_STATIC_CALLBACK KeyValCompareKey(const void* entry1, const void* entry2)
{
    GS_ASSERT(entry1)
    GS_ASSERT(entry2)
    if ((((SBKeyValuePair*)entry1)->key == NULL) || (((SBKeyValuePair*)entry2)->key == NULL))
...
SBServer SBAllocServer(SBServerList* slist, goa_uint32 publicip, unsigned short publicport)
{
    ...
    if (server == NULL)
        return NULL;
    server->keyvals =
        TableNew2(sizeof(SBKeyValuePair), NUM_BUCKETS, NUM_CHAINS, KeyValHashKey, KeyValCompareKey, KeyValFree);
