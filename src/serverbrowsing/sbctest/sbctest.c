...
static void AppDebug(const unsigned short* format, ...)
{
    ...
    gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice, "%s", tmp);
}
#else
static void AppDebug(const char* format, ...)
{
    va_list aList;
    va_start(aList, format);
    gsDebugVaList(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice, format, aList);
}
#endif
#else
...
{
    int i; // for-loop ctr
    gsi_char* defaultString =
        _T("");         // default string for SBServerGet functions - returns if specified string key is not found
    ...
    case sbc_serverupdated: // either basic or full information is now available for this server
        ...
        AppDebug(_T("  Players/MaxPlayers: %d/%d\n"),
                 SBServerGetIntValue(server, _T("numplayers"), defaultInt),
                 SBServerGetIntValue(server, _T("maxplayers"), defaultInt));
        ...
        if (SBServerHasFullKeys(server)) {
            ...
            // print player info
            AppDebug(_T("  Players:\n"));
            for (i = 0; i < SBServerGetIntValue(server, _T("numplayers"), 0);
                 i++) // loop through all players on the server
            {
...
int test_main(int argc, char** argp)
{
    ...
    sb = ServerBrowserNew(
        GAME_NAME, GAME_NAME, secret_key, version, maxConcUpdates, QVERSION_QR2, lanBrowse, SBCallback, userData);
    ...
    UpdateFinished =
        gsi_false; // this was set to true from the last update, so we set it back until the new update completes
