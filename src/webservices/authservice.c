...
const char* WS_AUTHSERVICE_NAMESPACES[WS_AUTHSERVICE_NAMESPACE_COUNT] = {WS_AUTHSERVICE_NAMESPACE
                                                                         "=\"http://gamespy.net/AuthService/\""};

const char WS_AUTHSERVICE_SIGNATURE_KEY[] = "BF05D63E93751AD4A59A4A7389CF0BE8"
        ...
        if (gsi_is_false(gsXmlMoveToStart(theResponseXml))
            ...
            if (gsi_is_false(gsXmlReadChildAsInt(theResponseXml, "responseCode", (int*)&response.mResponseCode))) {
                ...
                response.mLoginResult = WSLogin_ServerError;
                       ...
                       || gsi_is_false(gsXmlReadChildAsLargeInt(
                           theResponseXml, "peerkeyprivate", &response.mPrivateData.mPeerPrivateKey.exponent))) {
                response.mLoginResult = WSLogin_ParseError;
            } else {
                MD5_CTX md5;

                // peer privatekey modulus is same as peer public key modulus
                memcpy(&response.mPrivateData.mPeerPrivateKey.modulus,
                       &cert->mPeerPublicKey.modulus,
                       sizeof(cert->mPeerPublicKey.modulus));
                       ...
                       void* userData)
{
    ...
    if (writer != NULL) {
        ...
        if (gsi_is_false(gsXmlWriteOpenTag(writer, WS_AUTHSERVICE_NAMESPACE, WS_AUTHSERVICE_LOGINPROFILE))
            || gsi_is_false(
                gsXmlWriteIntElement(writer, WS_AUTHSERVICE_NAMESPACE, "version", WS_AUTHSERVICE_PROTOVERSION))
            ...
            || gsi_is_false(gsXmlWriteHexBinaryElement(
                writer, WS_AUTHSERVICE_NAMESPACE, "Value", encryptedPassword, GS_CRYPT_RSA_BYTE_SIZE))
        ...
        aTask = gsiExecuteSoap(
            wsAuthServiceURL, WS_AUTHSERVICE_LOGINPROFILE_SOAP, writer, wsiLoginProfileCallback, (void*)requestData);
        if (aTask == NULL) {
            gsXmlFreeWriter(writer);
            gsifree(requestData);
            return WSLogin_OutOfMemory;
                      ...
                      void* userData)
{
    ...
    if (writer != NULL) {
        ...
        aTask = gsiExecuteSoap(
            wsAuthServiceURL, WS_AUTHSERVICE_LOGINUNIQUE_SOAP, writer, wsLoginUniqueCallback, (void*)requestData);
        if (aTask == NULL) {
            gsXmlFreeWriter(writer);
            gsifree(requestData);
            return WSLogin_OutOfMemory;
                          ...
                          void* userData)
{
    ...
    if (writer != NULL) {
        ...
        aTask = gsiExecuteSoap(wsAuthServiceURL,
                               WS_AUTHSERVICE_LOGINREMOTEAUTH_SOAP,
                               writer,
                               wsLoginRemoteAuthCallback,
                               (void*)requestData);
        if (aTask == NULL) {
            gsXmlFreeWriter(writer);
            gsifree(requestData);
                                   ...
                                   void* theRequestData)
{
        ...
        if (gsi_is_false(gsXmlMoveToStart(theResponseXml))
            ...
            if (gsi_is_false(gsXmlReadChildAsInt(theResponseXml, "responseCode", (int*)&response.mResponseCode))) {
                ...
                if ( //gsi_is_false(gsXmlReadChildAsBase64Binary(theResponseXml, "authToken", NULL, &tokenLen)) ||
                    ...
                    || gsi_is_false(
                        gsXmlReadChildAsString(theResponseXml, "partnerChallenge", &challengeStr, &challengeLen))
                    || (tokenLen >= WS_LOGIN_AUTHTOKEN_LEN || challengeLen >= WS_LOGIN_PARTNERCHALLENGE_LEN)) {
                    response.mLoginResult = WSLogin_ParseError;
                } else {
    ...
    if (requestData->mUserCallback.mLoginPs3CertCallback != NULL) {
        WSLoginPs3CertCallback userCallback =
            (WSLoginPs3CertCallback)(requestData->mUserCallback.mLoginPs3CertCallback);
        (userCallback)(translatedResult, &response, requestData->mUserData);
    }
    gsifree(requestData);
    GSI_UNUSED(theRequestXml);
                       ...
                       void* userData)
{
    ...
    if (writer != NULL) {
        ...
        aTask = gsiExecuteSoap(
            wsAuthServiceURL, WS_AUTHSERVICE_LOGINPS3CERT_SOAP, writer, wsLoginPs3CertCallback, (void*)requestData);
        if (aTask == NULL) {
            gsXmlFreeWriter(writer);
            gsifree(requestData);
            return WSLogin_OutOfMemory;
...
gsi_bool wsLoginCertWriteXML(const GSLoginCertificate* cert, const char* aNamespace, GSXmlStreamWriter writer)
{
    ...
    if (gsi_is_false(gsXmlWriteIntElement(writer, aNamespace, "length", cert->mLength))
        ...
        || gsi_is_false(
            gsXmlWriteLargeIntElement(writer, aNamespace, "peerkeyexponent", &cert->mPeerPublicKey.exponent))
        || gsi_is_false(
            gsXmlWriteHexBinaryElement(writer, aNamespace, "serverdata", cert->mServerData, WS_LOGIN_SERVERDATA_LEN))
        || gsi_is_false(
            gsXmlWriteHexBinaryElement(writer, aNamespace, "signature", cert->mSignature, WS_LOGIN_SIGNATURE_LEN))) {
...
gsi_bool wsLoginCertReadXML(GSLoginCertificate* cert, GSXmlStreamReader reader)
{
    ...
    if (gsi_is_false(gsXmlReadChildAsInt(reader, "length", (int*)&cert->mLength))
        ...
        gsi_is_false(
            gsXmlReadChildAsHexBinary(reader, "serverdata", cert->mServerData, WS_LOGIN_SERVERDATA_LEN, &hexlen))
        ||

        gsi_is_false(
            gsXmlReadChildAsHexBinary(reader, "signature", cert->mSignature, WS_LOGIN_SIGNATURE_LEN, &hexlen))) {
        return gsi_false;
    }
    return gsi_true;
}
