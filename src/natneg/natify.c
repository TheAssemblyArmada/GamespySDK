...
void OutputMapping(const AddressMapping* theMap)
{
    if (theMap == NULL) {
        gsDebugFormat(
            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "ERROR: Port mapping not available.");
    } else {
        gsDebugFormat(GSIDebugCat_NatNeg,
                      GSIDebugType_Network,
                      GSIDebugLevel_Comment,
    ...
    if (sock != INVALID_SOCKET) {
        ...
        while (CanReceiveOnSocket(sock)) {
            ...
            if (ptype == NN_ERTTEST) {
                ...
                switch (p.cookie) {
                case packet_map1a:
                    gotADDRESS1a = gsi_true;
                    break;
                case packet_map1b:
                    gotADDRESS1b = gsi_true;
                    break;
                case packet_map2:
                    gotADDRESS2 = gsi_true;
                    break;
                case packet_map3:
                    gotADDRESS3 = gsi_true;
                    break;
                }
...
gsi_bool DetermineNatType(NAT* nat)
{
    ...
    if (nat->mappings[packet_map1a].publicIp == 0 || nat->mappings[packet_map2].publicIp == 0
        || nat->mappings[packet_map3].publicIp == 0)
        return (gsi_false);

    // Is there a NAT?
    if (!nat->portRestricted && !nat->ipRestricted
        ...
        if (!nat->ipRestricted && !nat->portRestricted
    ...
    if (nat->mappings[packet_map1b].publicPort != 0
        && nat->mappings[packet_map1a].publicPort != nat->mappings[packet_map1b].publicPort) {
