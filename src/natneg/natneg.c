...
static void SendReportPacket(NATNegotiator neg)
{
    ...
    gsDebugFormat(GSIDebugCat_NatNeg,
...
static void SendInitPackets(NATNegotiator neg)
{
    ...
    if (!neg->initAckRecv[NN_PT_NN2]) {
        ...
        gsDebugFormat(GSIDebugCat_NatNeg,
                      ...
                      inet_ntoa(*(struct in_addr*)&matchup2ip),
                      MATCHUP_PORT2);
    ...
    if (!neg->initAckRecv[NN_PT_NN3]) {
        ...
        gsDebugFormat(GSIDebugCat_NatNeg,
                      ...
                      inet_ntoa(*(struct in_addr*)&matchup3ip),
                      MATCHUP_PORT3);
...
static void SendPingPacket(NATNegotiator neg)
{
    ...
    gsDebugFormat(GSIDebugCat_NatNeg,
                  ...
                  inet_ntoa(*(struct in_addr*)&neg->guessedIP),
                  neg->guessedPort,
                  neg->gotRemoteData);
...
NegotiateError NNBeginNegotiation(int cookie,
                                  int clientindex,
                                  NegotiateProgressFunc progresscallback,
                                  NegotiateCompletedFunc completedcallback,
                                  void* userdata)
{
    return NNBeginNegotiationWithSocket(
        INVALID_SOCKET, cookie, clientindex, progresscallback, completedcallback, userdata);
}
...
NegotiateError NNBeginNegotiationWithSocket(SOCKET gamesocket,
                                            int cookie,
                                            int clientindex,
                                            NegotiateProgressFunc progresscallback,
                                            NegotiateCompletedFunc completedcallback,
...
static void NegotiateThink(NATNegotiator neg)
{
    ...
    {
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Memory, GSIDebugLevel_Notice, "Removing canceled negotiator\n");
        RemoveNegotiator(neg);
        return;
    }
    ...
    if (neg->state == ns_initsent || neg->state == ns_connectping) //see if we need to resend init packets
    {
        if (current_time() > neg->retryTime) {
            if (neg->retryCount > neg->maxRetryCount) {
                gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "RETRY FAILED...\n");
                ...
                gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "[retry]\n");
            }
        }
    }
    ...
    if (neg->state == ns_reportsent && current_time() > neg->retryTime) {
        if (neg->retryCount > neg->maxRetryCount) {
            gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "REPORT retry FAILED...\n");

            neg->completedCallback(
                neg->result, neg->connectedSocket, (struct sockaddr_in*)&neg->remoteaddr, neg->userdata);
...
static void SendConnectAck(NATNegotiator neg, struct sockaddr_in* toaddr)
{
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending connect ack...\n");
...
static void ProcessConnectPacket(NATNegotiator neg, NatNegPacket* p, struct sockaddr_in* fromaddr)
{
    gsDebugFormat(GSIDebugCat_NatNeg,
                  ...
                  inet_ntoa(*(struct in_addr*)&p->Packet.Connect.remoteIP),
...
static void ProcessPingPacket(NATNegotiator neg, NatNegPacket* p, struct sockaddr_in* fromaddr)
{
    ...
    else //they have our data, and we have their data - it's a connection!
    {
        ...
        {
            gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "CONNECT FINISHED\n");
...
static void ProcessInitPacket(NATNegotiator neg, NatNegPacket* p, struct sockaddr_in* fromaddr)
{
    ...
    case NN_ERTTEST:
        ...
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Got ERT\n");
        p->packettype = NN_ERTACK;
        SendPacket(neg->negotiateSock, fromaddr->sin_addr.s_addr, ntohs(fromaddr->sin_port), p, INITPACKET_SIZE);
        break;

    case NN_REPORT_ACK:
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Got REPORT ACK\n");
        neg->state = ns_reportack;
...
void NNProcessData(char* data, int len, struct sockaddr_in* fromaddr)
{
    ...
    if (ptype == NN_CONNECT || ptype == NN_CONNECT_PING) { //it's a connect packet
