...
static gsi_bool CreateReportAndSubmit()
{
    ...
    for (i = 0; i < SCTEST_NUM_PLAYERS; i++) {
        aResult = scCreateReport(gPlayerData[i].mStatsInterface,
                                 SCTEST_KEY_HEADER_VERSION,
                                 SCTEST_NUM_PLAYERS,
                                 SCTEST_NUM_TEAMS,
        ...
        for (k = 0; k < SCTEST_NUM_PLAYERS; k++) {
            ...
            scReportSetPlayerData(gPlayerData[i].mReport,
                                  (gsi_u32)k,
                                  gPlayerData[k].mConnectionId,
                                  gPlayerData[k].mTeam,
                                  playersGameResult,
                                  gPlayerData[k].mProfileId,
                                  &gPlayerData[k].mCertificate,
                                  gPlayerData[k].mStatsAuthdata);
            scReportAddStringValue(
                gPlayerData[i].mReport, SCTEST_KEY_PLAYER_NICK, gPlayerData[k].mCertificate.mUniqueNick);
...
static gsi_bool RunTests(gsi_u8 loginMethod, gsi_bool isLoginOnly)
{
    ...
    if (!isLoginOnly) {
        ...
        scCreateSession(gPlayerData[0].mStatsInterface,
