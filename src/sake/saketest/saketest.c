...
#define CHECK_GP_RESULT(func, errString)                                                                               \
    if (func != GP_NO_ERROR) {                                                                                         \
        printf("%s\n", errString);                                                                                     \
        exit(0);                                                                                                       \
    }
...
static void DebugCallback(GSIDebugCategory theCat,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void CreateRecordCallback(SAKE sake,
                                 SAKERequest request,
...
static void UpdateRecordCallback(SAKE sake,
                                 SAKERequest request,
                                 SAKERequestResult result,
                                 void* inputData,
                                 void* outputData,
...
static void SearchForRecordsCallback(SAKE sake,
                                     SAKERequest request,
                                     SAKERequestResult result,
                                     void* inputData,
                                     void* outputData,
...
static void GetSpecificRecordsCallback(SAKE sake,
                                       SAKERequest request,
                                       SAKERequestResult result,
                                       void* inputData,
                                       void* outputData,
...
static void GetRandomRecordCallback(SAKE sake,
                                    SAKERequest request,
                                    SAKERequestResult result,
                                    void* inputData,
                                    void* outputData,
...
static void RateRecordCallback(SAKE sake,
                               SAKERequest request,
                               SAKERequestResult result,
                               void* inputData,
                               void* outputData,
...
static void GetRecordLimitCallback(SAKE sake,
                                   SAKERequest request,
                                   SAKERequestResult result,
                                   void* inputData,
                                   void* outputData,
...
static GHTTPBool
UploadCompletedCallback(GHTTPRequest request, GHTTPResult result, char* buffer, GHTTPByteCount bufferLen, void* param)
{
...
static GHTTPBool
DownloadCompletedCallback(GHTTPRequest request, GHTTPResult result, char* buffer, GHTTPByteCount bufferLen, void* param)
{
    SAKEFileResult fileResult;
    ...
    printf("File Download: Downloaded %" PRI_BYTECOUNT " byte file\n", bufferLen);

    return GHTTPTrue;
}

static void
postCallback(GHTTPRequest request, int bytesPosted, int totalBytes, int objectsPosted, int totalObjects, void* param)
{
    printf("==============================\n");
...
static void RunTests(SAKE sake)
{
...
#if 1
    {
        ...
        ghttpPostAddFileFromMemory(
            post, _T("memory.file"), (const char*)&data, SAKE_UPLOAD_AMOUNT, _T("memory.file"), NULL);
...
static void LoginAndAuthenticate(SAKE sake)
{
    ...
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_REQUEST, (GPCallback)RecvBuddyRequest, NULL),
                    "gpSetCallback failed");
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_STATUS, (GPCallback)RecvBuddyStatus, NULL),
                    "gpSetCallback failed");
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_MESSAGE, (GPCallback)RecvBuddyMessage, NULL),
                    "gpSetCallback failed");
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_GAME_INVITE, (GPCallback)RecvGameInvite, NULL),
                    "gpSetCallback failed");
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_TRANSFER_CALLBACK, (GPCallback)TransferCallback, NULL),
                    "gpSetCallback failed");
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_AUTH, (GPCallback)RecvBuddyAuth, NULL), "gpSetCallback failed");
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_REVOKE, (GPCallback)RecvBuddyRevoke, NULL),
                    "gpSetCallback failed");

    // connect to GP
    printf("Connecting to GP...\n");
    CHECK_GP_RESULT(
        gpConnect(pconn, NICKNAME, EMAIL, PASSWORD, GP_NO_FIREWALL, GP_BLOCKING, (GPCallback)ConnectResponse, NULL),
        "gpConnect failed");
