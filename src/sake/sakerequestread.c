...
static SAKEStartRequestResult SAKE_CALL sakeiSearchForRecordsFillSoapRequest(SAKERequest request)
{
    ...
    if (input->mTargetRecordFilter != NULL)
        gsXmlWriteTStringElement(
            request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE, "targetfilter", input->mTargetRecordFilter);

    // write the surrounding record count
    gsXmlWriteIntElement(
        request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE, "surrounding", (gsi_u32)input->mSurroundingRecordsCount);
...
{
    static SAKEIRequestInfo info = {sizeof(SAKESearchForRecordsOutput),
...
{
    static SAKEIRequestInfo info = {sizeof(SAKEGetMyRecordsOutput),
...
{
    static SAKEIRequestInfo info = {sizeof(SAKEGetSpecificRecordsOutput),
...
static SAKERequestResult sakeiGetRandomRecordProcessSoapResponse(SAKERequest request)
{
    ...
    SAKERequestResult result =
        sakeiReadOutputRecords(request, &records, &numRecords, input->mNumFields, input->mFieldNames);
...
{
    static SAKEIRequestInfo info = {sizeof(SAKEGetRandomRecordOutput),
